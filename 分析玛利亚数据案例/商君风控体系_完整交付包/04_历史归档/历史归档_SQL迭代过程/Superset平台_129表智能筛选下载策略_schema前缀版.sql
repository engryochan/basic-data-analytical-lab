...
-- ██████████████████████████████████████████████████████████████████████████
WITH 候选_投注异常 AS (
    SELECT CAST(bet05 AS BIGINT) AS uid FROM ods_mariadb_2b.ods_a168_bet01
    GROUP BY CAST(bet05 AS BIGINT)
    HAVING COUNT(*) > 50
...
候选_同桌异常 AS (
    SELECT DISTINCT CAST(a.bet05 AS BIGINT) AS uid
    FROM ods_mariadb_2b.ods_a168_bet01 a
    JOIN ods_mariadb_2b.ods_a168_bet01 b ON a.bet03 = b.bet03 AND a.bet05 <> b.bet05
    GROUP BY a.bet05 HAVING COUNT(DISTINCT a.bet03) > 50
),
候选_系统标记 AS ( SELECT DISTINCT CAST(bet05 AS BIGINT) AS uid FROM ods_mariadb_2b.ods_a168_risklist ),
候选_荷官关联 AS (
    SELECT DISTINCT CAST(bet05 AS BIGINT) AS uid FROM ods_mariadb_2b.ods_a168_bet01 GROUP BY eid, bet05 HAVING COUNT(*) > 50
),
候选_最终 AS (
    SELECT uid FROM 候选_投注异常
    UNION SELECT uid FROM 候选_同桌异常
...
SELECT * FROM ods_mariadb_2b.ods_a168_bet01
