...
Usage:
  sparkr_submit.sh \
    ...
    [--r-library-archive s3://bucket/r-libraries.tar.gz] \
    [--jars URI[,URI...]]... \
...
Optional environment:
  ...
  SPARKR_DRIVER_MEMORY=4g
  SPARKR_DRIVER_CORES=1
  SPARKR_EXECUTOR_MEMORY=6g
  SPARKR_EXECUTOR_CORES=2
  SPARKR_NUM_EXECUTORS=4
  SPARKR_SHUFFLE_PARTITIONS=64
  YARN_QUEUE=prod
  SPARK_SQL_SESSION_TIMEZONE=Asia/Shanghai

R library archive:
  The archive is not downloaded or inspected by this script. It must be built
  for the target EMR release, R version, operating system, and CPU architecture.
  Its extracted root is used as R_LIBS_USER on the driver and executors.
...
driver_memory="${SPARKR_DRIVER_MEMORY-4g}"
driver_cores="${SPARKR_DRIVER_CORES-1}"
executor_memory="${SPARKR_EXECUTOR_MEMORY-6g}"
executor_cores="${SPARKR_EXECUTOR_CORES-2}"
num_executors="${SPARKR_NUM_EXECUTORS-4}"
shuffle_partitions="${SPARKR_SHUFFLE_PARTITIONS-64}"
...
if [[ -n "$r_library_archive" ]]; then
  require_s3_uri "--r-library-archive" "$r_library_archive"
  [[ "$r_library_archive" == *.tar.gz ]] ||
    fail "--r-library-archive must reference a .tar.gz object"
fi
...
if len(arguments) != jar_spec_count + file_spec_count + conf_entry_count + argument_count:
    raise SystemExit("invalid SparkR payload argument count")

offset = 0
jar_specs = arguments[offset:offset + jar_spec_count]
offset += jar_spec_count
file_specs = arguments[offset:offset + file_spec_count]
offset += file_spec_count
conf_entries = arguments[offset:offset + conf_entry_count]
...
if files:
    payload["files"] = files
if archive:
    payload["archives"] = [f"{archive}#rlibs"]
    conf.update(
        {
            "spark.yarn.appMasterEnv.R_LIBS_USER": "./rlibs",
            "spark.executorEnv.R_LIBS_USER": "./rlibs",
        }
    )
...
r_library_environment="disabled"
[[ -n "$r_library_archive" ]] && r_library_environment="enabled"
printf 'Submitting Livy SparkR batch: name=%s file=%s queue=%s rLibraryArchive=%s driverMemory=%s driverCores=%s executorMemory=%s executorCores=%s numExecutors=%s shufflePartitions=%s jars=%s files=%s conf=%s\n' \
  "$name" \
  "$application_file" \
  "$queue" \
  "$r_library_environment" \
  "$driver_memory" \
  "$driver_cores" \
  "$executor_memory" \
  "$executor_cores" \
...
if ! submit_response="$(
  ...
  printf 'Failed to submit Livy SparkR batch: url=%s/batches\n' "$LIVY_URL" >&2
  exit 1
fi
...
if [[ ! "$submit_status" =~ ^[0-9]{3}$ ]]; then
  printf 'Failed to parse Livy SparkR submit HTTP status\n' >&2
  exit 1
fi

if (( submit_status < 200 || submit_status >= 300 )); then
  printf 'Livy SparkR submit failed: status=%s url=%s/batches\n' "$submit_status" "$LIVY_URL" >&2
...
'
  ...
  printf 'Livy SparkR submit response did not contain a valid integer batch id\n' >&2
  exit 1
fi

printf 'Livy SparkR batch submitted: id=%s\n' "$batch_id"
[[ "$no_wait" == true ]] && exit 0
...
fetch_logs() {
  ...
  if ! log_response="$(
    ...
    printf 'Failed to query Livy SparkR batch logs: id=%s\n' "$batch_id" >&2
    return
  fi
  ...
  if [[ "$log_status" =~ ^[0-9]{3}$ ]] &&
    ...
    printf 'Livy SparkR log query failed: id=%s status=%s\n' "$batch_id" "$log_status" >&2
  fi
}
...
while true; do
  if ! state_response="$(
    ...
    printf 'Failed to query Livy SparkR batch state: id=%s\n' "$batch_id" >&2
    exit 1
  fi
  ...
  if [[ ! "$state_status" =~ ^[0-9]{3}$ ]] ||
    ...
    printf 'Livy SparkR state query failed: id=%s status=%s\n' "$batch_id" "$state_status" >&2
    exit 1
  fi

  if ! state="$(printf '%s' "$state_body" | parse_state)"; then
    printf 'Livy SparkR state response did not contain a valid state: id=%s\n' "$batch_id" >&2
    exit 1
  fi

  printf 'Livy SparkR batch state: id=%s state=%s elapsed=%ss\n' "$batch_id" "$state" "$elapsed"
  case "$state" in
    success)
      printf 'Livy SparkR batch succeeded: id=%s\n' "$batch_id"
      exit 0
      ;;
    dead|error|killed)
      printf 'Livy SparkR batch failed: id=%s state=%s\n' "$batch_id" "$state" >&2
      fetch_logs
      exit 1
      ;;
    not_started|starting|running|recovering)
      if (( elapsed >= LIVY_MAX_WAIT_SEC )); then
        printf 'Livy SparkR batch wait timed out: id=%s maxWait=%ss\n' "$batch_id" "$LIVY_MAX_WAIT_SEC" >&2
        fetch_logs
        exit 1
      fi
      ;;
