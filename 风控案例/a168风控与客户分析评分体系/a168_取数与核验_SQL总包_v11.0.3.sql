/* ╔═══════════════════════════════════════════════════════════════════════════╗
   ║  ★ 本档版本：v11.0.3（PATCH · 身份规则补注 ＋ 块注释闭合斧正）★           ║
   ║  ── 2026-08-23 · 三层身份 REL-0005 / C1 / v11.0.3（schema_diff = ∅）───── ║
   ║  一、等价声明：原 v11 ≡ v11.0.0。此系**声明**非改名——版本号是身份，       ║
   ║     追溯改名即身份冲突（§Z-16、六元组、下游 must= 皆以 v11 为锚）。        ║
   ║     v11.0.0 落盘时加 _superseded 归档，禁删。                             ║
   ║  二、本档只含 G-4／G-5 两项注释斧正，SQL 主体、字段、输出文件名一字未动。   ║
   ║     **纯代码 MD5 恒为 ce768c35b8d7aeb5ac28079166a8c422（5,961 行）**，     ║
   ║     与 v11.0.0 逐字节相同——此即 PATCH 级之硬判据，可机器验。             ║
   ║  三、未含者：G-1（Δ_old-new 正名）／G-2（判词四态升五态）／G-3（三诊断入册）║
   ║     ／G-6（状态登记）。其版级依「版本编号纪律 v1.0.0」逐级晋升：           ║
   ║       G-3 新增段落（只增不改）  → v11.1.0（MINOR）                        ║
   ║       G-1／G-2 破坏表头契约     → v12.0.0（MAJOR，须先经 v11.1.x）        ║
   ║       G-6 状态登记（纯注释）    → 可并入任一 PATCH                        ║
   ║  四、禁跳级：不得由 v11.0.3 直接跃至 v12.0.0；须循 PATCH → MINOR → MAJOR。 ║
   ╠═══════════════════════════════════════════════════════════════════════════╣
   ...
   ═══ 版本身份六元组（2026-08-13 立 · 铁律 T3·L7·S3）═══════════════════════
     ...
     故同一份文件存在三个互不相通的 MD5，取用时务必声明取的是哪一个：
       · 原样（CRLF）      —— 传输与归档以此为准
       · LF 归一后         —— 跨平台比对以此为准
        · 剥离注释后        —— 注释体例第六条所载者，仅用于验证注释零改动
      ⚠ 萃取规则（v11.0.3 补注 · 此前只书「剥离行尾注释后」，漏载剥块注释一步，
        致第三方照字面复现必失败——规则未写全之锚，不是锚）：
          ① 以非贪婪跨行正则剥去**块注释**（起始符至闭合符之整段，含符本身）
          ② 按行切（CRLF 或 LF 皆可，二者结果同）
          ③ 剥去双连字号起之**行内注释**
          ④ 逐行 rstrip（去行尾空白）
          ⑤ 弃空行
          ⑥ 以 **LF** 连接
          ⑦ UTF-8（无 BOM）编码后取 MD5
        本规则为 VERSION-META-02 之 PATCH 判据基础：规则不完整，PATCH 判定即不可复核。
        现行锚值：ce768c35b8d7aeb5ac28079166a8c422 · 5,961 行（v11.0.0 ~ v11.0.3 恒定）。
   ...
   ═══ 常数集中登记（2026-08-13 立 · 铁律 T3·L2·S3）═════════════════════════
     ...
     使「改一处漏一处」由不可察变为可核对：
       ...
       常数名          取值                出现处数   语义
       WIN_START       '2026-03-21'        187        窗口左端（闭区间）
       WIN_END         '2026-08-07'        261        窗口右端（开区间）
       GAME_BACCARAT   '101'               191        bet02 游戏类别：百家乐
       CAT_NORMAL      '1'                 213        category：一般注单
       REBET_NO        'N'                 119        bet38：非重对
       TESTLINE        age022 = '1'        45         公司测试线代理（214 条）
       LATE_DEF        shoe_pos >= 0.80    —          靴末段判据（2026-08-12 斧正）
     ⚠ 2026-08-23 v11 处数重登（全文检索现算）：旧表之 89／89／64／49／81 与卷首
       「v5 实测 179／248」互斥，皆版本失同步之遗留；今以现算取代并统一口径——
       **本表处数一律为「全文出现次数（含注释区）」**，此前未言明其口径，故生歧义。
       本轮新增 §R01-chk 族四条，各引窗口两端、三闸常数与 TESTLINE 各一次，故各有增益；
       上表处数系本轮（含 2026-08-23 注释斧正）改毕后之现算实测，含变更记录内之引用。
       ⚠ 注释改动亦会移动处数：本轮注释斧正后 CAT_NORMAL 由 211 增至 213（§Z-16 之 v11 条目
         与本注各引用了一次 category='1' 形之字面量），余五项未变。
         此即本表纪律之实例——不得以「未动代码」为由跳过重新点数。
       改动后须重新点数，不得沿用旧数。
       ⚠ 2026-08-18 v4：新增 §Z-14 零值诊断（SQL 主体首次改动，详见文末 §Z-16）。
   ⚠ 2026-08-18 注释斧正（代码零改动）：shoe_len 之定义，其注释与 CTE 名历来称其为
         「靴长／该靴长度」，与所算之『最大局号』语义不符，易致下游以 COUNT 重实现。
         本轮统一各定义行为同一命名警示横幅，并改正靴号取列与 ordx 之注释。
         ★★ 2026-08-23 全档现算斧正（旧文「三处」与「皆 GROUP BY shoe_id, table_id」
            二语皆与实测不符，今正之）★★
            实测 shoe_len 定义共 **六处**，粒度分两类，非「皆一致」：
              · GROUP BY shoe_id, table_id（含桌号）四处 —— §R01／§B01／§B01-D／§P11b
              · GROUP BY bet03（**不含桌号**）二处 —— §P12a 与 §P13 各一
            ★★ G-5 · 2026-08-23 紧急注释纠错（本段原文已被实测直接推翻）★★
              原文曾断言：「不含桌号者会把异桌同号靴合并，其 max_round 被系统性夸大，
              致该二处之 shoe_pos 被系统性压低」。该断言**不成立**，今撤回并正之。
              §R01-0b／§R01-0d 全域实测（2026-08-23）：
                · pct_cross_table ＝ 0        · max_tables_per_shoe ＝ 1
                · n_shoes(GROUP BY shoe_id) ＝ n_shoes(GROUP BY shoe_id, table_id)
                  ＝ 123,357，delta ＝ 0      · avg max_round 两者同为 49.7806
                · §R01-0d 跨桌重号逐行证据：零行（零行即证据，已照登）
              故 GROUP BY shoe_id 与 GROUP BY shoe_id, table_id 在本核验窗口内
              **逐条一致**；本轮不存在由省略 table_id 所致之跨桌合并证据。
              ⚠ 证据边界（不得外推）：上述系**本核验窗口、本数据口径下之 OBSERVED
                事实**，非「shoe_id 永久全局唯一」之保证。换窗口、换产品、换库，
                须重新核验。
              ⚠ 另须记明：GROUP BY dt, shoe_id, table_id（即 §R01-chk 族之新粒度）
                会**切断跨零点续接之同一物理靴**（§R01-0c 实测 3,092 条，
                pct_contiguous ＝ 100%），故**不得以其替代现行之 (shoe_id, table_id)**。
              ⚠ 本轮只斧正注释、不动代码；粒度是否统一属 v12 staging 之 G-1~G-6。
         ⚠ 2026-08-23 补：新立之 §R01-chk 族刻意**不沿用 shoe_len 之名**，改称
           shoe_new／shoe_old——核验该量之节反用易混之名，是自造下一个坑；
           且该族并报新旧两粒度，其 max_round 语义与本横幅所载完全一致。
   ...
   ═══ 使用纪律（实测教训固化，逐条有血泪）═══════════════════════════════════
     ...
     ⑨ 连接前先估配对规模：凡两表连接键「不含局键或会员键」、仅靠桌号或日期
        ★ 2026-08-23 斧正：本行原以连续星号强调「不含局键或会员键」，其后仅隔
          CJK 字符即接斜杠——正是 §DX-02 所载之塌缩模式（星号＋斜杠）。今去星号、
          斜杠改「或」，双管齐下。此为全档唯一一处该模式，已扫描确认。
   ...
   ═══ 编号三轴命名纪律（2026-08-23 立 · 详见 §ECON 第一节）════════════════
     · T-01 ~ T-15 ＝ 风险类型（typology），登记册 registry_risk_typology 之主键
     · R01／R02／R03b… ＝ 本包之分析段与交付件编号
     · V ＝ 价值轴（stake_all 分位），既非风险亦非经济结果
     三轴各有其义，禁互代。T-03 之主交付件恰为 R01_late_shoe.csv，
     然「T-03 ≠ R01」——前者为风险类型，后者为该类型之一份交付件。
     ⚠ 尤须记明：roi 不是任何一类 T 之判据。ROI 系经济后果，非风险原因；
       以 roi > 0 出名单、行处置者判 S1 FATAL（零假设下会员 ROI 之期望非 0，
       而为 μ0 ＝ −hold%，故「跑赢 0」不等于「跑赢赌场」）。
   ...
   【第四批 · 规则、经济学与特征】
     ...
     §R01-0 → 只看屏幕                           靴号粒度验证（★ 本族前置闸）
     §R01-chk → 数据库/R01chk_shoe_len_by_table.csv  靴长分母核验·逐桌
     §R01-inv → 数据库/R01inv_shoe_len_census.csv    靴长分母核验·全域普查
     §R01-anm → 数据库/R01anm_shoe_anomaly.csv       靴长分母核验·异常靴清单（账务对象）
     §ECON → 无查询体                            经济结果层口径登记（规章类；三分律与指标字典）
...
--      本包只保留可落实商业方案的最新版本；告示保留于此，以防日后误跑旧版。 */
-- ⚠ SC · 块注释闭合斧正（v11.0.3 · 2026-08-23）：
--   上方自「✅ 字段已实测确认」起之块注释，原**无自身闭合符**，系被其后 S-04b
--   横幅之闭合符顺手关掉。当时无碍纯属侥幸——属潜伏之「静默吞查询」风险：
--   设若日后删改该横幅，该起始符将一路吞至下一个闭合符，把其后整条 SELECT
--   静默注释掉——查询不报错，只是凭空消失。今补其闭合符，令二段各自独立。
--   ★ 连带立一条注释体例：**块注释内一律不得书写字面之块注释起始／闭合符**，
--     须以「块注释起始符」「块注释闭合符」等描述语代之——否则文中之闭合符
--     会当场提前关闭该块，其后之注释文字即被引擎当作 SQL 解析。
--     本条系实证所得：v11.0.3 施工首版即因此致纯代码行由 5,961 暴增至 6,347。
...
WHERE dt >= '2026-03-21' AND dt < '2026-08-07'                                                      -- 过滤条件：限定 dt不少于 '2026-03-21'、dt小于 '2026-08-07'，涉 dt（营业日）
  ...
  AND UPPER(TRIM(bet38)) = 'N';                                                                     -- 并列条件：限定 UPPER(TRIM(bet38))等于 'N'，涉 bet38（重對）
...
/* ───────────────────────────────────────────────────────────────────────────
   ...
   ★ 本行系**家族头**，非查询节：无查询体、无交付件，仅统辖 §DX-01 ~ §DX-05。
   ─────────────────────────────────────────────────────────────────────────── */
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ★ 本节系**纯存档说明**：无查询体、无交付件、无导出声明行。
     其后之 SELECT 属 §DX-03，不属本节——二者原共用一个块注释，视觉上易被读成
     一段前言，2026-08-23 已拆为两块（见文末 §Z-16 · v11 注释斧正）。
   ═══════════════════════════════════════════════════════════════════════════ */


/* ───────────────────────────────────────────────────────────────────────────
   §DX-03 · 取列定义的通用写法（比 DESC 好用：带列注释与顺序，一条顶三条）
   ★ 其后之 SELECT 属本节（§DX-03），非上一节（§DX-02，纯存档说明无查询体）。
   ▸ 实测（2026-08-08）：本条已跑通，68 列定义到手，关键确认已回填全包。
   ⚠ 破例声明（对照卷首导出声明纪律配套②「标注不需要者，注释内不得出现任何
     导出文件名」）：下方两个文件名系**反向禁令**，非导出指引——一为留档可用之名、
     一为绝不可用之名；去之则禁令失去指涉对象，故刻意保留并在此声明破例事由。
   ─────────────────────────────────────────────────────────────────────────── */
-- ▸ 导出：不需要 —— §DX-03 取列定义（屏幕看结果，把列名填回 §E02b / §K01c）。
--   若要留档，存为「数据库/V_columns_dict.csv」。
--   ⛔ 绝不可存为 R_rebate_dist.csv —— 那是 E1-11（退水分布）的专属文件名；
--      §DX-02 提到它只因旧报错信息里的 token 恰是这个词，与本条输出无关。
...
/* ───────────────────────────────────────────────────────────────────────────
   ...
   ★★ 2026-08-12 B-01 斧正：判据由绝对局号 round_no>=50 改为相对靴位 shoe_pos>=0.80 ★★
     ...
     ⚠ 2026-08-23 边界补注：判据含等号（>=）且局号为正整数，故实收局数为
       N − ⌈0.80N⌉ + 1，其占比略高于 20%——N=50 时落入第 40~50 局共 11 局，
       即 22%，非 20%。「约 20%」系区间之名义值；据以做零偏好基准比对时应改用
      ★ Z2-07（2026-08-23 · 通用公式入册，禁再以常数 20%／22% 代之）：
          late_n(N)      ＝ N − ⌈0.80 × N⌉ + 1        （实收局数）
          late_share0(N) ＝ late_n(N) ÷ N             （零偏好基准占比）
        其值随 N 跳变（锯齿函数）：N=45 → 10/45 ＝ 22.22%；N=50 → 11/50 ＝ 22.00%；
        N=52 → 11/52 ＝ 21.15%；N=56 → 12/56 ＝ 21.43%。
        ⚠ 本包实测靴长四族（p50 ＝ 56／52／49／48）其基准值各异；以单一常数作
          全域基准者，对短靴族偏松、对长靴族偏紧，判 S2。
        ⚠ 凡比对 late_share 与基准者，须**逐靴以其自身 N 现算 late_share0(N)**。
       上式现算，否则基准偏低约 2 个百分点。此为注释补注，代码不动。
   ★ 新增 late_def 口径锁列：旧版 CSV 因缺此列而在报告侧读入时当场报错，
     使「新文字配旧数字」的静默语义漂移变成显式失败。
   对应报告：@sec-r01
   ★ 2026-08-23 立：本节输出含 roi_late／roi_early／stake_late_avg／stake_early_avg
     等经济列，然此等列**非本节判据**，亦非 T-03 之判据——T-03 之判据列在册者为
     late_share／shoe_pos_p50／stake_ramp／tail_share，判级皆 PROFILE_ONLY 且已
     判 FATAL。经济列之身分见 §ECON（经济结果层口径登记），禁作触发器。
   ★ 2026-08-23 立：本节坐标系之分母 MAX(round_no) 之核验，见其后 §R01-chk 族
     （§R01-0 粒度验证／§R01-chk 逐桌／§R01-inv 全域普查／§R01-anm 异常清单）。
     二者互为回指，删其一即血统断裂；本节代码不因该族而改动。
...
/* ═══════════════════════════════════════════════════════════════════════════
   §R01-chk 族 · 靴长分母核验（核验类，非取数类）
   ───────────────────────────────────────────────────────────────────────────
   立节缘由（2026-08-23 · 用户令）：
     §R01 之坐标系 shoe_pos = round_no ÷ MAX(round_no)，其分母历来判为
     「观测靴长之下偏代理」（见卷首 shoe_len 命名警示横幅与 §R01 之 B-01 斧正）。
     该判级自立以来未经实证，属在册 P0 待表项。本族即其关闭工具。
   与 §R01 之关系：本族核验 §R01 之坐标系分母，二者互为回指——
     §R01 头注已加「本节分母之核验见 §R01-chk 族」，删其一即血统断裂。
   命名从既有体例：§R02-0 之「-0 屏幕诊断」、§R03-chk 之「-chk 核验」、
     §R03-inv 之「-inv 普查」皆在册；本族新增 -anm 后缀（异常清单），
     首见于此，已录入 §Z-16。
   四闸声明（对照 §Z-13 之「⛔ 交付件缺闸 14 条」）：
     本族四条**四闸齐备**——剔测试线（五路 LEFT JOIN + COALESCE IS NULL）、
     版本去重（rn = 1）、一般注单（category = '1'）、非重对（bet38 = 'N'），
     另加数值守门三项（bet05 > 0 / bet11 > 0 / bet04 > 0）。不入缺闸名单。
   ★ CTE 血统：test_agents / ranked / base 三段取自 §R01 原文，
     **唯一改动 = ranked 与 base 增列 b.dt（投影增列，筛选条件一字未改）**。
     增 dt 之故：§R01-0 须验靴号是否跨日复用，无 dt 即无从验。
     此改动只在本族之内，§R01 本体不动——避免动料源而累及下游三档。
   ★ shoe CTE 之命名：本族一律用 shoe_new / shoe_old，不复用 shoe_len 之名。
     卷首命名警示横幅已载：shoe_len 所算者为「最大局号」非「局数」，
     其名与义不符；本族既为核验该量而立，更不得沿用易混之名。
   ★★ 粒度裁定（G-4 · 2026-08-23 据实测反转；原文方向错误，已撤回）★★
     三层粒度：
       g1 = shoe_id                    —— §P12a／§P13 现行
       g2 = shoe_id, table_id          —— §R01／§B01／§B01-D／§P11b 现行
       g3 = dt, shoe_id, table_id      —— 本族所用之「新粒度」
     §R01-0b／0c／0d 全域实测（2026-08-23）：
       · g1 靴数 123,357 ＝ g2 靴数 123,357，delta ＝ 0；avg max_round 同为 49.7806
       · g3 靴数 126,449 ＝ g2 ＋ 3,092，而 3,092 恰等于 §R01-0 之 n_cross_day
       · 跨日 3,092 组：day_span 全为 1、pct_contiguous ＝ 100%、
         restart_at_1 ＝ 0、overlap ＝ 0、avg_join_gap ＝ 1.022
         （前日止于第 25.50 局、次日自第 26.52 局续接，合并后止于第 50.20 局）
     故：跨日之成因系**同一物理靴跨零点被营业日切开**，非靴号复用；
       **g2 为 §R01 之正式分析粒度**（本窗口内等价于 g1）；
       **g3 系人为把一条物理靴切成两片，仅作诊断对照，不得作正式分母**。
     原文所谓「旧粒度会把异日同号靴合并、MAX(round_no) 被系统性夸大」，
     方向恰相反：受损者是 g3（切断），非 g2（合并）。今撤回该断言。
     ⚠ 本族之 n_shoes_delta 与 avg_maxround_inflation 两列名皆含方向预设，
       其正名（Δ_old−new）属 v12 staging 之 G-1，本件不施工。
     ⚠ 证据边界：以上系本核验窗口、本数据口径之 OBSERVED 事实，换窗口须重验。
     ⚠ Z2-04（2026-08-23 · 判级两分，两句不可互换）：
       可写：「在**本核验窗口（2026-03-21 ~ 2026-08-06）、本数据口径**下，
             g1 ≡ g2 且 g3 ＝ g2 ＋ 3,092」——此为 **OBSERVED**。
       不可写：「shoe_id 在本表全局唯一」「跨桌永不重号」——此系 **INFERRED
             之外推**，本核验未曾测过窗外、他产品、他库。
       ★ 判级之别：**窗内实测所得为 OBSERVED；由之外推至窗外者为 INFERRED**。
         二者混书，下游即把一个窗口内之观察当成表结构之永久保证。
   ★★ 裁定纪律（永久生效，不因本族结果而松动）★★
     ★ REFUTED 禁复活之机械禁令（Z2-08 · 2026-08-23 · 自「劝诫」升为「门禁」）：
       原文以说理禁其复活，属**劝诫**，可被下一位维护者以「新证据」绕过。今立门禁：
         凡 label_validation_status ＝ REFUTED 或 causal_status ＝ REFUTED 者，
         **不得**因下列任一而恢复其 admit_to_risk_decision：
           ① 上游诊断结论改变（粒度、分母、坐标系之澄清）；
           ② 阈值重选（改 0.80 为他值、改分位为他分位）；
           ③ 子样本、分层、换窗后之显著结果；
           ④ 新增协变量后之显著结果；
           ⑤ 经济结果层观察到 ROI／GGR 为正。
         **唯一**可恢复之途：**重跑原四项独立验证且四项皆翻转**，并另立变更单，
         记明四项之新旧读数逐项对照。
       ⚠ 本禁令之对象是 REFUTED 之**状态**，非某一判据之名；故对 T-03 成立者，
         对此后任何被判 REFUTED 者同样成立。
     本族任何结果皆**不得复活 late_share**：若约 50 局系真靴长，则
     shoe_pos>=0.80 量的就是靴末段而其已被四项独立检验证伪；若系低估，
     则该判据连自身所称之物都未曾测过，证伪只会更彻底。
     本族能改的只有 shoe_pos 坐标系之判级，及其是否仍配作 §R01 之横轴。
     ★★ shoe_pos 身份锁（Z2-06 · 2026-08-23 新立）★★
       shoe_pos ＝ round_no ÷ MAX(round_no)，其分母系**该靴实际出现之最大局号**，
       非该靴之**牌张容量**。故其所量者为 **observed position（已观测位置比）**，
       **非 deck-depth（发牌深度）**，亦非切牌位之函数。二者之别至关紧要：
         · observed position 之分母随该靴**实际打了多少局**而变——早收之靴
           （换靴、故障、清台）分母偏小，同一物理深度会被算得偏「晚」；
         · deck-depth 之分母系牌靴容量（八副 416 张之属），与打了几局无关。
       ⚠ 故 shoe_pos ＝ 0.80 **不等于**「已发出八成牌」，只等于
         「在该靴已观测之局序中位居后二成」。
       ⚠ 凡以 shoe_pos 论「算牌优势随发牌深度累积」者属**坐标系错配**，判 S1；
         真欲论深度须另取牌张消耗量或切牌位，本包现无该列（UNKNOWN）。
     ⚠ G-4 补注（2026-08-23）· 逻辑边界，两句不可互换：
       可写：「R01 之 shoe grain／跨日粒度污染疑虑已排除（CLOSED／OBSERVED），
             故 T-03 之指标无须因 shoe grain 有误而重算。」
       不可写：「因粒度已澄清，故 T-03 再次被证伪。」
       T-03 之 REFUTED／FATAL 恒依其自身四项独立验证而立，
       **不得以上游诊断之结论替代下游验证之证据**——此为串线，判 S1。
   预期行数：§R01-0 一行 · §R01-chk 桌数行 · §R01-inv 一行 · §R01-anm 数十至数百行
   ─────────────────────────────────────────────────────────────────────────── */


-- §R01-0 · 靴号粒度验证（★ 本族前置闸：未过则 §R01-chk 起全部作废）
-- ▸ 导出：不需要 —— §R01-0 靴号粒度验证（判 bet03 是否跨日复用），屏幕看结果。
--   ★★ 判读（G-4 · 2026-08-23 据实测反转，原判读指引方向错误已撤回）★★
--     原文曾指引「pct_cross_day > 0 即证 bet03 系日内循环序号，§R01-chk 起一律以
--     新粒度为准」。该指引错误：pct_cross_day > 0 只证「同一(靴,桌)横跨两个营业日」，
--     未能分辨其成因究系「跨零点同一靴」抑或「靴号隔日复用」——二者皆产生
--     pct_cross_day > 0。判别式在**次日之起始局号**，须另跑 §R01-0c。
--     本轮实测：pct_cross_day = 2.506546%、max_days_per_shoe = 2；
--     §R01-0c 判明 day_span 全为 1、pct_contiguous = 100%、restart_at_1 = 0，
--     即**跨零点同一物理靴**。故 (shoe_id, table_id) 为正确粒度，新粒度反系切断。
--     ⚠ 本条查询本体一字未动；改者仅此判读指引。
WITH test_agents AS (                                                                               -- 公共表表达式：开启中间结果集 test_agents，其后各行为其定义体（§R01-0）
  SELECT age001 AS agent_id                                                                         -- 取列：产出「agent_id」
  FROM ods_mariadb_2b.ods_a168_agent                                                                -- 取数来源：取自代理主档（五级代理线归属来源）
  WHERE age022 = '1'                                  -- 铁律④b：公司测试线
),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-0）
ranked AS (                                            -- 需求 §3.2：同单号保留最新版本
...
base AS (                                                                                           -- 公共表表达式：开启中间结果集 base，其后各行为其定义体（§R01-0）
...
grain AS (                                                                                          -- 公共表表达式：开启中间结果集 grain，其后各行为其定义体（§R01-0）
  SELECT shoe_id, table_id,                                                                         -- 取列：起始取列子句，本行先列 shoe_id, table_id，涉 bet03（靴号）、bet39（桌号）
         COUNT(DISTINCT dt)  AS n_days,                                                             -- 计数表达式：营业日的去重个数——大于 1 即证靴号跨日复用，产出「n_days」
         MIN(dt)             AS dt_min,                                                             -- 聚合表达式：该靴号最早出现之营业日，产出「dt_min」
         MAX(dt)             AS dt_max,                                                             -- 聚合表达式：该靴号最晚出现之营业日，产出「dt_max」
         MAX(round_no)       AS max_round_old_grain                                                 -- 聚合表达式：旧粒度之最大局号（≠COUNT 局数），产出「max_round_old_grain」
  FROM base                                                                                         -- 取数来源：取自本条自建的中间结果集 base
  GROUP BY shoe_id, table_id                                                                        -- 分组：按 shoe_id, table_id 汇总——刻意用旧粒度，以量其被夸大之程度
)                                                                                                   -- 续行：收束上方的子查询或函数括号（§R01-0）
SELECT                                                                                              -- 取列：起始取列子句
  COUNT(*)                                                       AS n_shoe_table_old,               -- 计数表达式：旧粒度之（靴，桌）组数，产出「n_shoe_table_old」
  SUM(CASE WHEN n_days > 1 THEN 1 ELSE 0 END)                    AS n_cross_day,                    -- 条件分支：跨日复用之组数，产出「n_cross_day」
  ROUND(SUM(CASE WHEN n_days > 1 THEN 1 ELSE 0 END) * 100.0                                         -- 条件分支：跨日复用之百分比——本条判读之主刻度
        / NULLIF(COUNT(*), 0), 6)                                AS pct_cross_day,                  -- 除法计算：分母以 NULLIF 守门防零除，产出「pct_cross_day」
  MAX(n_days)                                                    AS max_days_per_shoe,              -- 聚合表达式：单一靴号最多横跨几个营业日，产出「max_days_per_shoe」
  ROUND(AVG(n_days), 6)                                          AS avg_days_per_shoe,              -- 聚合表达式：平均横跨营业日数，产出「avg_days_per_shoe」
  ROUND(AVG(CASE WHEN n_days > 1 THEN max_round_old_grain END), 4) AS avg_maxround_crossday,        -- 条件分支：跨日组之旧粒度最大局号均值——夸大之直接证据，产出「avg_maxround_crossday」
  ROUND(AVG(CASE WHEN n_days = 1 THEN max_round_old_grain END), 4) AS avg_maxround_singleday,       -- 条件分支：单日组之旧粒度最大局号均值——对照侧，产出「avg_maxround_singleday」
  MAX(CASE WHEN n_days > 1 THEN max_round_old_grain END)           AS peak_maxround_crossday,       -- 条件分支：跨日组之最大局号峰值，产出「peak_maxround_crossday」
  SUM(CASE WHEN shoe_id  = '(空)' THEN 1 ELSE 0 END)             AS n_shoe_sentinel,                -- 条件分支：靴号哨兵组数——零行亦须明示，缺档不是无内容，产出「n_shoe_sentinel」
  SUM(CASE WHEN table_id = '(空)' THEN 1 ELSE 0 END)             AS n_table_sentinel                -- 条件分支：桌号哨兵组数，产出「n_table_sentinel」
FROM grain;                                                                                         -- 取数来源：取自本条自建的中间结果集 grain


-- §R01-chk · 靴长分母核验·逐桌（新旧两粒度并报 ＋ Fisher-z 相关区间）
-- ▸ 导出：需要 —— 存为「数据库/R01chk_shoe_len_by_table.csv」（§R01-chk 靴长分母核验·逐桌）。
--   ★ 本条无 LIMIT：Superset 显示行数有上限（使用纪律⑤），导出前须核对结果行数
--     与全库桌数一致，否则「未出现」类判断一律无效。
--   ★ 判读三处：
--     ① corr_density_verdict —— CI 跨 0 者方向不可读；CI < 0 则与低估假说反向
--        （密桌反而靴短），指向切牌规程异质。
--        ⚠ G-4 补注（2026-08-23）：现行第三态之判词「CI > 0 即分母低估假说得证」
--          过强，只证方向、未证量级。本轮实测 29／30 桌 CI > 0，然桌内 r 中位
--          仅 0.101（范围 0.061~0.186），R² 约 1.02%——密度只解释靴长变异之约 1%；
--          而跨桌 r ＝ −0.600（R² 约 36%）方向相反且量级主导。故正确读法为
--          「方向显著但实质可忽略，主导因素系跨桌切牌规程异质」。
--          判词四态升五态（新增「CI > 0 且 |r| < 0.2」档）属 v12 staging 之 G-2，
--          本件不施工；在其落地前，本列之第三态一律照本补注理解。
--     ② collision_flag —— ceiling_observed 逾 p99 之 1.5 倍者，靴号碰撞或跨日合并可疑。
--     ③ round_at_pos080 —— 本桌 shoe_pos>=0.80 对应之起始局号；逐桌不同即证
--        同一相对靴位在不同桌对应之牌深不同，跨桌可比性受损。
...
ORDER BY z.avg_max_round DESC, z.table_id;                                                          -- 排序：按 avg_max_round（降序）, table_id 排列；导出必带排序，否则分页无稳定序（曾致 36.49% 重复行）


-- §R01-inv · 靴长分母核验·全域普查（判物理硬顶与跨桌规程异质）
-- ▸ 导出：需要 —— 存为「数据库/R01inv_shoe_len_census.csv」（§R01-inv 靴长分母核验·全域普查）。
--   ★ 判读二处：
--     ① n_len_ge_70 = 0 且 p99 封于某值 —— 切牌收靴之硬指纹，物理上限存在，
--        则约 50 局系真靴长而非低估；反之若见 70 局以上者，短靴系截断而非规程。
--     ② corr_cross_table（单位＝桌，非靴，与 §R01-chk 之桌内相关不可混为一谈）
--        显著为负 —— 密桌反而靴短，与低估假说反向，规程异质坐实。
...
-- §R01-anm · 靴长分母核验·异常靴清单（碰撞与残靴之逐行证据）
-- ▸ 导出：需要 —— 存为「数据库/R01anm_shoe_anomaly.csv」（§R01-anm 靴长分母核验·异常靴清单）。
--   ★ 本清单系**账务对象，非会员对象**：所列者为（日，桌，靴）三元组，不含会员号，
--     禁作任何风控判据之输入；查账在先、查人在后。
--   ★ 上界阈值由数据反解（全域 p99 × 1.5），不硬写——本包不设普适门槛。
...
ORDER BY s.max_round DESC, s.dt, s.table_id, s.shoe_id;                                             -- 排序：按 max_round（降序）, dt, table_id, shoe_id 排列；导出必带排序，否则分页无稳定序（曾致 36.49% 重复行）




/* ═══════════════════════════════════════════════════════════════════════════
   §ECON · 经济结果层口径登记（规章类 · 无查询体 · 无交付件）
   ───────────────────────────────────────────────────────────────────────────
   立节缘由（2026-08-23 · 用户令 ＋ REDTEAM 裁定）：
     业务侧提问「15 类风险会员之判据既为 roi > 0，其余 14 类是否亦应补 roi／avg」。
     该提问之前提有误，须先正名（见「一、前提斧正」）；但其结论方向正确——
     15 类之案件输出确应统一携带经济指标，惟其身分为「结果」非「判据」。
     本节即该层之口径登记：先定义口径，再动代码；未登记者不得入表。
   本节性质：规章（治理规则），非数据、非查询。全节无 SQL、无导出声明行。
   本节不改任何既有查询；ROI 与 avg 之实际入表，须待「六、施工顺序」走完，
     并另立变更单，届时方产生代码改动与版本进位。
   ═══════════════════════════════════════════════════════════════════════════

   ── 一、前提斧正（三条命名纪律，永久生效）───────────────────────────────

   斧正之一：判据非 roi。
     「15 类风险会员之判据标准是会员之 roi > 0」——此语不成立，一条亦不成立。
     以 T-03（尾投／靴尾下注）为例，其判据列在册者为 late_share、shoe_pos_p50、
     stake_ramp、tail_share 四项，判级皆 PROFILE_ONLY，且该类
     label_validation_status = REFUTED、causal_status = REFUTED、
     admit_to_risk_decision = false，已判 FATAL。roi 从来不是其判据。
     ROI 之为物，是「经济后果」，不是「风险原因」。此二者相混，即本包所载
     最危险之串线：经济结果 → criterion → risk trigger。
     ⚠ 更须记明：本项目已现算证实，以 roi > 0 为判据者，其命中主体系方差——
       零假设下会员 ROI 之期望非 0 而为 −hold%，故「跑赢 0」不等于「跑赢赌场」。
       据 roi > 0 出名单、行处置者，判 S1 FATAL。

   斧正之二：T 与 R 不得混称。
     · T-01 ~ T-15 ＝ 风险类型（typology），登记册 registry_risk_typology 之主键；
     · R01／R02／R03b… ＝ 本包之分析段与交付件编号；
     · V ＝ 价值轴（stake_all 分位），既非风险亦非经济结果。
     三者各有其轴，禁互代。T-03 之主交付件恰为 R01_late_shoe.csv，
     然「T-03 ≠ R01」——前者为类型，后者为该类型之一份交付件。

   斧正之三：禁裸 avg。
     avg 单独存在即语义漂移之源：其可为 stake_avg、pnl_avg、round_avg、
     shoe_len_avg 之任一。本层一律用语义化字段名，禁 avg 裸列。
     本包现有 R01_late_shoe.csv 之 stake_late_avg 与 stake_early_avg 已合此律，
     其名自明分段与所平均之物，可作范式。

   ── 二、行业类比之斧正（本节最须留意之一处）─────────────────────────────

     业界所载之风控框架（liability／exposure、pricing、trading、CLV、line
     movement、arbitrage、closing odds 等）多出自 sportsbook（体育博彩）。
     本平台 a168 系真人厅百家乐，属赌场产品，非 sportsbook。二者之风险机理
     并不同源，照搬即错配：

       sportsbook 之机理          | a168 百家乐之对应物
       ---------------------------|------------------------------------------
       赔率由交易台定价，可被击败   | 赔率由规则固定，无定价函数、无交易台
       CLV／收盘价差              | 不存在（无收盘价）
       line movement 择时         | 不存在（无盘口移动）
       跨庄套利 arbitrage         | 对应「对押／打水套退水」，机理不同
       liability 敞口管理         | 对应「限红档位 × 台桌承载」，非动态定价
       sharp bettor               | 对应「优势玩家」：算牌、切牌观察、边缘识别、
                                  |   串谋、结算完整性利用

     故本层之指标设计不取 sportsbook 之 avg_odds／CLV 一族，改取赌场机理者：
       · 玩法结构（bet09 之取值分布，含边注占比）
       · 免佣与否（commission：0 一般／1 免佣，影响庄家理论优势）
       · 三态结果（胜／负／和局 push），禁以二态胜率冒充——和局非负
       · 台桌与靴之覆盖（n_active_tables／n_active_shoes）
     ⚠ 判级：本表所列之对应关系为 INFERRED（机理类比），非 OBSERVED；
       其中「玩法结构」与「免佣」二项之可得性已由 §DX-04／§DX-05 实测在册，
       余项之可得性须逐项探针后方可登记为必备。

   ── 三、三分律（本层之根本，禁合并）─────────────────────────────────────

     风险判据（Risk Criterion） ≠ 经济结果（Economic Outcome） ≠ 价值优先（Value）

       风险判据  答「此会员为何进入 T-XX」——各类各自独立，本层一字不动。
       经济结果  答「此会员对平台造成何种经济后果」——本层所登记者。
       价值优先  答「谁值得优先占用复核产能」——V 轴，另有其轴。

     三者互不蕴含：V5 头部会员不等于高风险，高 ROI 不等于应处置，
     行为异常亦不等于已造成经济损失。合并任二者，即制造冤案或漏案。

      ★★ 反向锁（Z2-02 · 2026-08-23 新立 · 四向皆禁）★★
        原三分律只禁「判据 → 经济」之正向串线，未禁其反向。今补：
          禁 ①  经济结果 → 风险判据        （高 ROI 故判风险）
          禁 ②  价值优先 → 风险判据        （大客故判风险，或大客故免判）
          禁 ③  风险判据 → 价值优先        （命中风险故降其价值位序）
          禁 ④  经济结果 → 价值优先之**唯一**依据（V 轴须自陈定义，非由 ROI 派生）
        四向皆禁，方为「三分」；只禁一向者，仍可自另一向串回。
        ⚠ 实例（本包已现算之教训）：V 轴 stake_all 之高 AUC，部分反映
          「谁被复核得多」而非「谁更危险」——此即 ③ 之反向污染。
          故 V 轴**不得作因果证据**，其限界须原样保留。

   ── 四、经济指标字典（口径登记 · 未登记者不得入表）───────────────────────

     ⚠ 铁律：每一指标须登记七要素——名、粒度、窗口、分子、分母、零分母处置、
       角色。缺一即不得入表。以下为 A 档（通用）之登记式：

       metric            grain   window            numerator    denominator   zero_denom  role
       ----------------- ------- ----------------- ------------ ------------- ----------- ------------
       n_orders          member  analysis_window   COUNT(*)     —             —           OUTCOME
       active_days       member  analysis_window   nunique(dt)  —             —           OUTCOME
       active_shoes      member  analysis_window   nunique(靴)  —             —           OUTCOME
       stake_sum         member  analysis_window   SUM(stake)   —             —           OUTCOME
       stake_avg         member  analysis_window   SUM(stake)   n_orders      NULL        OUTCOME
       pnl_sum           member  analysis_window   SUM(pnl)     —             —           OUTCOME
       pnl_avg           member  analysis_window   SUM(pnl)     n_orders      NULL        OUTCOME
       roi               member  analysis_window   pnl_sum      stake_sum     NULL        OUTCOME
       ggr               member  analysis_window   −SUM(pnl)    —             —           OUTCOME
       hold_pct          member  analysis_window   −SUM(pnl)    stake_sum     NULL        OUTCOME

     ★ ROI 之两式不可混（本层最易失守之一处）：
         式甲  SUM(pnl) ÷ SUM(stake)      —— 暴露加权 ROI，钱说了算，答「平台赚多少」
         式乙  AVG(member_roi)            —— 逐员等权均值，人说了算，答「会员长什么样」
       二式数值可差甚远，且答的不是同一问题。凡出 ROI 者须标明取何式；
       并报为佳，互代为错。本包既有之域级读数一律取式甲。

     ★ 尺度锁：ROI 为净收益率，持平 ＝ 0（非回报倍数 1+ROI）；分母恒为本金口径
       （stake ＝ bet13 ÷ bet11），非洗码口径。二口径不可互比。
     ★ 零假设锁：会员 ROI 之零假设非 0，而为 μ0 ＝ −hold%（由域级总帐反号现算）。
       ⚠ Z2-01（2026-08-23 · 分层化）：上句之 μ0 系**全局近似值**，正式形为
         **μ0(g, w) ＝ −hold%(g, w)** —— g 为玩法／注型组（bet09 取值：庄／闲／和／
         对子／各式边注），w 为时间窗。百家乐各注型之庄家优势本不相同
         （庄 ≈ 1.06%、闲 ≈ 1.24%、和 ≈ 14.4%，边注更高），且免佣与否
         （commission 0／1）另改庄注优势。以全局 μ0 判专押和局或边注之会员，
         其零假设即系统性偏误。
       ⚠ 判级：分层 μ0(g,w) 各组值须由域级总帐**按组反号现算**后方得登记，
         现为 **UNKNOWN**（尚未逐组现算）。未现算前，凡用 μ0 者须标明所用系
         「全局近似 μ0」抑或「分层 μ0(g,w)」，**禁不标而用**。
     ★ 绝对值锁：|ROI| > 1 者谓净盈亏逾其分母，先入结算完整性九项排查序——
       本金／红利／彩池／退水／冲正／重复／汇率／部分结算／调整，查账在先、查人在后。

     B 档（可选 · 稳定性与规模，视可得性逐项探针后方可升必备）：
       stake_p50、stake_p90、stake_max、stake_cv、n_active_tables、
       win_rate_3state（胜／负／和三态，禁二态）、side_entropy、commission_share

     C 档（类别专属，随 T 类而异，见第五节裁定表）

   ── 五、T-01 ~ T-15 逐类裁定（判据不动，只加结果层）─────────────────────

     裁定读法：「ROI 权重」谓经济结果对解释该类商业意义之轻重，
     非谓其为判据；任何一类皆不以 roi > 0 为触发条件。

       类     风险类型            判据（不动，登记册为准）        结果层  ROI权重  类别专属指标
       ------ ------------------- ----------------------------- ------- -------- ------------------------
       T-01   同桌聚集             lift／jaccard／same_rate       应补    辅助     n_same_rounds、n_tables
       T-02   荷官—玩家串谋        delta_z／delta_p               应补    辅助     dealer_hold_gap、n_dealers
       T-03   尾投／靴尾下注        late_share 等（PROFILE_ONLY）  应补    辅助     分段 roi_late／roi_early
       T-04   跨账户对打／对冲      opposite_rate                  应补    高       hedge_stake、pair_pnl_net
       T-05   自我对冲／打水        hedge_rate／stake_cv           应补    高       rebate_dep、hedge_stake
       T-06   异常 IP 聚集         n_member_eff                   应补    辅助     n_ip、subnet_span
       T-07   技术型玩家           稳定性／结构熵                  应补    高       roi_shrunk、persistence
       T-08   退水／占成套利        rebate／commission             应补    高       rebate_rate、rebate_expense
       T-09   代理线自打／养号      member quality                 应补    高       agent_line_pnl、n_downline
       T-10   账务恒等式残差        resid_identity                 应补    辅助     resid_abs、resid_rate
       T-11   多账户／共享设备      IP／jaccard／注册邻近           应补    辅助     device_span、reg_proximity
       T-12   机器人／脚本下注      stake_cv／delay variance       应补    辅助     interval_cv、burst_rate
       T-13   夜间异常活动          night_share                    应补    辅助     night_stake_share
       T-14   限红试探             limit utilization              应补    高       stake_to_limit、limit_hits
       T-15   静默复活／账户接管    silent_days／lifecycle          应补    视案     revive_pnl、pre_post_delta

     ★ T-03 之特别处置（本表唯一须另加措辞锁者）：
       该类已判 FATAL_PROFILE_ONLY。其经济结果层可出、且值得出，但表述须写作
       「该会员具备 T-03 画像标签；经济结果层观察到 ROI 为 X」，
       不得写作「T-03 风险会员 ROI 为 X」。前者陈述并列事实，后者暗示因果。

   ── 六、economic_role 五值（防下游 SQL 误当 criterion）─────────────────

       NONE            不适用
       PROFILE         仅画像，不入任何决策
       OUTCOME         描述经济结果（本层之默认值）
       PRIORITIZATION  可辅助案件排序，不改变是否命中
       VALIDATION      用于验证某风险假说，须另附假说编号

     凡入表之经济指标须携此角色列。缺此列者，下游极易将 roi 一列自动当成
     criterion 使用——此即本节所防之根本失效模式。

   ── 七、Schema Debt 登记（优先级高于加 ROI）──────────────────────────────

     登记册 YAML 与 CSV 之准入语言现存双重真相之嫌：
     ★ Z2-03（2026-08-23 · 措辞升级 · 自「之嫌」改判 OBSERVED）：
       上句所述**非「嫌疑」而系实况**——二侧词汇集合互异，且皆可被机器读取，
       故其为 **OBSERVED 之双重真相**，非待证之疑。措辞不得再写作「之嫌」。
     ★ SSOT 强制条（Z2-03b · 新立）：
       登记册之**唯一真相源为 YAML**；CSV 系其投影，只读、不得独立成据。
       凡二侧冲突者一律以 YAML 为准，且该冲突须登记为 SC，不得静默择一。
       ⚠ 在 SSOT 未以代码强制（loader 侧断言）之前，任何据 CSV 之准入列
         所作之判定，一律判 **S2 BLOCK**。
       YAML 侧：admit_to_feature_scoring／admit_to_risk_decision
       CSV  侧：admit_to_scoring／admit_to_profile／admit_to_control
                ranking／scoring／trigger／enforcement
     v1.4.0 已锁「废五层互斥制，改主生命周期状态 × 阻断门禁双轴状态机」，
     而 CSV 侧旧词仍在，且可被机器读取。设若 YAML 之
     admit_to_risk_decision = false 而 CSV 之 trigger = true，听谁？
     ⚠ 本项列为下一次 Schema RedTeam 之第一检查项，优先级高于本节之加 ROI。
     ⚠ 本包不代改登记册；此处只作登记与指向。

   ── 八、施工顺序（锁死 · 禁跳步）────────────────────────────────────────

       STEP 0  命名纪律：T-01~T-15 ＝ 风险类型；R01… ＝ 交付件；V ＝ 价值轴
                 状态：本节第一节已立 ✅
       STEP 1  §R01-0（跨日）＋ §R01-0b（跨桌）二条只读诊断
                 状态：⛔ 未执行 —— 本层一切以 R01 为模板之动作皆待此
       STEP 2  锁定 R01 之 shoe_len／shoe_pos 粒度
       STEP 3  自 R01_late_shoe.csv 逐字段登记 roi／stake／pnl／n_bets，禁裸 avg
       STEP 4  建立经济指标字典（本节第四节为其骨架，尚待实测填实）
       STEP 5  T-01~T-15 逐类映射：criterion 与 economic outcome 分列
       STEP 6  定 UNIVERSAL／CATEGORY-SPECIFIC／PROFILE-ONLY／VALIDATION-ONLY
       STEP 7  再决定是否改登记册 schema
       STEP 8  若正式入册，方产生版本变更与代码改动

     ⚠ 当前状态锁：
       · 本节为规章登记，代码零改动，故不触发版本进位；
       · 在 STEP 1 与 STEP 2 未完成之前，禁以 R01 之 roi／avg 口径为其余
         14 类之模板——R01 之 shoe_len 粒度尚在 OPEN，若以其为范本，
         等于把一个未决之粒度问题扩散至全体系；
       · 本节所列 A／B／C 三档指标，凡未经可得性探针者一律判 UNKNOWN，
         不得径写入任何交付件之表头契约。
   ═══════════════════════════════════════════════════════════════════════════ */
...
WITH ranked AS (                                                                                    -- 公共表表达式：开启中间结果集 ranked——口径与 §R03 一致（§RC-00）
  SELECT b.bet01, b.bet03, b.bet04, b.bet39, b.bet05, b.bet09, b.bet11,                             -- 取列：起始取列子句，涉 bet03（靴号）、bet04（局内序号）、bet39（桌号）、bet05（会员号）、bet09（玩法）
         b.bet13, b.bet38, b.eid, b.ip, b.bet18, b.bet19, b.bet20,                                  -- 续行：接续上一取列子句，涉 bet38（重對）、eid（荷官工号）、ip（下注 IP）、bet18~20（代理线）
         ROW_NUMBER() OVER (PARTITION BY b.bet01                                                    -- 行号窗口表达式：按注单号开窗取最终态
           ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC) AS rn                           -- 排序：取最新一版，产出「rn」
  FROM ods_mariadb_2b.ods_a168_bet02 b                                                              -- 取数来源：取自注单明细表
  WHERE b.dt >= '2026-03-21' AND b.dt < '2026-08-07'                                                -- 过滤条件：限定 E1 全局窗，涉 dt（营业日）
...
FROM ods_mariadb_2b.ods_a168_bet02                                                                  -- 取数来源：取自注单明细表（金额与行为口径的第一料源）
WHERE dt >= '2026-03-21' AND dt < '2026-08-07'                                                      -- 过滤条件：限定 dt不少于 '2026-03-21'、dt小于 '2026-08-07'，涉 dt（营业日）
  AND bet02 = '101' AND category = '1' AND UPPER(TRIM(bet38)) = 'N'                                 -- 并列条件：限定 bet02等于 '101'、category等于 '1'、UPPER(TRIM(bet38))等于 'N'，涉 bet02（游戏类别）、bet38（重對）
...
rk AS (                                                                                             -- 公共表表达式：开启中间结果集 rk，其后各行为其定义体（§EX-07b）
  SELECT b.bet01, b.updatetime, b.sync_time, b.dt, b.category, b.bet38,                             -- 取列：起始取列子句，本行先列 b.bet01, b.updatetime, b.sync_time, b.dt, b.category, b.bet38，涉 bet38（重對）、dt（营业日）
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   §Z-16 · 变更记录（v11.0.2 → v11.0.3，2026-08-23 · PATCH）
     性质：**纯注释区，SQL 主体一字未动**；纯代码 MD5 恒为
       ce768c35b8d7aeb5ac28079166a8c422、纯代码行数恒为 5,961。
     缘由：v11.0.2 经 G1–G10 全闸机械验收 ACCEPTED 后，尚余二笔已登记之债，今清。
     ① SC · 块注释闭合斧正（**本轮最要紧之一项**）：
        实测全档块注释起始符 224 处而闭合符 223 处。逐字符状态机扫描定位：
        「✅ 字段已实测确认」起之块注释**无自身闭合符**，系被其后 S-04b 横幅之
        闭合符顺手关掉。当时无碍纯属侥幸——设若日后删改该横幅，该起始符将一路
        吞至下一个闭合符，把其后整条 SELECT 静默注释掉：**查询不报错，只是凭空消失**。
        ★ 此项证明「配对总数相等」不足以证结构无误——须逐字符状态机扫描，
          因内嵌之起始符在不嵌套之引擎中只是文本，计数相抵而结构已错位。
     ② 连带新立注释体例一条（实证所得）：
        **块注释内一律不得书写字面之块注释起始／闭合符**。
        缘由：v11.0.3 施工首版于新注释内写了字面标记，当场提前闭合该块，
        致纯代码行由 5,961 暴增至 6,347、逐 token 差异 6,782 —— 即刻被 PATCH
        硬判据拦下。**修者自蹈所修之错，恰为该条体例之最强证据。**
     ③ 卷首「版本身份六元组」之第三 MD5 补注完整萃取规则：
        此前只书「剥离行尾注释后」，**漏载剥块注释一步**。第三方照字面复现必失败
        （本轮即有实例：以行注释剥法重算得 8,999 行，与在册 5,961 行差 3,038）。
        ★ 立锁：**规则未写全之锚，不是锚**——因无第二人能复核之。
     ④ 常数集中登记现算复核（承卷首纪律「改动后须重新点数」）：
        WIN_START 187／WIN_END 261／GAME_BACCARAT 191／CAT_NORMAL 213／
        REBET_NO 119／TESTLINE 45 —— 六项与 v11.0.1 在册值**全同**。
        缘由：v11.0.2 之 Z2-04 虽书窗口日期，然未加引号，故未移动字面量处数。
        ★ 本项系**实测复核**，非沿用旧数。
     ⑤ 改前六元组（v11.0.2 · 2026-08-23）：
          行数(CRLF) 10,496 · 字节 1,317,001
          MD5(原样) 7b287ff9065bba637435c335da9d651e
          MD5(LF 归一) 49ff68f5a3610d299158d9dc95016a84 · CRLF · UTF-8(无 BOM)
          MD5(纯代码) ce768c35b8d7aeb5ac28079166a8c422 · 5,961 行
        v11.0.2 加 _superseded 归档，禁删。

   §Z-16 · 变更记录（v11.0.1 → v11.0.2，2026-08-23 · PATCH / REDTEAM semantic-hardening）
     性质：**纯注释区语义强化，SQL 主体一字未动**。
       纯代码 MD5 恒为 ce768c35b8d7aeb5ac28079166a8c422、纯代码行数恒为 5,961，
       此二者即本次 PATCH 之硬判据；任一变动即非 PATCH。
     缘由：REDTEAM 复审 v11.0.1，判「准许进入 v11.0.2，不准进入 v12，
       不准借 PATCH 名义改变既有 SQL 血统」，并列 Z2-01 ~ Z2-08 八项必改。
     ① Z2-01 · ROI 零假设分层化（S1）：μ0 由全局常数改书为 μ0(g,w) ＝ −hold%(g,w)；
        g 为玩法／注型组、w 为时间窗。缘由：百家乐各注型庄家优势本不相同
        （庄 ≈ 1.06%、闲 ≈ 1.24%、和 ≈ 14.4%，边注更高），免佣另改庄注优势；
        以全局 μ0 判专押和局或边注之会员，其零假设即系统性偏误。
        分层各组值现判 UNKNOWN，未现算前用 μ0 须标明系全局近似抑或分层。
     ② Z2-02 · 三轴反向锁（S1）：原三分律只禁正向串线，今补四向皆禁——
        经济→判据、价值→判据、判据→价值、经济→价值之唯一依据。
     ③ Z2-03 · YAML／CSV 双重真相由「之嫌」改判 **OBSERVED**（S2），
        并立 SSOT 强制条 Z2-03b：YAML 为唯一真相源，CSV 只读投影；
        未以 loader 代码强制前，据 CSV 准入列所作判定一律判 S2 BLOCK。
     ④ Z2-04 · 证据等级两分（S2）：窗内实测为 OBSERVED，外推窗外为 INFERRED；
        禁写「shoe_id 全局唯一」「跨桌永不重号」。
     ⑤ Z2-05 · GROUP BY dt, bet03 由「一律禁止」改为**语境锁**（S1）：
        还原物理靴时禁用，统计营业日内靴段时正确；凡用者须明书统计单位，未书判 S2。
     ⑥ Z2-06 · shoe_pos 身份锁（S1）：其分母系该靴实际最大局号，
        故所量者为 observed position，**非 deck-depth**；shoe_pos ＝ 0.80
        不等于「已发出八成牌」。以其论算牌深度者属坐标系错配，判 S1。
     ⑦ Z2-07 · late_n 通用公式入册（S2 · 两处）：
        late_n(N) ＝ N − ⌈0.80N⌉ + 1、late_share0(N) ＝ late_n(N) ÷ N；
        其值随 N 呈锯齿（N=45 → 22.22%、50 → 22.00%、52 → 21.15%、56 → 21.43%）。
        本包靴长四族 p50 ＝ 56／52／49／48，单一常数作全域基准者判 S2。
     ⑧ Z2-08 · REFUTED 禁复活之机械禁令（S1）：由「劝诫」升为「门禁」，
        五条禁径（上游诊断改变／阈值重选／子样本分层换窗／新增协变量／ROI-GGR 为正）
        皆不得恢复 admit_to_risk_decision；唯一途径为重跑原四项独立验证且四项皆翻转。
        本禁令之对象是 REFUTED 之**状态**，非某一判据之名。
     ⑨ 本档未含 G-1／G-2／G-3／G-6，亦未含 SQL 主体任何改动。
     ⑩ 改前六元组（v11.0.1 · 2026-08-23）：
          行数(CRLF) 10,373 · 字节 1,306,165
          MD5(原样) 0ca299179ba3ad478443c5b58b08b8d6
          MD5(LF 归一) 9400ae862e76c92c1d3088d8696243d8 · CRLF · UTF-8(无 BOM)
          MD5(纯代码) ce768c35b8d7aeb5ac28079166a8c422 · 5,961 行
        v11.0.1 加 _superseded 归档，禁删。

   §Z-16 · 变更记录（v11.0.0 → v11.0.1，2026-08-23 · PATCH）
     ★ 版级：PATCH。硬判据——纯代码 MD5 与 v11.0.0 逐字节相同
       （ce768c35b8d7aeb5ac28079166a8c422 · 5,961 行），代码零改动。
     ★ 版本纪律变更（用户令，本档为首个落地件）：
       自即日起，本项目一切档案改用**三段式版本号 MAJOR.MINOR.PATCH**，
       每段进位至 999 方晋一级；**禁由 vN 直跃 vN+1**，须循 PATCH → MINOR
       → MAJOR 逐级。全文见「规范/版本编号纪律_v1.0.0.md」。
       等价声明：原 v10 ≡ v10.0.0、原 v11 ≡ v11.0.0。此系声明，非追溯改名——
       版本号是身份不是序号，追溯改名即身份冲突（既有 §Z-16 条目、六元组登记、
       下游三档 must= 清单皆以旧名为锚，改之则全族失锚）。
     ① G-4 · §R01-chk 族粒度指引反转（四处）：族头注「粒度辨义」→「粒度裁定」；
        §R01-0 判读指引；§R01-chk 判词补注；裁定纪律加 T-03 逻辑边界。
        缘由：§R01-0b／0c／0d 全域实测（2026-08-23）推翻原指引之方向——
          g1(shoe_id) 123,357 靴 ≡ g2(shoe_id, table_id) 123,357 靴，delta = 0，
          avg max_round 同为 49.7806；g3(dt, shoe_id, table_id) 126,449 靴
          ＝ g2 ＋ 3,092，而 3,092 恰等于 n_cross_day。
          跨日 3,092 组：day_span 恒为 1、pct_contiguous = 100%、
          restart_at_1 = 0、overlap = 0、avg_join_gap = 1.022
          （前日止于第 25.50 局、次日自第 26.52 局无缝续接）。
        故：跨日成因系**同一物理靴跨零点被营业日切开**，非靴号复用；
          g2 为正式分析粒度，g3 系人为切断，仅作诊断对照。
        ★ T-03 逻辑边界（新立，禁互换）：可写「粒度污染疑虑已排除，故 T-03 指标
          无须因 shoe grain 有误而重算」；不可写「因粒度已澄清，故 T-03 再次被
          证伪」。T-03 之 REFUTED／FATAL 恒依其自身四项独立验证，不得以上游
          诊断替代下游验证——此为串线，判 S1。
     ② G-5 · shoe_len 与 §P12a／§P13 错误警示纠正（三处）：卷首 shoe_len 横幅、
        §P12a 就地横幅、§P13 就地横幅。
        缘由：原文断言「不含桌号者会把异桌同号靴合并、max_round 被系统性夸大、
          shoe_pos 被系统性压低」，已被 §R01-0b／0d 直接推翻——
          pct_cross_table = 0、max_tables_per_shoe = 1、§R01-0d 逐行证据零行。
        ★ 证据边界（措辞已收紧，不得外推）：以上系**本核验窗口、本数据口径下之
          OBSERVED 事实**，非「shoe_id 永久全局唯一」之保证；换窗口、换产品、
          换库须重新核验。
        ★ 并加反向禁令：切勿改用 GROUP BY dt, bet03——该写法会切断跨零点续接
         ★ Z2-05（2026-08-23 · 由「一律禁止」改为**语境锁**）：
           上句措辞过宽——该写法之对错**依分析目的而定**：
             · 欲还原**物理靴**（一副牌之完整生命周期）→ 用 (shoe_id, table_id)；
               改用 (dt, bet03) 即切断跨零点续接之同一物理靴 → **此时禁用**。
             · 欲统计**营业日内之靴段**（日结、班次、日报对账）→ (dt, bet03)
               恰为所需粒度 → **此时正确**。
           ★ 故禁令不在写法，而在**目的与粒度之匹配**：凡用 (dt, bet03) 者，
             须于该处明书「本条统计单位为**营业日内靴段**，非物理靴」；
             未书者判 S2 —— 读者无从分辨其所数为何物。
          之同一物理靴（§R01-0c 实测 3,092 条、pct_contiguous = 100%）。
     ③ 本档未含 G-1／G-2／G-3／G-6，其版级与次序见件头第三条。
     ④ 改前六元组（v11.0.0 · 2026-08-23）：
          行数(CRLF) 10,273 · 字节 1,296,319
          MD5(原样) 9f2285d10983903a9f311deb5550cbb7
          MD5(LF 归一) 73339d69da8b75b75678e5d6bacbb948 · CRLF · UTF-8(无 BOM)
        v11.0.0 加 _superseded 归档，禁删。

   §Z-16 · 变更记录（v10 → v11，2026-08-23）
     ① 新增 **§R01-chk 族 · 靴长分母核验**（四条，位于 §R01 之后、§R02 之前）：
          §R01-0   靴号粒度验证（只看屏幕；本族前置闸）
          §R01-chk 逐桌核验     → 数据库/R01chk_shoe_len_by_table.csv
          §R01-inv 全域普查     → 数据库/R01inv_shoe_len_census.csv
          §R01-anm 异常靴清单   → 数据库/R01anm_shoe_anomaly.csv（账务对象，非会员对象）
        立节缘由：§R01 之坐标系分母 MAX(round_no) 历判「观测靴长之下偏代理」，
        该判级自立以来未经实证，属在册 P0 待表项；本族即其关闭工具。
        ★ 四闸齐备（剔测试线／版本去重／一般注单／非重对）＋ 数值守门三项，
          不入 §Z-13 之「⛔ 交付件缺闸」名单。
        ★ CTE 血统：test_agents／ranked／base 取自 §R01 原文，**唯一改动 ＝
          ranked 与 base 增列 b.dt**（投影增列，筛选条件一字未改）；改动只在
          本族之内，§R01 本体零改动，故下游三档之 must= 清单不受累。
        ★ 命名：-0／-chk／-inv 三后缀承 §R02-0／§R03-chk／§R03-inv 之既有体例；
          **-anm（异常清单）系本包新增后缀，首见于此**，特此立册。
        ★ 粒度并报：新粒度 (dt, table_id, shoe_id) 与旧粒度 (table_id, shoe_id)
          同表并列，n_shoes_delta 与 avg_maxround_inflation 两列即粒度污染之量。
        ★ 阈值：碰撞切点取全域 p99 × 1.5（数据反解，非硬写）；Fisher-z 之样本量
          护栏由统计量本身反解（n ≤ 3 者 z 无定义），不设普适 n ≥ 30 之类门槛。
        ★ 哨兵：空串靴号／桌号一律归「(空)」并单列 table_flag，禁并入正常桌——
          承 eid = -1 之既有处置。
        ★ 同名异义斧正（落笔前全档扫描所得）：本族 §R01-inv 之跨桌 CTE 原拟名 tbl，
          与 §EX-14c 之列别名「AS tbl」撞名（一为中间结果集、一为列），改称
          by_table；另本族一律不用 shoe_len 之名（见卷首横幅补注）。
     ② 卷首三处同步：执行总索引第四批补四行；常数集中登记处数现算重登并统一
        「全文出现次数（含注释区）」之口径；shoe_len 命名警示横幅补注本族不沿用
        其名之由。
     ③ §R01 头注加回指「本节分母之核验见 §R01-chk 族」，与本族互为回指。
        ⚠ §P11b（§R01 输出之复制副本）之同文头注**未改**——副本以指引为体例，
          回指只立于本体，免二处各自演进。
     ④ ★★ 裁定纪律（永久生效）：本族任何结果皆**不得复活 late_share**——
        若约 50 局系真靴长，则 shoe_pos>=0.80 量的就是靴末段而其已被四项独立
        检验证伪；若系低估，则该判据连自身所称之物都未曾测过，证伪只会更彻底。
        本族能改的只有 shoe_pos 坐标系之判级，及其是否仍配作 §R01 之横轴。
     ⑤ 改前六元组（v10）：行数(CRLF) 9,475 · 字节 1,195,130 ·
        MD5(原样) 62985e9b2932bd7ab70fba7ee9c01cb8 · CRLF · UTF-8(无 BOM)。
        v10 须加 _superseded 归档，禁删。

     ═══ v11 · 注释全面斧正（2026-08-23 · 用户令「以免误解」）═══════════════
       ★ 本轮**代码零改动**：仅改注释与块注释边界；剥离全部注释后之档，
         与斧正前逐字节相同——此即体例第六条之验收法。
       ★ 版本号不再进位：用户令「暂时斧正 v11 后仍命名为 v11」，故本轮并入
         v10 → v11 条目之下，不另立 v12；MD5 随之更新，见本节末。

       C-01 §DX 家族头与 §DX-01 原共用一个块注释 → 拆为两块；家族头加
            「无查询体、无交付件」之声明。
       C-02 §DX-02 与 §DX-03 原共用一个块注释，其间仅以 ═══ 分隔，致读者
            无从判断其后 SELECT 属谁 → 拆为两块；§DX-02 加「纯存档说明」声明，
            §DX-03 加「其后 SELECT 属本节」声明。
            并将 §DX-03 之导出行由「▸ 导出：**不需要** ——」归一为全包体例
            「-- ▸ 导出：不需要 —— …」（原写法系全档唯一之第三式）。
            另加破例声明：该节注释内出现两个 CSV 名，系**反向禁令**而非导出指引，
            与导出声明纪律配套②之立意不悖，故刻意保留并写明事由。
       C-03 卷首使用纪律⑨ 原作「连接键(星号星号)不含局键或会员键(星号星号)、
            仅靠桌号/日期」——连续星号与其后斜杠之间仅隔 CJK，正是 §DX-02 所载
            之塌缩模式。今去星号、斜杠改「或」。全档扫描确认仅此一处。
       C-04 §EX-19b 之行尾注释内含分号（违纪律⑪「注释内不写分号」）→ 去之。
            全档扫描确认仅此一处。
       C-05 **bet38 正名 75 处**：卷首字段正名速查早已判定「旧记『测试标识』有误，
            bet38 系重對（enum Y/N）」，然行尾注释仍有 75 处沿用被判错之旧义，
            而正名后之「重對」仅 8 处。今全数归一为「bet38（重對）」。
            ★ 此为本轮最大宗之注释误导：测试线系由 age022='1' 五路 LEFT JOIN
              排除，与 bet38 无涉；照旧注释理解者会误以为测试线已由 bet38 剔除。
       C-06 **GGR 句式误植 9 处**：注释生成器凡见减号即套「汇总后取负号——会员
            净输赢取负即平台毛利（GGR）」，而其中九处与 GGR 全无干系——
            香农熵之前置负号 2 处、方差之二阶矩减一阶矩平方 1 处、
            分段 ROI 之差 1 处、观测减期望之超额 1 处、
            §R01 与 §P11b 之 stake_early_avg／stake_ramp 各 2 处（实算均注与斜率）。
            今逐条按真实语义改写并附「与 GGR 无涉」。
            ★ 另五处（平台净得／游戏输赢贡献／house_hold_pct×2／ggr）确为
              −SUM(game_pnl)，语义正确，一字未动。
       C-07 **卷首 shoe_len 横幅二语与实测不符**（本轮最重之发现）：
            旧文称「三处定义」「皆 MAX(round_no) GROUP BY shoe_id, table_id」。
            全档现算：实为 **六处**，且粒度分两类——含桌号者四处（§R01／§B01／
            §B01-D／§P11b），**不含桌号者二处**（§P12a、§P13，只 GROUP BY bet03）。
            不含桌号即异桌同号靴被合并，max_round 系统性夸大、shoe_pos 系统性压低。
            此为口径差异非精度差异，横幅已按现算重写。
       C-08 上述二处原**无命名警示横幅**（旧文称「统一三处」，故遗漏），
            今就地补「命名与粒度双警示」横幅，写明其与另四处粒度不同及其后果。
            ⚠ 只补注释、不动代码：统一粒度须另立变更单并同步下游三档 must= 清单。
       C-09 §R01 与 §P11b 之「零偏好基准下期望约 20%」补边界注：判据含等号且
            局号为正整数，实收局数为 N − ⌈0.80N⌉ + 1，N=50 时为 11 局即 **22%**。
            「约 20%」系区间名义值；据以做基准比对者应改用该式现算，
            否则基准偏低约 2 个百分点。
       C-10 本条。

     ═══ v11 · 经济结果层口径登记入包（2026-08-23 · 用户令 ＋ REDTEAM 裁定）═══
       ★ 本轮同为**代码零改动**：新增者系规章类块注释，无 SQL、无导出声明行；
         剥离注释后之纯代码与本轮之前逐字节相同，故 v11 仍不进位。

       E-01 新增 **§ECON · 经济结果层口径登记**（规章类，位于 §R01-chk 族之后、
            §R02 之前）。缘起：业务侧提问「15 类之判据既为 roi > 0，其余 14 类
            是否亦应补 roi／avg」。
       E-02 前提斧正（本轮最要紧之一条）：**roi 不是任何一类 T 之判据**，
            一条亦不是。以 T-03 为例，其判据列为 late_share／shoe_pos_p50／
            stake_ramp／tail_share，判级皆 PROFILE_ONLY 且已判 FATAL；
            roi 从来只是其经济后果。以 roi > 0 出名单行处置者判 S1 FATAL——
            零假设下会员 ROI 之期望非 0 而为 μ0 ＝ −hold%。
       E-03 命名三轴入卷首：T-01~T-15（风险类型）／R01…（交付件）／V（价值轴）
            禁互代；「T-03 ≠ R01」，后者仅为前者之一份交付件。
       E-04 §R01 头注加防误读锁：该节输出之 roi_late／roi_early 等经济列
            非本节判据、亦非 T-03 判据，身分见 §ECON。
       E-05 行业类比斧正：业界所载风控框架（liability／CLV／line movement／
            closing odds／arbitrage）多出自 sportsbook；本平台系真人厅百家乐，
            赔率由规则固定、无定价函数、无交易台、无收盘价，照搬即错配。
            §ECON 第二节已列对应表并判级 INFERRED，改取赌场机理者：
            玩法结构、免佣与否、三态结果（胜／负／和，禁二态胜率）、台桌与靴覆盖。
       E-06 三分律入册：风险判据 ≠ 经济结果 ≠ 价值优先，三者互不蕴含，禁合并。
       E-07 经济指标字典七要素（名／粒度／窗口／分子／分母／零分母／角色）
            与 ROI 两式之别（暴露加权式甲 vs 逐员等权式乙，禁互代）入册。
       E-08 economic_role 五值（NONE／PROFILE／OUTCOME／PRIORITIZATION／
            VALIDATION）入册，以防下游 SQL 将 roi 一列自动当成 criterion。
       E-09 Schema Debt 登记：登记册 YAML 之 admit_to_feature_scoring／
            admit_to_risk_decision 与 CSV 之 admit_to_scoring／ranking／scoring／
            trigger／enforcement 存双重真相之嫌，列为下一次 Schema RedTeam
            第一检查项，优先级高于加 ROI。本包不代改登记册，只登记与指向。
       E-10 施工顺序 STEP 0~8 锁死；**当前状态：STEP 1（§R01-0 ＋ §R01-0b）
            未执行**，故在其完成前，禁以 R01 之 roi／avg 口径为其余 14 类之模板——
            R01 之 shoe_len 粒度尚在 OPEN，以其为范本即把未决问题扩散至全体系。

       ⚠ 本轮**未做**而如实登记者：
         · 未为任何 T 类新增经济列之实际 SQL——口径未填实前不动代码；
         · 未改登记册 YAML／CSV；
         · 未执行 §R01-0／§R01-0b；
         · §ECON 第四节 A／B／C 三档指标凡未经可得性探针者一律判 UNKNOWN，
           不得径写入任何交付件之表头契约。
       ═══════════════════════════════════════════════════════════════════════

       ⚠ 本轮**未改**而如实登记者（留待另立变更单）：
         · 卷一旧式导出行「▸ 导出：「数据库/xxx.csv」」22 条与新式并存——
           卷首③已载其计数口径，非缺陷，本轮不强行归一以免动及卷一体例；
         · 卷首诸计数（154 条／75 条／72 条／69 个等）互斥之旧数，已由既有
           ⚠ 注声明「以现算为准」，本轮不逐条重算，另立专项；
         · §P12a／§P13 之 shoe_len 粒度差异——只加警示，不改代码。
       ═══════════════════════════════════════════════════════════════════════
