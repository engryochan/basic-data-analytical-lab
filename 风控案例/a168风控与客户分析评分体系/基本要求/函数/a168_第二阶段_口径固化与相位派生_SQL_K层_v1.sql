...
--         须回查是否误伤，不得径行接受。
--   ▸ 导出：需要 —— 存为「数据库/K01_valid_order_funnel.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH v AS (
...
--         使该项排除由不可见变为可核对。
--   ▸ 导出：需要 —— 存为「数据库/K02_testline_impact.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
...
--   ▸ 导出：需要 —— 存为「数据库/K03_amount_identity.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
-- ═══════════════════════════════════════════════════════════════════════════
--   §K-04 · 桌台窗口自数据求（严禁写死任何秒数）
--   缘起：第一阶段以手写之 13 与 25 为界划分制式，属硬编码阈值，自犯所禁。
--         本条改为逐桌反解窗口上界，秒数由数据给出，非由人给出。
--   方法：先求各局最末下注秒，再取该桌之高分位数为窗口上界。
--         分位数取 WIN_QUANTILE 等于 0.99，理由是逾制局占 0.37，
--         取 0.99 可容纳该量级之异常而不被其拉高。
--         并出 P50、P95、P999 与最大值，供检视分位数之选取是否稳健。
--   判读：若 P95 与 P99 相差在一秒之内，则窗口上界稳固。
--         若二者相差数秒，则该桌窗口不清晰，须单独查明后方可用于相位计算。
--   ▸ 导出：需要 —— 存为「数据库/K04_table_window.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
--         封盘前一秒实为一个整秒区间，非瞬时。
--   ▸ 导出：需要 —— 存为「数据库/K05_bet_phase_dist.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
--   ▸ 导出：需要 —— 存为「数据库/K06_player_round.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
--   ▸ 导出：需要 —— 存为「数据库/K07_grain_conservation.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
--         被剔除者非随机，而系统性偏向某一类局面。
--   ▸ 导出：需要 —— 存为「数据库/K08_sentinel_dealer.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
--   ▸ 导出：需要 —— 存为「数据库/K09_join_cardinality.csv」
-- ═══════════════════════════════════════════════════════════════════════════
WITH ta AS (
  SELECT DISTINCT age001 AS aid
  FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'
...
--   导出汇总，与语句一一对应
--     §K-00  不需要
--     §K-01  数据库/K01_valid_order_funnel.csv
--     §K-02  数据库/K02_testline_impact.csv
--     §K-03  数据库/K03_amount_identity.csv
--     §K-04  数据库/K04_table_window.csv
--     §K-05  数据库/K05_bet_phase_dist.csv
--     §K-06  数据库/K06_player_round.csv
--     §K-07  数据库/K07_grain_conservation.csv
--     §K-08  数据库/K08_sentinel_dealer.csv
--     §K-09  数据库/K09_join_cardinality.csv
