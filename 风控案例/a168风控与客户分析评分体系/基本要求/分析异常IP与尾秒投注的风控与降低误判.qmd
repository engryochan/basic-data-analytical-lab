...
```{r}
C01_有效订单漏斗核对 <- here(getwd(), '归档数据库', 'C01_有效订单漏斗核对.csv') |> 
  fread()
C01_有效订单漏斗核对 |> 
  gt()
```
...
```{r}
C02_IP清洗影响量化 <- here(getwd(), '归档数据库', 'C02_IP清洗影响量化.csv') |> 
  fread()
C02_IP清洗影响量化 |> 
  gt()
```
...
```{r}
C03_高聚集IP待确认清单 <- here(getwd(), '归档数据库', 'C03_高聚集IP待确认清单.csv') |> 
  fread()
C03_高聚集IP待确认清单 |> 
  gt() |>
  tab_options(
...
```{r}
C04_金额盈利口径日度对账 <- here(getwd(), '归档数据库', 'C04_金额盈利口径日度对账.csv') |> 
  fread()
C04_金额盈利口径日度对账 |> 
  gt() |>
  tab_options(
...
```{r}
C06_会员盈亏判定_全量窗口 <- here(getwd(), '归档数据库', 'C06_会员盈亏判定_全量窗口.csv') |> 
  fread()
...
```{r}
C07_月度稳定性面板 <- here(getwd(), '归档数据库', 'C07_月度稳定性面板.csv') |> 
  fread()
C07_月度稳定性面板 |> 
  gt()
```
...
```{r}
C08_滚动30日稳定性面板 <- here(getwd(), '归档数据库', 'C08_滚动30日稳定性面板.csv') |> 
  fread()
...
```{r}
C09_会员月度盈亏胜率面板 <- here(getwd(), '归档数据库', 'C09_会员月度盈亏胜率面板.csv') |> 
  fread()
