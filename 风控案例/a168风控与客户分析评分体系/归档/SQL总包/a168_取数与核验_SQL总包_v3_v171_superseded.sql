...
HAVING COUNT(*) >= 171                                                                              -- 分组后过滤：最小样本量闸【2026-08-17 优化落地，原基准 30】
                                                                                                    --   依据：S_player_tail.csv 实测 Wilson 95% 区间宽度——
                                                                                                    --     30→0.3369（为目标 0.15 之 2.25 倍，未达标）｜50→0.2671｜100→0.1923
                                                                                                    --     171→0.1482（首次达标）。原注称「低于 30 宽度>0.15」字面为真但具误导：
                                                                                                    --     30 并未使宽度达标，它是工程折中而非统计闸门。
                                                                                                    --   业务代价（实测，非估计）：会员 159,603→57,676（留存 36.14%），但——
                                                                                                    --     总局数留存 91.64%｜尾段洗码量留存 93.91%｜尾段退水留存 95.87%，
                                                                                                    --     即以 6.09% 洗码量覆盖换取一倍统计精度与 63.9% 审核量削减。
                                                                                                    --   ⚠ 本改动使输出行数由 159,603 降至 57,676，S_player_tail.csv 须重导。
...
SELECT flag_ip,                                                                                     -- 取列：起始取列子句，本行先列 flag_ip
       CASE WHEN tail_share >= PERCENTILE_APPROX(tail_share, 0.90) OVER () THEN 1 ELSE 0 END AS flag_tail_p90,-- 条件分支：产出「flag_tail_p90」【2026-08-17 优化落地，原基准写死 0.47】
                                                                                                    --   依据：别名称 p90，然实测 tail_share 之 P90 = 0.4156（基准口径）／0.4269
                                                                                                    --     （HAVING 改 171 后口径），与写死之 0.47 偏差 −11.6%／−9.2%。
                                                                                                    --     按 0.47 切命中 10,763 人（6.74%）；按真 P90 切 15,961 人（10.00%）——
                                                                                                    --     少捞约三分之一。此为硬编码失同步：一次算出写死，数据更新未随之变。
                                                                                                    --   改法：不再写死任何数值，改由引擎实算，口径随数据自动跟随，
                                                                                                    --     合于本项目「一切数字渲染时实算」铁律。
                                                                                                    --   ⚠ 若 StarRocks 版本不支持 PERCENTILE_APPROX 配 OVER()，退而求其次：
                                                                                                    --     先以子查询算出 P90 再 CROSS JOIN 回贴；切勿退回写死。X_combo.csv 须重导。
