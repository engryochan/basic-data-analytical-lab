/* ╔═══════════════════════════════════════════════════════════════════════════╗
   ...
   ═══ 分析窗口（正名版，务必按此措辞对外）═══════════════════════════════════
     ...
     改窗时须同时全量替换两枚窗口字面量（v5 实测：'2026-03-21' 共 182 处／'2026-08-07' 共 251 处），不得只改其一；
   ...
   ═══ 血统关系登记（2026-08-13 立 · 一族四档）════════════════════════════
     本包为「取数层」，居一族四档之料源位。改本包即改全族之数字，故列明：
       ...
       a168_取数与核验_SQL总包_v7.sql               本包(料源)   一切数字之唯一出处
   ...
   【第六批 · 评估线归宗（P11 尾段两域族 v6 并入 ＋ P12 序列与事件族 v5 并入）】
     ...
     §P11b → 数据库/P11_雷欧尾段投注模型会员名单.csv  （契约名不英译；§R01 同构加扩展列）
     ...
     §P12d → 数据库/R01_late_shoe_w2.csv      §R01 原文仅改两处窗口字面量（新窗）
   ...
   ╚══════════════════════════════════════════════════════════════════════════╝
     ...
     配套三条：
       ...
       ③ 全包现状（2026-08-12 逐行实测；计数口径＝行首以 -- 起首的导出标记行）：
          产出交付件的语句 72 条（新式 50 条 + 卷一旧式块注释 22 条；v5 并入 §P12a/§P12b/§P12c 三条，v6 并入 §P11a~d 四条；§P12b-pre 只看屏幕不计，§P12d 系改窗指引无独立语句），
...
-- ─────────────────────────────────────────────────────────────────────────
-- §P11a · 阿泽（洗码口径）会员名单
-- 导出：存为「数据库/P11_阿泽尾段投注模型会员名单.csv」（契约名，不英译）
-- ─────────────────────────────────────────────────────────────────────────
-- eval = FALSE
-- 阿泽（模型）：原码保持不变，仅加五闸
...
-- 导出：存为「数据库/P11_雷欧尾段投注模型会员名单.csv」（契约名，不英译）
-- ─────────────────────────────────────────────────────────────────────────
-- eval = FALSE
/* ───────────────────────────────────────────────────────────────────────────
   §R01 · R01_late_shoe.csv
...
-- 导出：存为「数据库/P11_阿泽尾段投注总帐.csv」（契约名，不英译）
-- ─────────────────────────────────────────────────────────────────────────
-- eval = FALSE
...
-- 导出：存为「数据库/P11_雷欧尾段投注总帐.csv」（契约名，不英译）
-- ─────────────────────────────────────────────────────────────────────────
-- eval = FALSE
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12b-pre · content 段式目检（只看屏幕，不导出；先于 §P12b 执行）
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12a · 平台日度序列（存为「数据库/P12a_platform_daily.csv」）
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12b · 会员事件表（存为「数据库/P12b_member_events.csv」）
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12c · 会员逐日盈亏（存为「数据库/P12c_member_daily_pnl.csv」）
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12d · 新窗口雷欧名单（存为「数据库/R01_late_shoe_w2.csv」）——改窗指引
   ▸ 非新 SQL：即本包 §R01 整段原文**仅改两处窗口字面量**（别无一字之差，
     防口径漂移）：
       旧：AND b.dt >= '2026-03-21' AND b.dt < '2026-08-07'
       新：AND b.dt >= '2026-08-08' AND b.dt < '2026-08-20'
   ▸ 输出列、口径锁（late_def='shoe_pos>=0.80'）、HAVING n>=100 一律原样；
     intake_ver 建议置 'M2A_W2' 以别窗次。
   ▸ 提醒：新窗仅 12 天，n>=100 命中人数必远小于 139 天窗——此系窗口长度
     之数学而非异常；样本量小则真样本外 AUC 之 CI 相应加宽，门禁按五项并报
     如实陈述，不得因人数少而降门槛。
   ───────────────────────────────────────────────────────────────────────── */
...
-- ─────────────────────────────────────────────────────────────────────────
-- §P13 · 会员入场行为层（存为「数据库/P13_personal_entry.csv」）
