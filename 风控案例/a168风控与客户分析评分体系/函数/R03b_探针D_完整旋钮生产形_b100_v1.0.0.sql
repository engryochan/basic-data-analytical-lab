-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- R03b ＃071 · 验收探针 D · 完整旋钮之生产形 · 第 100 批   v1.0.0   2026-09-05
-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- 【本体】自现行总包 ＃071（分批作业版1万）逐字复制，排序键已是裁剪后之 5 键。只改两处：
--   ① 最外层 SELECT 加旋钮提示；② 批次窗改第 100 批，以便与现档逐字节比对。
--
-- 【相对探针 C，本探针【新增两个旋钮】—— 皆系 2026-09-05 四条报错所迫】
--   ★ query_mem_limit = 34,359,738,368（32 GiB）
--     现况 query_mem_limit = 0 ＝【不限】⇒ 本查询会尽力抢占，直到撞进程／资源组之顶。
--     ⇒ 于已被他者占满之节点上，此举既抢不到、又把自己逼死。
--     给一个明确预算后：① 本查询自我设限，不再与他者争最后一 GiB；
--                      ② spill_mem_limit_threshold = 0.8 遂变成「用到【预算】之 80%% 即落盘」，
--                         而非「用到【节点上限】之 80%%」—— 后者在节点已被占满时根本触发不了。
--     ⇒ 这才是先生前问「能否用到九成而非撑爆」之【真正落点】。
--   ★ pipeline_dop = 8（现况 0 ＝ 自动，上限 max_pipeline_dop = 64）
--     降并行度即削峰值；代价是慢些。
--
-- 【⛔ 并请弃用 `SET enable_spill = true;` 之写法】
--   探针 M3 之 P-6／P-7 已判死：SET 单独执行后，另一次执行读回 @@enable_spill = 0
--   ⇒ Superset SQL Lab 每次执行不复用会话。先生近日所加之该行，**极可能一直没有生效**。
--   ⇒ 一律改用本探针之 SET_VAR 提示 —— 它绑在这一句上，必然生效（P-1~P-4 已证）。
--
-- 【⚑ 须诚实言明】2026-09-05 之四条报错，皆落在同一节点 BE:10267（172.31.80.38），
--   其中两条在【仅 79.37%% / 84.07%%】占用处即拒配 0.88 MB / 5.00 MB，
--   另两条之 Used 逐字节相同（107,096,518,320，高于上限 3.98 MB）。
--   ⇒ **主因在集群侧（疑该节点内存未释放，且 172.31.89.70 已宕），SQL 无从根治。**
--   本探针之旋钮只能【把本查询变成更小、更守规矩的一个目标】，不能修好那个节点。
--
-- 【只读】纯 SELECT。提示只改执行方式，不改任何列值与行序。
-- 【验收法】与现档 R03b_player_dealer_daily_b100.csv 逐字节比对
--   （10,000 行 · 151 栏 · 15,614,711 B · MD5 e1503f6e29f189f9676fd0f7a7ee580f）。
--   ★ 完全一致方可入生产。
--
-- 【若仍失败 · 加码阶梯（逐级下调，每级皆须重验逐字节）】
--   第一级　query_mem_limit=32 GiB · pipeline_dop=8   ← 本探针
--   第二级　query_mem_limit=16 GiB · pipeline_dop=4
--   第三级　query_mem_limit= 8 GiB · pipeline_dop=4 · spill_mode='force'
--   ⛔ 若第三级仍在【八成占用】处拒配，即可断定与本查询无关，
--      须持 审计/StarRocks_BE节点宕机_报修_v1.0.0_20260905.md 报部门。
-- ══════════════════════════════════════════════════════════════════════════════════════════════

SELECT /*+ SET_VAR(query_mem_limit=34359738368,enable_spill=true,spill_mode='auto',spill_mem_limit_threshold=0.8,pipeline_dop=8) */ z.*,
...
--       【原 26 键逐字留档，可原样恢复】
         --       原：ROW_NUMBER() OVER (ORDER BY y.`residual_b` ASC, y.`roi` DESC, y.`bet_date`, y.`uid`, y.`dealer_id`, y.`is_sentinel_dealer`, y.`stake_amount`, y.`profit_amount`, y.`net_pnl`, y.`win_rate`, y.`n_related_orders`, y.`n_rounds_eff`, y.`p_base_mix_w`, y.`p_base_mix_unw`, y.`z_score_w`, y.`z_score_unw`, y.`z_score`, y.`delta_p`, y.`eligibility_status`, y.`comparison_id`, y.`filter_stage`, y.`filter_rule_version`, y.`cmp_time_window`, y.`cmp_population`, y.`cmp_label`, y.`z_score_alias_status`) AS audit_rn
