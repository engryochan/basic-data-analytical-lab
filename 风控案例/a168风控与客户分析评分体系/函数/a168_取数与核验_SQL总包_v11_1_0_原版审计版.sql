-- ══════════════════════════════════════════════════════════════════════════════
-- ★ a168 取数与核验 SQL 总包 · v11.1.0 · 【原版审计版】★
-- ══════════════════════════════════════════════════════════════════════════════
-- 立于 2026-08-24 · 派生自 a168_取数与核验_SQL总包_v11_1_0.sql（REL-0006 / C1）
-- 应 REDTEAM 2026-08-24 裁定：为原版与分批版建立**同一套全局观测坐标 audit_rn**，
-- 以证分批只是物理切片，未改变 observation set。
--
-- 【身份】派生作业件，非总包本体，不得取代 v11.1.0，不入 §Z-16 变更记录。
--   内层查询自本体逐字复制，仅剥去末尾顶层 ORDER BY 与分号，另加一列 audit_rn。
--
-- 【与分批作业版之关系 · 四锁（逐条照 REDTEAM 裁定）】
--   锁一：两版之 ROW_NUMBER() OVER (ORDER BY ...) **逐字一致** —— 本档之键系自
--         分批作业版复制，非另行推导；两档同源同式，故 audit_rn 同号必同行。
--   锁二：排序须确定性解决 tie —— 键尾补齐全部输出别名，故 ordering tuple 唯一。
--   锁三：audit_rn 只作审计字段，**不得进入任何业务指标计算**。
--   锁四：两版经 audit_rn 逐行字段核验后，方可宣布输出完全一致。
--
-- 【★ 一处物理约束，务请先生明察 ★】
--   导出上限每批十万行。凡 T_true > 100000 之件，本档**同样导不出全量**——
--   本档之用途不是「一次导全作对照」，而是：
--     ① 供 §D07 分块指纹取数（原版留库，只导 69,641 行之分块表，一次可导）；
--     ② 供 T_true ≤ 100000 之件一次导全，与分批版直接逐行 JOIN 对账。
--   若某件 T_true > 100000 而先生仍要逐行对账，则本档亦须切片——那便是分批作业版。
--   ★ 故：**两版之别只在有无切片，audit_rn 之定义完全相同。**
--
-- 【体例】全档只用行注释，不引入块注释。
-- ══════════════════════════════════════════════════════════════════════════════
-- ──────────────────────────────────────────────────────────────────────────────
--   1. A_anchor.csv   [总包 v11.1.0 行 2999~3009 · 输出 5 列]
--     原 ORDER BY：n_orders DESC
--     audit_rn 之排序键与分批作业版第 1 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_orders` DESC, y.`ip`, y.`n_member`, y.`first_dt`, y.`last_dt`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--   2. B01_bt_panel.csv   [总包 v11.1.0 行 5314~5392 · 输出 14 列]
--     原 ORDER BY：bet_date, member_id
--     audit_rn 之排序键与分批作业版第 2 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`bet_date`, y.`member_id`, y.`stake`, y.`game_pnl`, y.`net_pnl`, y.`rebate`, y.`validbet`, y.`n_rounds`, y.`n_tables`, y.`n_ip`, y.`late_def`, y.`late_share_d`, y.`hedge_rate_d`, y.`stake_cv_d`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--   3. B01_bt_panel_delta.csv   [总包 v11.1.0 行 5406~5484 · 输出 14 列]
--     原 ORDER BY：member_id, bet_date
--     audit_rn 之排序键与分批作业版第 3 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`bet_date`, y.`stake`, y.`game_pnl`, y.`net_pnl`, y.`rebate`, y.`validbet`, y.`n_rounds`, y.`n_tables`, y.`n_ip`, y.`late_def`, y.`late_share_d`, y.`hedge_rate_d`, y.`stake_cv_d`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--   5. B4_5b_tip_nature.csv   [总包 v11.1.0 行 12072~12103 · 输出 16 列]
--     原 ORDER BY：n_rows DESC, bet09_value
--     audit_rn 之排序键与分批作业版第 5 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rows` DESC, y.`bet09_value`, y.`n_members`, y.`n_days`, y.`n_b13_blank`, y.`n_b13_zero`, y.`n_b13_pos`, y.`n_b14_blank`, y.`n_b14_zero`, y.`n_b14_pos`, y.`n_both_pos`, y.`n_b13pos_b14zero`, y.`b13_sum`, y.`b14_sum`, y.`n_comm`, y.`verdict_nature`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--   6. B_online_base.csv   [总包 v11.1.0 行 2929~2991 · 输出 7 列]
--     原 ORDER BY：table_id, sec_elapsed
--     audit_rn 之排序键与分批作业版第 6 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`table_id`, y.`sec_elapsed`, y.`n_orders`, y.`n_player`, y.`n_rounds`, y.`valid_bet`, y.`game_pnl`) AS audit_rn
FROM (
  ...
  GROUP BY table_id, sec_elapsed
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--   7. C01_ip_chain.csv   [总包 v11.1.0 行 1663~1723 · 输出 8 列]
--     原 ORDER BY：member_per_chain DESC
--     audit_rn 之排序键与分批作业版第 7 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_per_chain` DESC, y.`bet_ip`, y.`n_member`, y.`n_lv3_chain`, y.`n_orders`, y.`stake`, y.`game_pnl`, y.`net_pnl`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--   8. C06_hedge_pairs.csv   [总包 v11.1.0 行 5814~5886 · 输出 7 列]
--     原 ORDER BY：opposite_rate DESC, n_opposite_round DESC
--     audit_rn 之排序键与分批作业版第 8 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`opposite_rate` DESC, y.`n_opposite_round` DESC, y.`bet_ip`, y.`m_a`, y.`m_b`, y.`n_same_round`, y.`hedge_coverage`) AS audit_rn
FROM (
  ...
  rk AS (                                                                                             -- 公共表表达式：开启中间结果集 rk，其后各行为其定义体（§C06fix）
    SELECT b.bet01, b.updatetime, b.sync_time, b.dt, b.bet02,                                         -- 取列：起始取列子句，本行先列 b.bet01, b.updatetime, b.sync_time, b.dt, b.bet02，涉 bet02（游戏类别）、dt（营业日）
           ...
           b.bet21, b.bet22, b.bet38, b.bet39, b.category,                                            -- 续行：接续上一取列子句，续列 b.bet21, b.bet22, b.bet38, b.bet39, b.category，涉 bet21（四级代理线）、bet22（五级代理线）、bet38（重對）
...
-- ──────────────────────────────────────────────────────────────────────────────
--   9. C06d_hedge_pairs_dated.csv   [总包 v11.1.0 行 8067~8133 · 输出 9 列]
--     原 ORDER BY：opposite_rate DESC, n_opposite_round DESC
--     audit_rn 之排序键与分批作业版第 9 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`opposite_rate` DESC, y.`n_opposite_round` DESC, y.`bet_ip`, y.`m_a`, y.`m_b`, y.`n_same_round`, y.`first_opposite_dt`, y.`last_opposite_dt`, y.`n_opposite_days`) AS audit_rn
FROM (
  WITH ta AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  10. C08_subnet_all.csv   [总包 v11.1.0 行 1759~1820 · 输出 8 列]
--     原 ORDER BY：n_member DESC
--     audit_rn 之排序键与分批作业版第 10 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_member` DESC, y.`subnet_24`, y.`n_ip`, y.`n_lv3_chain`, y.`n_orders`, y.`stake`, y.`game_pnl`, y.`valid_bet`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  11. C08_subnet_sparse.csv   [总包 v11.1.0 行 1829~1890 · 输出 8 列]
--     原 ORDER BY：n_member DESC
--     audit_rn 之排序键与分批作业版第 11 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_member` DESC, y.`subnet_24`, y.`n_ip`, y.`n_lv3_chain`, y.`member_per_ip`, y.`n_orders`, y.`valid_bet`, y.`game_pnl`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  12. D03S_daily_roi_diff.csv   [总包 v11.1.0 行 1991~2062 · 输出 7 列]
--     原 ORDER BY：bet_date
--     audit_rn 之排序键与分批作业版第 12 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`bet_date`, y.`n_rounds`, y.`vb_tail`, y.`vb_norm`, y.`roi_tail`, y.`roi_norm`, y.`roi_diff`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
  ...
  SELECT bet_date, COUNT(DISTINCT round_key) AS n_rounds,                                             -- 取列：起始取列子句，本行先取「n_rounds」，涉 round_key（局键）、bet_date（营业日）
    ...
    - SUM(CASE WHEN progress< 0.90 THEN game_pnl ELSE 0 END)                                          -- 加减计算：晚段 ROI 减早段 ROI ＝ 分段差，与 GGR 无涉
...
-- ──────────────────────────────────────────────────────────────────────────────
--  13. DICT_01a_bet41_tri.csv   [总包 v11.1.0 行 11764~11798 · 输出 14 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 13 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope`, y.`n_rows`, y.`sum_stake`, y.`sum_validbet`, y.`sum_bet41`, y.`sum_bet16`, y.`r41_vb`, y.`r16_vb`, y.`r_vb_stake`, y.`avg_bet15_pct`, y.`n_41_eq_vb`, y.`n_41_eq_16`, y.`pct_41_eq_vb`, y.`verdict_bet41`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  14. DICT_01b_roundkey_card.csv   [总包 v11.1.0 行 11812~11835 · 输出 14 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 14 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope`, y.`n_rows`, y.`nd_bet03`, y.`nd_bet04`, y.`nd_bet39`, y.`nd_bet40`, y.`min_bet04`, y.`max_bet04`, y.`min_bet39`, y.`max_bet39`, y.`nd_round_key`, y.`nd_shoe_table`, y.`avg_bets_per_round`, y.`verdict_key`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  15. DICT_01c_dimension.csv   [总包 v11.1.0 行 11850~11878 · 输出 12 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 15 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope`, y.`n_rows`, y.`avg_validbet`, y.`max_bet23`, y.`avg_bet23`, y.`max_bet27`, y.`max_bet28`, y.`avg_bet28`, y.`max_bet32`, y.`max_bet15`, y.`avg_sum_comm`, y.`verdict_dimension`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  16. DICT_01d_bet_latency.csv   [总包 v11.1.0 行 11893~11926 · 输出 14 列]
--     原 ORDER BY：n_rows DESC, table_id
--     audit_rn 之排序键与分批作业版第 16 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rows` DESC, y.`table_id`, y.`n_members`, y.`n_b06_null`, y.`n_b08_null`, y.`n_negative`, y.`lat_min`, y.`lat_avg`, y.`lat_max`, y.`lat_p01`, y.`lat_p50`, y.`lat_p99`, y.`lat_sd`, y.`verdict_latency`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  17. DX04_bet09_profile.csv   [总包 v11.1.0 行 5971~6005 · 输出 9 列]
--     原 ORDER BY：n_orders DESC
--     audit_rn 之排序键与分批作业版第 17 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_orders` DESC, y.`bet_side`, y.`n_member`, y.`stake_total`, y.`player_pnl`, y.`house_hold_pct`, y.`win_rate_base`, y.`avg_stake`, y.`stake_cv`) AS audit_rn
FROM (
  WITH ranked AS (                                                                                    -- 公共表表达式：开启中间结果集 ranked，其后各行为其定义体（§DX-04）
    SELECT b.bet01, b.updatetime, b.sync_time, b.dt, b.bet02,                                         -- 取列：起始取列子句，本行先列 b.bet01, b.updatetime, b.sync_time, b.dt, b.bet02，涉 bet02（游戏类别）、dt（营业日）
           ...
           b.bet38, b.category,                                                                       -- 续行：接续上一取列子句，续列 b.bet38, b.category，涉 bet38（重對）
           ROW_NUMBER() OVER (                                                                        -- 行号窗口表达式：以行号窗口取每组头部或去重，免出重复行
             PARTITION BY b.bet01                                                                     -- 窗口分区：按 b.bet01 分组开窗，组内各自编号或排名
             ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC) AS rn                           -- 排序：按 b.updatetime（降序）, b.sync_time（降序）, b.dt（降序）) AS rn 排列；导出必带排序，否则分页无稳定序（曾致 36.49% 重复行）
    FROM ods_mariadb_2b.ods_a168_bet02 b                                                              -- 取数来源：取自注单明细表（金额与行为口径的第一料源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  18. DX05_product_panorama.csv   [总包 v11.1.0 行 6020~6073 · 输出 14 列]
--     原 ORDER BY：n_orders DESC
--     audit_rn 之排序键与分批作业版第 18 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_orders` DESC, y.`product_code`, y.`scope`, y.`n_member`, y.`n_dealer`, y.`stake_total`, y.`validbet_total`, y.`player_pnl`, y.`ggr`, y.`house_hold_pct`, y.`rebate_total`, y.`rebate_rate`, y.`order_share`, y.`stake_share`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  19. E02_segment_econ.csv   [总包 v11.1.0 行 5707~5797 · 输出 9 列]
--     原 ORDER BY：validbet DESC
--     audit_rn 之排序键与分批作业版第 19 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`validbet` DESC, y.`segment`, y.`n_member`, y.`ggr`, y.`rebate`, y.`commission`, y.`comm_scale`, y.`reb_scale`, y.`tenure_months`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  20. EV01_disposal_events.csv   [总包 v11.1.0 行 7112~7153 · 输出 13 列]
--     原 ORDER BY：optime
--     audit_rn 之排序键与分批作业版第 20 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`event_id`, y.`event_time`, y.`event_date`, y.`fn`, y.`operator`, y.`operator_ip`, y.`event_type`, y.`member_login`, y.`table_ref`, y.`round_ref`, y.`whitelist_ip`, y.`is_cancel`, y.`content_len`) AS audit_rn
FROM (
  SELECT                                                                                              -- 续行：接续上一取列子句，续列 SELECT
...
-- ──────────────────────────────────────────────────────────────────────────────
--  21. EV02_member_disposal.csv   [总包 v11.1.0 行 7289~7322 · 输出 12 列]
--     原 ORDER BY：lmc09
--     audit_rn 之排序键与分批作业版第 21 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`event_id`, y.`event_date`, y.`event_time`, y.`type_code`, y.`action`, y.`operator`, y.`event_type`, y.`val_before`, y.`val_after`, y.`summary`, y.`is_batch`, y.`stmt_len`) AS audit_rn
FROM (
  SELECT                                                                                              -- 续行：接续上一取列子句，续列 SELECT
...
-- ──────────────────────────────────────────────────────────────────────────────
--  22. I_ip_agg.csv   [总包 v11.1.0 行 2695~2768 · 输出 14 列]
--     原 ORDER BY：n_member_eff DESC
--     audit_rn 之排序键与分批作业版第 22 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_member_eff` DESC, y.`window_id`, y.`bet_ip`, y.`n_member_raw`, y.`n_prof_A`, y.`n_prof_B`, y.`n_prof_C`, y.`n_orders`, y.`n_rounds`, y.`stake`, y.`net_pnl`, y.`game_pnl`, y.`share_p50`, y.`share_p90`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  23. I_ip_player.csv   [总包 v11.1.0 行 1589~1654 · 输出 14 列]
--     原 ORDER BY：n_orders_ip DESC
--     audit_rn 之排序键与分批作业版第 23 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_orders_ip` DESC, y.`bet_ip`, y.`member_id`, y.`lv3`, y.`n_rounds_ip`, y.`stake_ip`, y.`game_pnl_ip`, y.`rebate_ip`, y.`net_pnl_ip`, y.`n_orders_all`, y.`n_rounds_all`, y.`net_pnl_all`, y.`game_pnl_all`, y.`ip_order_share`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  24. K01_risk_feature_matrix.csv   [总包 v11.1.0 行 5499~5591 · 输出 15 列]
--     原 ORDER BY：f.validbet DESC
--     audit_rn 之排序键与分批作业版第 24 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`validbet` DESC, y.`member_id`, y.`is_seed`, y.`n_rounds`, y.`n_days`, y.`n_tables`, y.`n_ip`, y.`n_dealer`, y.`side_entropy`, y.`stake_cv`, y.`stake_p90_p50`, y.`hedge_rate`, y.`rebate_dep`, y.`night_share`, y.`roi_game`) AS audit_rn
FROM (
  ...
  side_ent AS (                                                                                       -- 公共表表达式：开启中间结果集 side_ent，其后各行为其定义体（§K01）
    SELECT s.member_id,                                                                               -- 取列：起始取列子句，本行先列 s.member_id，涉 member_id（会员号）
           -SUM((s.n_side*1.0/t.n_all) * LOG2(s.n_side*1.0/t.n_all)) AS side_entropy                  -- 加减计算：香农熵定义式之前置负号，与 GGR 无涉，产出「side_entropy」
...
-- ──────────────────────────────────────────────────────────────────────────────
--  25. K01b_seed_goldip.csv   [总包 v11.1.0 行 5637~5643 · 输出 2 列]
--     原 ORDER BY：member_id
--     audit_rn 之排序键与分批作业版第 25 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`seed_src`) AS audit_rn
FROM (
  ...
  WHERE b.dt >= '2026-03-21' AND b.dt < '2026-08-07'                                                  -- 过滤条件：限定 b.dt不少于 '2026-03-21'、b.dt小于 '2026-08-07'，涉 dt（营业日）
    AND b.bet02 = '101' AND b.category = '1'                                                          -- 并列条件：限定 b.bet02等于 '101'、b.category等于 '1'，涉 bet02（游戏类别）
    AND UPPER(TRIM(b.bet38)) = 'N'                                                                    -- 并列条件：限定 UPPER(TRIM(b.bet38))等于 'N'，涉 bet38（重對）
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  26. K01b_seed_manual.csv   [总包 v11.1.0 行 5618~5625 · 输出 2 列]
--     原 ORDER BY：member_id
--     audit_rn 之排序键与分批作业版第 26 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`seed_src`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  27. K01c_seed_dated.csv   [总包 v11.1.0 行 8330~8337 · 输出 6 列]
--     原 ORDER BY：addtime
--     audit_rn 之排序键与分批作业版第 27 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`addtime`, y.`seed_id`, y.`seed_ip`, y.`creator`, y.`add_date`, y.`remarks`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  28. L_label_dist.csv   [总包 v11.1.0 行 1472~1476 · 输出 6 列]
--     原 ORDER BY：n_rows DESC
--     audit_rn 之排序键与分批作业版第 28 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rows` DESC, y.`risk`, y.`orders`, y.`n_member`, y.`first_dt`, y.`last_dt`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  29. P01A_crosstab.csv   [总包 v11.1.0 行 9199~9219 · 输出 13 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 29 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rows`, y.`bet16_null`, y.`bet16_zero`, y.`bet16_nonzero`, y.`bet41_null`, y.`bet41_zero`, y.`bet41_nonzero`, y.`equal_n`, y.`unequal_n`, y.`both_zero_n`, y.`b16zero_b41pos_n`, y.`both_pos_n`, y.`b16pos_b41zero_n`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  30. P01B_diff_ratio.csv   [总包 v11.1.0 行 9223~9240 · 输出 11 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 30 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n`, y.`diff_min`, y.`diff_p50`, y.`diff_p90`, y.`diff_p99`, y.`diff_max`, y.`ratio_min`, y.`ratio_p01`, y.`ratio_p50`, y.`ratio_p99`, y.`ratio_max`) AS audit_rn
FROM (
  SELECT COUNT(*)                                                        AS n,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  31. P01C_split_by_dim.csv   [总包 v11.1.0 行 9246~9269 · 输出 14 列]
--     原 ORDER BY：ym, bet09, commission, bet10
--     audit_rn 之排序键与分批作业版第 31 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`ym`, y.`bet09`, y.`commission`, y.`bet10`, y.`n`, y.`equal_n`, y.`unequal_n`, y.`b16_nonzero_n`, y.`b41_nonzero_n`, y.`avg_b16`, y.`avg_b41`, y.`avg_diff`, y.`p50_b16`, y.`p50_b41`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  32. P01D_vs_validbet.csv   [总包 v11.1.0 行 9274~9290 · 输出 9 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 32 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n`, y.`comparable_n`, y.`plausible_n`, y.`r41_p01`, y.`r41_p50`, y.`r41_p90`, y.`r41_p99`, y.`r16_p50`, y.`r16_p99`) AS audit_rn
FROM (
  SELECT COUNT(*)                                                         AS n,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  33. P01E_identity_test.csv   [总包 v11.1.0 行 9295~9330 · 输出 8 列]
--     原 ORDER BY：hypothesis
--     audit_rn 之排序键与分批作业版第 33 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`hypothesis`, y.`n`, y.`exact_match_n`, y.`exact_match_rate`, y.`abs_err_p50`, y.`abs_err_p95`, y.`abs_err_p99`, y.`abs_err_max`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  34. P02A_lmc_samples.csv   [总包 v11.1.0 行 9351~9363 · 输出 9 列]  [🚩第 [2] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
WITH r AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  35. P02B_changestatus_patterns.csv   [总包 v11.1.0 行 9369~9378 · 输出 6 列]
--     原 ORDER BY：n DESC
--     audit_rn 之排序键与分批作业版第 35 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n` DESC, y.`内容前40字`, y.`涉实体数`, y.`最早`, y.`最晚`, y.`其中层级7数`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  36. P02C_key_check.csv   [总包 v11.1.0 行 9385~9401 · 输出 8 列]
--     原 ORDER BY：l.lmc03
--     audit_rn 之排序键与分批作业版第 36 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`层级`, y.`日志行数`, y.`去重实体数`, y.`实体ID为0之脏行`, y.`命中注单会员数`, y.`命中去重实体数`, y.`最早`, y.`最晚`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  37. P02D_events_in_window.csv   [总包 v11.1.0 行 9406~9414 · 输出 4 列]
--     原 ORDER BY：类别, 层级
--     audit_rn 之排序键与分批作业版第 37 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`类别`, y.`层级`, y.`窗口内行数`, y.`涉实体数`) AS audit_rn
FROM (
  SELECT lmc04                                                            AS 类别,
         lmc03                                                            AS 层级,
         COUNT(*)                                                         AS 窗口内行数,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  38. P0C02_标签口径.csv   [总包 v11.1.0 行 7728~7761 · 输出 6 列]  [🚩第 [1, 2, 3, 4, 5, 6] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
...
-- ──────────────────────────────────────────────────────────────────────────────
--  39. P0C03_标记次数分布.csv   [总包 v11.1.0 行 7767~7781 · 输出 3 列]
--     原 ORDER BY：标记次数
--     audit_rn 之排序键与分批作业版第 39 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`标记次数`, y.`会员数`, y.`累计会员数`) AS audit_rn
FROM (
  SELECT  标记次数,
          COUNT(*)                                      AS 会员数,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  40. P0C04_同日泄漏检验.csv   [总包 v11.1.0 行 7793~7828 · 输出 7 列]
--     原 ORDER BY：标记日_洗码量 DESC
--     audit_rn 之排序键与分批作业版第 40 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`标记日_洗码量` DESC, y.`mem`, y.`mark_day`, y.`标记日_注单数`, y.`前30日均_注单数`, y.`前30日均_洗码量`, y.`前30日活跃日数`) AS audit_rn
FROM (
  WITH d0 AS (
      SELECT  CAST(bet05 AS STRING)                     AS mem,
              MIN(dt)                                   AS mark_day
...
-- ──────────────────────────────────────────────────────────────────────────────
--  41. P0C05_标记时点定位.csv   [总包 v11.1.0 行 7833~7864 · 输出 8 列]
--     原 ORDER BY：标记距首投_日
--     audit_rn 之排序键与分批作业版第 41 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`标记距首投_日`, y.`mem`, y.`mark_day`, y.`首投日`, y.`末投日`, y.`活跃日数`, y.`末投距标记_日`, y.`时点性质`) AS audit_rn
FROM (
  SELECT  d0.mem,
          d0.mark_day,
          life.首投日,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  42. P0C06_LabelB骨架.csv   [总包 v11.1.0 行 7871~7904 · 输出 8 列]
--     原 ORDER BY：a.mem, a.anchor_day
--     audit_rn 之排序键与分批作业版第 42 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`mem`, y.`anchor_day`, y.`特征窗起`, y.`特征窗止`, y.`结果窗起`, y.`结果窗止`, y.`label_b_未来14日被标记`, y.`label_a_当日被标记`) AS audit_rn
FROM (
  WITH mk AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  43. P0C09_安慰剂检验.csv   [总包 v11.1.0 行 7995~8032 · 输出 7 列]
--     原 ORDER BY：a.mem, a.d
--     audit_rn 之排序键与分批作业版第 43 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`mem`, y.`参照日`, y.`参照日_注单数`, y.`参照日_洗码量`, y.`前30日均_注单数`, y.`前30日均_洗码量`, y.`前30日活跃日数`) AS audit_rn
FROM (
  WITH mk AS (
      SELECT  DISTINCT CAST(bet05 AS STRING)            AS mem,
              dt                                        AS mark_day
...
-- ──────────────────────────────────────────────────────────────────────────────
--  44. P11_阿泽尾段投注总帐.csv   [总包 v11.1.0 行 9519~9655 · 输出 21 列]
--     原 ORDER BY：b.scope_def
--     audit_rn 之排序键与分批作业版第 44 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope_def`, y.`n_orders`, y.`n_orders_pct`, y.`n_members`, y.`n_members_pct`, y.`n_lv1`, y.`n_lv2`, y.`n_lv3`, y.`n_lv4`, y.`n_lv5`, y.`stake_sum`, y.`stake_pct`, y.`validbet_sum`, y.`validbet_pct`, y.`ggr_sum`, y.`ggr_pct`, y.`rebate16_sum`, y.`rebate16_pct`, y.`rebate41_sum`, y.`ngr_by16`, y.`ngr_by16_pct`) AS audit_rn
FROM (
  WITH test_agents AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  45. P11_阿泽尾段投注模型会员名单.csv   [总包 v11.1.0 行 9443~9500 · 输出 9 列]
--     原 ORDER BY：roi DESC, profit DESC
--     audit_rn 之排序键与分批作业版第 45 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`roi` DESC, y.`profit` DESC, y.`player_id`, y.`total_orders`, y.`late_orders`, y.`late_ratio`, y.`win_orders`, y.`win_rate`, y.`valid_bet`) AS audit_rn
FROM (
  WITH test_agents AS (
      SELECT age001 AS agent_id
      FROM ods_mariadb_2b.ods_a168_agent
...
-- ──────────────────────────────────────────────────────────────────────────────
--  46. P11_雷欧尾段投注总帐.csv   [总包 v11.1.0 行 9667~9769 · 输出 21 列]
--     原 ORDER BY：b.scope_def
--     audit_rn 之排序键与分批作业版第 46 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope_def`, y.`n_orders`, y.`n_orders_pct`, y.`n_members`, y.`n_members_pct`, y.`n_lv1`, y.`n_lv2`, y.`n_lv3`, y.`n_lv4`, y.`n_lv5`, y.`stake_sum`, y.`stake_pct`, y.`validbet_sum`, y.`validbet_pct`, y.`ggr_sum`, y.`ggr_pct`, y.`rebate16_sum`, y.`rebate16_pct`, y.`rebate41_sum`, y.`ngr_by16`, y.`ngr_by16_pct`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  47. P12a_platform_daily.csv   [总包 v11.1.0 行 9808~9873 · 输出 9 列]
--     原 ORDER BY：dt
--     audit_rn 之排序键与分批作业版第 47 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`dt`, y.`n_orders`, y.`n_members`, y.`stake_sum`, y.`validbet_sum`, y.`ggr_sum`, y.`late_stake_share`, y.`stake_ramp_day`, y.`hold_pct`) AS audit_rn
FROM (
  ...
  ranked AS (                                                                                         -- 公共表表达式：开启中间结果集 ranked——同注单号取最新版本（§P12a）
      SELECT b.bet01, b.bet03, b.bet04, b.bet05, b.bet11, b.bet13, b.bet14,                           -- 取列：起始取列子句，涉 bet03（靴号）、bet04（局内序号）、bet05（会员号）
             ...
             b.bet38, b.category, b.validbet, b.dt,                                                   -- 续行：接续取列，涉 bet38（重對）、validbet（洗码量）、dt（营业日）
  ...
  shoe_len AS (          /* ★★ 命名与粒度双注（2026-08-23 立；G-5 已据实测纠错）★★ 一、本 CTE 名为 shoe_len（靴长）系历史沿用，所算 max_round = MAX(round_no) 为『最大局号』非『局数』，下游切勿以 COUNT 重实现。二、本处 GROUP BY 只含 bet03，与 §R01／§B01／§B01-D／§P11b 之 (shoe_id, table_id) 写法不同；然 §R01-0b 全域实测：pct_cross_table = 0、max_tables_per_shoe = 1、两粒度靴数同为 123,357（delta = 0）、avg max_round 同为 49.7806，§R01-0d 逐行证据零行——故二者在本核验窗口内逐条一致，本轮无跨桌合并证据。此系本窗口本口径之 OBSERVED 事实，不外推为 shoe_id 永久全局唯一之保证，换窗口须重验。三、切勿改用 GROUP BY dt, bet03：该写法会切断跨零点续接之同一物理靴（§R01-0c 实测 3,092 条、pct_contiguous = 100%）。 */
...
-- ──────────────────────────────────────────────────────────────────────────────
--  48. P12b_member_events.csv   [总包 v11.1.0 行 9885~9941 · 输出 8 列]
--     原 ORDER BY：m.uid
--     audit_rn 之排序键与分批作业版第 48 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`uid`, y.`first_dt`, y.`last_dt`, y.`active_days`, y.`span_days`, y.`n_orders`, y.`mark_dt`, y.`censor_flag`) AS audit_rn
FROM (
  ...
  ranked AS (                                                                                         -- 公共表表达式：开启中间结果集 ranked——同注单号取最新版本（§P12b）
      SELECT b.bet01, b.bet05, b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                           -- 取列：起始取列子句，涉 bet05（会员号）、bet18~22（五级代理线）
             b.bet38, b.category, b.dt,                                                               -- 续行：接续取列，涉 bet38（重對）、dt（营业日）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  49. P12c_member_daily_pnl.csv   [总包 v11.1.0 行 9954~10005 · 输出 6 列]
--     原 ORDER BY：g.uid, g.dt
--     audit_rn 之排序键与分批作业版第 49 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`uid`, y.`dt`, y.`day_orders`, y.`day_stake`, y.`day_vb`, y.`day_pnl`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  50. P13_personal_entry.csv   [总包 v11.1.0 行 10145~10204 · 输出 7 列]
--     原 ORDER BY：uid
--     audit_rn 之排序键与分批作业版第 50 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`uid`, y.`n_shoes`, y.`n_orders`, y.`entry_pos_p50`, y.`exit_pos_p50`, y.`bet_density_p50`, y.`entry_round_avg`) AS audit_rn
FROM (
  ...
  shoe_len AS (          /* ★★ 命名与粒度双注（2026-08-23 立；G-5 已据实测纠错）★★ 一、本 CTE 名为 shoe_len（靴长）系历史沿用，所算 max_round = MAX(round_no) 为『最大局号』非『局数』。二、本处 GROUP BY 只含 bet03，与 §R01／§B01／§B01-D／§P11b 之 (shoe_id, table_id) 写法不同；然 §R01-0b 全域实测：pct_cross_table = 0、max_tables_per_shoe = 1、两粒度靴数同为 123,357（delta = 0）、avg max_round 同为 49.7806，§R01-0d 逐行证据零行——故二者在本核验窗口内逐条一致，本轮无跨桌合并证据。此系本窗口本口径之 OBSERVED 事实，不外推为 shoe_id 永久全局唯一之保证，换窗口须重验。三、切勿改用 GROUP BY dt, bet03：该写法会切断跨零点续接之同一物理靴（§R01-0c 实测 3,092 条、pct_contiguous = 100%）。 */
...
-- ──────────────────────────────────────────────────────────────────────────────
--  51. PH0_8r1_rg_case.csv   [总包 v11.1.0 行 13308~13322 · 输出 8 列]
--     原 ORDER BY：dt, lmc08, lmc02
--     audit_rn 之排序键与分批作业版第 51 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`dt_day`, y.`category`, y.`target_id`, y.`operator_id`, y.`change_time`, y.`operator_ip`, y.`content_full`, y.`sql_full`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
    dt                                                                     AS dt_day,                 -- 取列：分区日
...
-- ──────────────────────────────────────────────────────────────────────────────
--  52. PH0_8r2_rg_crosstable.csv   [总包 v11.1.0 行 13334~13390 · 输出 5 列]  [🚩第 [1, 2, 3, 4, 5] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
...
-- ──────────────────────────────────────────────────────────────────────────────
--  53. PI01_treatment_delta_member.csv   [总包 v11.1.0 行 13468~13599 · 输出 21 列]
--     原 ORDER BY：d_revenue, member_id
--     audit_rn 之排序键与分批作业版第 53 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`d_revenue`, y.`member_id`, y.`treat_time`, y.`treat_field`, y.`n_treat_events`, y.`n_pre`, y.`n_post`, y.`d_pre`, y.`d_post`, y.`stake_pre`, y.`stake_post`, y.`d_stake`, y.`d_validbet`, y.`hold_pre_pct`, y.`hold_post_pct`, y.`d_hold_pct`, y.`roi_pre`, y.`roi_post`, y.`d_roi`, y.`d_bets_per_day`, y.`verdict_exposure`) AS audit_rn
FROM (
  WITH ev AS (                                                                                        -- 公共表表达式：开启中间结果集 ev——处置日志原始事件
    SELECT CAST(lmc02 AS STRING)                                           AS member_id,              -- 取值表达式：取用 lmc02（会员编号）
           CAST(lmc08 AS STRING)                                           AS action_time,            -- 取值表达式：★ 取用 lmc08（操作时点·实证列义，见 §TL-10）
           CAST(lmc05 AS STRING)                                           AS content                 -- 取值表达式：★ 取用 lmc05（变更内容·实证列义，见 §TL-10）
    FROM ods_mariadb_2b.ods_a168_log_mem_change                                                       -- 取数来源：取自会员变更日志
    WHERE dt >= '2026-03-21' AND dt < '2026-08-07'                                                    -- 过滤条件：限定分析窗
      AND CAST(lmc04 AS STRING) IN ('edit', 'changestatus')                                           -- 并列条件：★ 只留二类，剔 add（§TL-10 实测百家乐零命中）
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  seg AS (                                                                                            -- 公共表表达式：开启中间结果集 seg——内容逐段展开
    SELECT e.member_id, e.action_time, TRIM(s.piece)                       AS piece                   -- 取列：会员、时点、逐段去空白
    FROM ev e, unnest(split(e.content, ';')) AS s(piece)                                              -- 行展开：以分号切段（§TL-11 同法）
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  norm AS (                                                                                           -- 公共表表达式：开启中间结果集 norm——冒号式与空格式并收
    SELECT member_id, action_time,                                                                    -- 取列：会员、时点
           CASE WHEN piece LIKE '%:%'
                THEN TRIM(SPLIT_PART(SPLIT_PART(piece, ':', 1), ' ', 1))
                ELSE TRIM(SPLIT_PART(piece, ' ', 1)) END                   AS field_name,             -- 条件取值：★ 两式并收（§TL-11 载空格式 815 条曾被静默丢弃）
           CASE WHEN piece LIKE '%:%'
                THEN TRIM(SPLIT_PART(SPLIT_PART(piece, ':', 2), '=>', 1))
                ELSE TRIM(SPLIT_PART(SPLIT_PART(piece, ' ', 2), '=>', 1)) END AS value_before,        -- 条件取值：旧值
           TRIM(SPLIT_PART(piece, '=>', 2))                                AS value_after             -- 取值表达式：新值一律取箭头之后
    FROM seg                                                                                          -- 取数来源：取自本条自建的中间结果集 seg
    WHERE piece LIKE '%=>%'                                                                           -- 过滤条件：须含箭头方为跳变
      AND LENGTH(TRIM(SPLIT_PART(piece, '=>', 1))) BETWEEN 2 AND 20                                   -- 并列条件：★ 护栏——列名须短（§TL-10 载 URL 冒号曾骗过解析）
      AND piece NOT LIKE '%//%'                                                                       -- 并列条件：护栏——排除 URL
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  tr AS (                                                                                             -- 公共表表达式：开启中间结果集 tr——每会员之首次收紧处置
    SELECT member_id                                                       AS mid,                    -- 取列：会员编号
           MIN(CAST(action_time AS DATETIME))                              AS t_treat,                -- 聚合取值：★ 首次处置时点——前后窗之切点
           COUNT(*)                                                        AS n_events,               -- 计数表达式：该会员之跳变数
           MIN(field_name)                                                 AS first_field              -- 聚合取值：首个受改字段（供追溯）
    FROM norm                                                                                         -- 取数来源：取自本条自建的中间结果集 norm
    WHERE field_name IN ('mem016', 'mem017', 'mem015', 'mem020')                                      -- 过滤条件：★ 四列系风控处置（§TL-09 列义已译）
      AND UPPER(TRIM(value_after)) IN ('N', '0')                                                      -- 并列条件：★ 只取收紧向（停用／禁投），非解除向
    GROUP BY member_id                                                                                -- 分组：按会员汇总
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  ta AS (                                                                                             -- 公共表表达式：开启中间结果集 ta——公司测试线代理
    SELECT DISTINCT age001 AS aid FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'               -- 取列与过滤：铁律④b——公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  rk AS (                                                                                             -- 公共表表达式：开启中间结果集 rk——同注单号取最新版本
    SELECT b.bet05, b.bet09, b.bet13, b.bet14, b.bet11, b.validbet, b.bet39,                          -- 取列：会员、注型、本金、派彩、汇率、洗码、桌号
           b.bet08, b.bet06, b.dt, b.category, b.bet38, b.commission,                                 -- 续行：下注时间、开局时间、营业日、类别、重对、佣金制
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：五级代理位
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base——过基础闸之母体
    SELECT TRIM(r.bet05)                                                   AS mid,                    -- 取列：会员编号
           ...
           TRIM(r.bet39)                                                   AS tbl,                    -- 取列：★ 桌号取 bet39（bet04 系子场次，§DICT-01b 已证）
           TRIM(r.commission)                                              AS comm,                   -- 取列：佣金制标记（★ 实测 8 值，语义 UNKNOWN）
           r.dt                                                            AS biz_date,               -- 取列：营业日
           CAST(r.bet08 AS DATETIME)                                       AS t_bet,                  -- 取值表达式：下注时点
           CAST(r.bet06 AS DATETIME)                                       AS t_open,                 -- 取值表达式：开局时点
           CAST(NULLIF(TRIM(r.bet13), '') AS DECIMAL(20,8))
             / NULLIF(CAST(NULLIF(TRIM(r.bet11), '') AS DECIMAL(20,8)), 0) AS stake,                  -- 取值表达式：本金归一
           CAST(NULLIF(TRIM(r.validbet), '') AS DECIMAL(20,8))
             / NULLIF(CAST(NULLIF(TRIM(r.bet11), '') AS DECIMAL(20,8)), 0) AS vbet,                   -- 取值表达式：有效投注归一
           (CAST(NULLIF(TRIM(r.bet13), '') AS DECIMAL(20,8))
          - CAST(NULLIF(TRIM(r.bet14), '') AS DECIMAL(20,8)))
             / NULLIF(CAST(NULLIF(TRIM(r.bet11), '') AS DECIMAL(20,8)), 0) AS ggr                     -- 取值表达式：平台毛利（本金减派彩）
    FROM rk r                                                                                         -- 取数来源：取自本条自建的中间结果集 rk
    LEFT JOIN ta t1 ON t1.aid = r.bet18                                                               -- 左连接：一级代理是否测试线
    LEFT JOIN ta t2 ON t2.aid = r.bet19                                                               -- 左连接：二级代理是否测试线
    LEFT JOIN ta t3 ON t3.aid = r.bet20                                                               -- 左连接：三级代理是否测试线
    ...
    WHERE r.rn = 1                                                                                    -- 过滤条件：基础闸①——最新版本
      AND r.category = '1'                                                                            -- 并列条件：基础闸②——一般注单（§QSB-01 已证 cat2 系小费）
  ...
  tw AS (                                                                                             -- 公共表表达式：开启中间结果集 tw——按处置时点切前后窗
    SELECT b.mid,                                                                                     -- 取列：会员编号
           CASE WHEN b.t_bet < t.t_treat THEN 'PRE' ELSE 'POST' END        AS phase,                  -- 条件取值：★ 以首次处置时点切前后
           b.stake, b.vbet, b.ggr, b.biz_date                                                         -- 续行：本金、有效投注、毛利、营业日
    FROM base b                                                                                       -- 取数来源：取自本条自建的中间结果集 base
    JOIN tr t ON t.mid = b.mid                                                                        -- 连接：只取受处置之会员
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  agg AS (                                                                                            -- 公共表表达式：开启中间结果集 agg——前后窗各自汇总
    SELECT mid,                                                                                       -- 取列：会员编号
           SUM(CASE WHEN phase = 'PRE'  THEN stake ELSE 0 END)             AS stake_pre,              -- 条件汇总：处置前本金
           SUM(CASE WHEN phase = 'POST' THEN stake ELSE 0 END)             AS stake_post,             -- 条件汇总：处置后本金
           SUM(CASE WHEN phase = 'PRE'  THEN vbet  ELSE 0 END)             AS vbet_pre,               -- 条件汇总：处置前有效投注
           SUM(CASE WHEN phase = 'POST' THEN vbet  ELSE 0 END)             AS vbet_post,              -- 条件汇总：处置后有效投注
           SUM(CASE WHEN phase = 'PRE'  THEN ggr   ELSE 0 END)             AS ggr_pre,                -- 条件汇总：处置前平台毛利
           SUM(CASE WHEN phase = 'POST' THEN ggr   ELSE 0 END)             AS ggr_post,               -- 条件汇总：处置后平台毛利
           SUM(CASE WHEN phase = 'PRE'  THEN 1 ELSE 0 END)                 AS n_pre,                  -- 条件计数：处置前注数
           SUM(CASE WHEN phase = 'POST' THEN 1 ELSE 0 END)                 AS n_post,                 -- 条件计数：处置后注数
           COUNT(DISTINCT CASE WHEN phase = 'PRE'  THEN biz_date END)      AS d_pre,                  -- 条件去重计数：处置前活跃天数
           COUNT(DISTINCT CASE WHEN phase = 'POST' THEN biz_date END)      AS d_post                  -- 条件去重计数：处置后活跃天数
    FROM tw                                                                                           -- 取数来源：取自本条自建的中间结果集 tw
    GROUP BY mid                                                                                      -- 分组：按会员汇总
  )                                                                                                   -- 续行：收束上方的子查询或函数括号
  SELECT                                                                                              -- 取列：起始取列子句
    a.mid                                                                  AS member_id,              -- 取列：会员编号
    t.t_treat                                                              AS treat_time,             -- 取列：★ 处置时点（可回溯 §TL-11 之 episode）
    t.first_field                                                          AS treat_field,            -- 取列：首个受改字段
    t.n_events                                                             AS n_treat_events,         -- 取列：该会员之跳变数
    a.n_pre, a.n_post, a.d_pre, a.d_post,                                                             -- 续行：前后注数与活跃天数
    ROUND(a.stake_pre, 2)                                                  AS stake_pre,              -- 取值表达式：处置前本金
    ROUND(a.stake_post, 2)                                                 AS stake_post,             -- 取值表达式：处置后本金
    ROUND(a.stake_post - a.stake_pre, 2)                                   AS d_stake,                -- 加减计算：★ ΔStake
    ROUND(a.vbet_post - a.vbet_pre, 2)                                     AS d_validbet,             -- 加减计算：★ ΔValidBet
    ROUND(a.ggr_post - a.ggr_pre, 2)                                       AS d_revenue,              -- 加减计算：★ ΔRevenue（平台毛利增减）
    ROUND(a.ggr_pre  * 100.0 / NULLIF(a.vbet_pre, 0), 6)                   AS hold_pre_pct,           -- 比率表达式：处置前 hold%
    ROUND(a.ggr_post * 100.0 / NULLIF(a.vbet_post, 0), 6)                  AS hold_post_pct,          -- 比率表达式：处置后 hold%
    ROUND(a.ggr_post * 100.0 / NULLIF(a.vbet_post, 0)
        - a.ggr_pre  * 100.0 / NULLIF(a.vbet_pre, 0), 6)                   AS d_hold_pct,             -- 加减计算：★ ΔHold%
    ROUND(-1 * a.ggr_pre  / NULLIF(a.stake_pre, 0), 6)                     AS roi_pre,                -- 比率表达式：处置前会员 ROI
    ROUND(-1 * a.ggr_post / NULLIF(a.stake_post, 0), 6)                    AS roi_post,               -- 比率表达式：处置后会员 ROI
    ROUND(-1 * a.ggr_post / NULLIF(a.stake_post, 0)
        + 1 * a.ggr_pre  / NULLIF(a.stake_pre, 0), 6)                      AS d_roi,                  -- 加减计算：★ ΔROI
    ROUND(a.n_post * 1.0 / NULLIF(a.d_post, 0)
        - a.n_pre  * 1.0 / NULLIF(a.d_pre, 0), 4)                          AS d_bets_per_day,         -- 加减计算：★ Δ日均注数
    CASE                                                                                              -- 条件分支：起始分支——暴露态裁定
      WHEN a.n_post = 0
        THEN 'NO_EXPOSURE —— 处置后完全未下注（★ 禁写 ΔROI = 0）'                                       -- 分支判定：零暴露
      WHEN a.n_pre = 0
        THEN 'NO_BASELINE —— 处置前无注单，无从比对'                                                    -- 分支判定：无基线
      WHEN a.ggr_post - a.ggr_pre < 0
        THEN '★ 平台毛利下降 —— 处置后少赚（须并读 ΔStake 判其系收敛抑或流失）'                             -- 分支判定：毛利降
      ELSE '◐ 平台毛利上升或持平'                                                                       -- 分支判定：毛利升
    END                                                                    AS verdict_exposure        -- 条件分支：收束上方判词，产出「verdict_exposure」
  FROM agg a                                                                                          -- 取数来源：取自本条自建的中间结果集 agg
  JOIN tr t ON t.mid = a.mid                                                                          -- 连接：接回处置台账取其时点与字段
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  54. PI02_treatment_delta_platform.csv   [总包 v11.1.0 行 13612~13713 · 输出 13 列]
--     原 ORDER BY：cohort, phase
--     audit_rn 之排序键与分批作业版第 54 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`cohort`, y.`phase`, y.`n_members`, y.`n_bets`, y.`n_dates`, y.`sum_stake`, y.`sum_validbet`, y.`sum_revenue`, y.`hold_pct`, y.`roi`, y.`revenue_per_member`, y.`share_of_stake_pct`, y.`share_of_revenue_pct`) AS audit_rn
FROM (
  WITH ev AS (                                                                                        -- 公共表表达式：开启中间结果集 ev——处置日志原始事件
    SELECT CAST(lmc02 AS STRING)                                           AS member_id,              -- 取值表达式：取用 lmc02（会员编号）
           CAST(lmc08 AS STRING)                                           AS action_time,            -- 取值表达式：★ 取用 lmc08（操作时点·实证列义，见 §TL-10）
  ...
  lab AS (                                                                                            -- 公共表表达式：开启中间结果集 lab——标注受处置与否
    SELECT b.mid, b.stake, b.vbet, b.ggr, b.biz_date,                                                 -- 取列：会员、本金、有效投注、毛利、营业日
           CASE WHEN t.mid IS NULL THEN 'UNTREATED' ELSE 'TREATED' END     AS grp,                    -- 条件取值：受处置与否
           CASE WHEN t.mid IS NULL THEN 'NA'
                WHEN b.t_bet < t.t_treat THEN 'PRE' ELSE 'POST' END        AS phase                   -- 条件取值：前后窗
    FROM base b                                                                                       -- 取数来源：取自本条自建的中间结果集 base
    LEFT JOIN tr t ON t.mid = b.mid                                                                   -- 左连接：处置台账（未命中即未处置）
  )                                                                                                   -- 续行：收束上方的子查询或函数括号
  SELECT                                                                                              -- 取列：起始取列子句
    grp                                                                    AS cohort,                 -- 取列：分组
    phase                                                                  AS phase,                  -- 取列：前后窗
    COUNT(DISTINCT mid)                                                    AS n_members,              -- 计数表达式：会员数
    COUNT(*)                                                               AS n_bets,                 -- 计数表达式：注数
    COUNT(DISTINCT biz_date)                                               AS n_dates,                -- 计数表达式：涉及天数
    ROUND(SUM(stake), 2)                                                   AS sum_stake,              -- 汇总表达式：本金总额
    ROUND(SUM(vbet), 2)                                                    AS sum_validbet,           -- 汇总表达式：有效投注总额
    ROUND(SUM(ggr), 2)                                                     AS sum_revenue,            -- 汇总表达式：★ 平台毛利总额
    ROUND(SUM(ggr) * 100.0 / NULLIF(SUM(vbet), 0), 6)                      AS hold_pct,               -- 比率表达式：hold%
    ROUND(-1 * SUM(ggr) / NULLIF(SUM(stake), 0), 6)                        AS roi,                    -- 比率表达式：会员侧 ROI
    ROUND(SUM(ggr) / NULLIF(COUNT(DISTINCT mid), 0), 2)                    AS revenue_per_member,     -- 比率表达式：★ 人均贡献平台毛利
    ROUND(SUM(stake) * 100.0 / SUM(SUM(stake)) OVER (), 6)                 AS share_of_stake_pct,     -- 比率表达式：★ 占全平台本金之比
    ROUND(SUM(ggr) * 100.0 / NULLIF(SUM(SUM(ggr)) OVER (), 0), 6)          AS share_of_revenue_pct    -- 比率表达式：★ 占全平台毛利之比
  FROM lab                                                                                            -- 取数来源：取自本条自建的中间结果集 lab
  GROUP BY grp, phase                                                                                 -- 分组：按组别与前后窗汇总
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  55. PI03_reinvest_loop.csv   [总包 v11.1.0 行 13728~13791 · 输出 13 列]
--     原 ORDER BY：n_pairs DESC, prev_outcome
--     audit_rn 之排序键与分批作业版第 55 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_pairs` DESC, y.`prev_outcome`, y.`n_members`, y.`n_negative_gap`, y.`gap_p10`, y.`gap_p50`, y.`gap_p90`, y.`gap_avg`, y.`ratio_p50`, y.`ratio_p90`, y.`ratio_avg`, y.`n_double_up`, y.`pct_double_up`) AS audit_rn
FROM (
  WITH ta AS (                                                                                        -- 公共表表达式：开启中间结果集 ta——公司测试线代理
    SELECT DISTINCT age001 AS aid FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'               -- 取列与过滤：铁律④b——公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  rk AS (                                                                                             -- 公共表表达式：开启中间结果集 rk——同注单号取最新版本
    SELECT b.bet05, b.bet13, b.bet14, b.bet11, b.bet08, b.bet39, b.category, b.bet38,                 -- 取列：会员、本金、派彩、汇率、下注时点、桌号、类别、重对
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：五级代理位
           ROW_NUMBER() OVER (PARTITION BY b.bet01
             ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC)      AS rn                      -- 行号窗口表达式：三级排序取最新
    FROM ods_mariadb_2b.ods_a168_bet02 b                                                              -- 取数来源：取自注单主表
    WHERE b.dt >= '2026-03-21' AND b.dt < '2026-08-07'                                                -- 过滤条件：限定分析窗
      AND CAST(b.bet02 AS STRING) = '101'                                                             -- 并列条件：限定百家乐
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base——过基础闸之母体
    SELECT TRIM(r.bet05)                                                   AS mid,                    -- 取列：会员编号
           ...
           (CAST(NULLIF(TRIM(r.bet13), '') AS DECIMAL(20,8))
          - CAST(NULLIF(TRIM(r.bet14), '') AS DECIMAL(20,8)))
             / NULLIF(CAST(NULLIF(TRIM(r.bet11), '') AS DECIMAL(20,8)), 0) AS ggr                     -- 取值表达式：平台毛利
    FROM rk r                                                                                         -- 取数来源：取自本条自建的中间结果集 rk
    LEFT JOIN ta t1 ON t1.aid = r.bet18                                                               -- 左连接：一级代理是否测试线
    LEFT JOIN ta t2 ON t2.aid = r.bet19                                                               -- 左连接：二级代理是否测试线
    LEFT JOIN ta t3 ON t3.aid = r.bet20                                                               -- 左连接：三级代理是否测试线
    LEFT JOIN ta t4 ON t4.aid = r.bet21                                                               -- 左连接：四级代理是否测试线
    LEFT JOIN ta t5 ON t5.aid = r.bet22                                                               -- 左连接：五级代理是否测试线
    WHERE r.rn = 1 AND r.category = '1' AND UPPER(TRIM(r.bet38)) = 'N'                                -- 过滤条件：基础闸①②③
      AND COALESCE(t1.aid, t2.aid, t3.aid, t4.aid, t5.aid) IS NULL                                    -- 并列条件：基础闸④——非测试线
      AND CAST(NULLIF(TRIM(r.bet11), '') AS DECIMAL(20,8)) > 0                                        -- 并列条件：基础闸⑤——汇率为正
      AND CAST(NULLIF(TRIM(r.bet13), '') AS DECIMAL(20,8)) > 0                                        -- 并列条件：基础闸⑥——本金为正
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  seq AS (                                                                                            -- 公共表表达式：开启中间结果集 seq——按会员排下注序
    SELECT mid, t_bet, stake, ggr,                                                                    -- 取列：会员、时点、本金、毛利
           LEAD(t_bet) OVER (PARTITION BY mid ORDER BY t_bet)              AS t_next,                 -- 前瞻窗口表达式：同会员之下一注时点
           LEAD(stake) OVER (PARTITION BY mid ORDER BY t_bet)              AS stake_next,             -- 前瞻窗口表达式：同会员之下一注注额
           CASE WHEN ggr > 0 THEN 'LOSS'
                WHEN ggr < 0 THEN 'WIN' ELSE 'PUSH' END                    AS outcome                 -- 条件取值：★ 本注结果三态（会员视角）
    FROM base                                                                                         -- 取数来源：取自本条自建的中间结果集 base
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  gp AS (                                                                                             -- 公共表表达式：开启中间结果集 gp——计间隔与加码比
    SELECT mid, outcome,                                                                              -- 取列：会员、本注结果
           TIMESTAMPDIFF(SECOND, t_bet, t_next)                            AS gap_sec,                -- 时间差表达式：★ 至下一注之间隔（秒）
           stake_next / NULLIF(stake, 0)                                   AS stake_ratio             -- 除法计算：★ 下一注注额倍数
    FROM seq                                                                                          -- 取数来源：取自本条自建的中间结果集 seq
    WHERE t_next IS NOT NULL                                                                          -- 过滤条件：末注无下一注，剔除
  )                                                                                                   -- 续行：收束上方的子查询或函数括号
  SELECT                                                                                              -- 取列：起始取列子句
    outcome                                                                AS prev_outcome,           -- 取列：★ 前一注之结果
    COUNT(*)                                                               AS n_pairs,                -- 计数表达式：相邻注对数
    COUNT(DISTINCT mid)                                                    AS n_members,              -- 计数表达式：涉及会员数
    SUM(CASE WHEN gap_sec < 0 THEN 1 ELSE 0 END)                           AS n_negative_gap,         -- 条件计数：★ 负间隔——保留并查
    ROUND(PERCENTILE_APPROX(gap_sec, 0.10), 4)                             AS gap_p10,                -- 分位表达式：间隔 10% 分位
    ROUND(PERCENTILE_APPROX(gap_sec, 0.50), 4)                             AS gap_p50,                -- 分位表达式：★ 间隔中位——三态并读即见差异
    ROUND(PERCENTILE_APPROX(gap_sec, 0.90), 4)                             AS gap_p90,                -- 分位表达式：间隔 90% 分位
    ROUND(AVG(gap_sec), 4)                                                 AS gap_avg,                -- 聚合表达式：间隔均值
    ROUND(PERCENTILE_APPROX(stake_ratio, 0.50), 6)                         AS ratio_p50,              -- 分位表达式：★ 加码比中位——>1 即亏后加码
    ROUND(PERCENTILE_APPROX(stake_ratio, 0.90), 6)                         AS ratio_p90,              -- 分位表达式：加码比 90% 分位
    ROUND(AVG(stake_ratio), 6)                                             AS ratio_avg,              -- 聚合表达式：加码比均值
    SUM(CASE WHEN stake_ratio >= 2 THEN 1 ELSE 0 END)                      AS n_double_up,            -- 条件计数：★ 下一注加倍及以上者
    ROUND(SUM(CASE WHEN stake_ratio >= 2 THEN 1 ELSE 0 END) * 100.0
          / NULLIF(COUNT(*), 0), 6)                                        AS pct_double_up           -- 比率表达式：加倍率
  FROM gp                                                                                             -- 取数来源：取自本条自建的中间结果集 gp
  GROUP BY outcome                                                                                    -- 分组：按前一注结果汇总
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  56. PI04_member_5d_profile.csv   [总包 v11.1.0 行 13808~13877 · 输出 20 列]
--     原 ORDER BY：sum_revenue DESC, member_id
--     audit_rn 之排序键与分批作业版第 56 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`sum_revenue` DESC, y.`member_id`, y.`n_bets`, y.`n_active_days`, y.`bets_per_day`, y.`n_bet_types`, y.`sum_stake`, y.`sum_validbet`, y.`hold_pct`, y.`roi`, y.`n_tables`, y.`n_comm_mode`, y.`first_date`, y.`last_date`, y.`span_days`, y.`night_share_pct`, y.`avg_latency_sec`, y.`share_stake_pct`, y.`share_revenue_pct`, y.`verdict_observability`) AS audit_rn
FROM (
  WITH ta AS (                                                                                        -- 公共表表达式：开启中间结果集 ta——公司测试线代理
    SELECT DISTINCT age001 AS aid FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'               -- 取列与过滤：铁律④b——公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
  rk AS (                                                                                             -- 公共表表达式：开启中间结果集 rk——同注单号取最新版本
    SELECT b.bet05, b.bet09, b.bet13, b.bet14, b.bet11, b.validbet, b.bet39, b.commission,            -- 取列：会员、注型、本金、派彩、汇率、洗码、桌号、佣金制
           b.bet08, b.bet06, b.dt, b.category, b.bet38,                                               -- 续行：下注时点、开局时点、营业日、类别、重对
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：五级代理位
           ROW_NUMBER() OVER (PARTITION BY b.bet01
             ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC)      AS rn                      -- 行号窗口表达式：三级排序取最新
    FROM ods_mariadb_2b.ods_a168_bet02 b                                                              -- 取数来源：取自注单主表
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base——过基础闸之母体
    SELECT TRIM(r.bet05)                                                   AS mid,                    -- 取列：会员编号
           TRIM(r.bet09)                                                   AS g9,                     -- 取列：注型
           TRIM(r.bet39)                                                   AS tbl,                    -- 取列：★ 桌号取 bet39
           TRIM(r.commission)                                              AS comm,                   -- 取列：佣金制标记
           r.dt                                                            AS biz_date,               -- 取列：营业日
           CAST(r.bet08 AS DATETIME)                                       AS t_bet,                  -- 取值表达式：下注时点
           CAST(r.bet06 AS DATETIME)                                       AS t_open,                 -- 取值表达式：开局时点
           CAST(NULLIF(TRIM(r.bet13), '') AS DECIMAL(20,8))
    ...
    LEFT JOIN ta t5 ON t5.aid = r.bet22                                                               -- 左连接：五级代理是否测试线
    WHERE r.rn = 1 AND r.category = '1' AND UPPER(TRIM(r.bet38)) = 'N'                                -- 过滤条件：基础闸①②③
      AND COALESCE(t1.aid, t2.aid, t3.aid, t4.aid, t5.aid) IS NULL                                    -- 并列条件：基础闸④
      AND CAST(NULLIF(TRIM(r.bet11), '') AS DECIMAL(20,8)) > 0                                        -- 并列条件：基础闸⑤
      AND CAST(NULLIF(TRIM(r.bet13), '') AS DECIMAL(20,8)) > 0                                        -- 并列条件：基础闸⑥
  )                                                                                                   -- 续行：收束上方的子查询或函数括号
  SELECT                                                                                              -- 取列：起始取列子句
    mid                                                                    AS member_id,              -- 取列：会员编号
    COUNT(*)                                                               AS n_bets,                 -- 计数表达式：〔B〕注数
    COUNT(DISTINCT biz_date)                                               AS n_active_days,          -- 计数表达式：〔B〕活跃天数
    ROUND(COUNT(*) * 1.0 / NULLIF(COUNT(DISTINCT biz_date), 0), 4)         AS bets_per_day,           -- 比率表达式：〔B〕日均注数
    COUNT(DISTINCT g9)                                                     AS n_bet_types,            -- 计数表达式：〔B〕所押注型数
    ROUND(SUM(stake), 2)                                                   AS sum_stake,              -- 汇总表达式：〔E〕本金
    ROUND(SUM(vbet), 2)                                                    AS sum_validbet,           -- 汇总表达式：〔E〕有效投注
    ROUND(SUM(ggr), 2)                                                     AS sum_revenue,            -- 汇总表达式：〔E〕★ 贡献之平台毛利
    ROUND(SUM(ggr) * 100.0 / NULLIF(SUM(vbet), 0), 6)                      AS hold_pct,               -- 比率表达式：〔E〕hold%
    ROUND(-1 * SUM(ggr) / NULLIF(SUM(stake), 0), 6)                        AS roi,                    -- 比率表达式：〔E〕会员 ROI
    COUNT(DISTINCT tbl)                                                    AS n_tables,               -- 计数表达式：〔N〕★ 所涉桌数
    COUNT(DISTINCT comm)                                                   AS n_comm_mode,            -- 计数表达式：〔N〕所涉佣金制数
    MIN(biz_date)                                                          AS first_date,             -- 聚合取值：〔T〕首注日
    MAX(biz_date)                                                          AS last_date,              -- 聚合取值：〔T〕末注日
    DATEDIFF(MAX(biz_date), MIN(biz_date)) + 1                             AS span_days,              -- 时间差表达式：〔T〕★ 观测跨度（非窗长）
    ROUND(SUM(CASE WHEN HOUR(t_bet) < 6 THEN 1 ELSE 0 END) * 100.0
          / NULLIF(COUNT(*), 0), 6)                                        AS night_share_pct,        -- 比率表达式：〔T〕夜间注占比（0–6 时）
    ROUND(AVG(TIMESTAMPDIFF(SECOND, t_open, t_bet)), 4)                    AS avg_latency_sec,        -- 聚合表达式：〔T〕★ 下注延迟均值
    ROUND(SUM(stake) * 100.0 / SUM(SUM(stake)) OVER (), 8)                 AS share_stake_pct,        -- 比率表达式：〔I〕★ 占全平台本金之比
    ROUND(SUM(ggr) * 100.0 / NULLIF(SUM(SUM(ggr)) OVER (), 0), 8)          AS share_revenue_pct,      -- 比率表达式：〔I〕★ 占全平台毛利之比
    CASE                                                                                              -- 条件分支：起始分支——可观测性裁定
      WHEN MIN(biz_date) = '2026-03-21'
        THEN 'LEFT_CENSORED —— 首注即窗首，真实起点在窗外（禁解为新会员）'                                 -- 分支判定：左截断
      WHEN DATEDIFF(MAX(biz_date), MIN(biz_date)) + 1 < 7
        THEN 'SHORT_SPAN —— 跨度不足七日，指标不稳'                                                     -- 分支判定：跨度过短
      ELSE 'OBSERVABLE'                                                                               -- 分支判定：可观测
    END                                                                    AS verdict_observability   -- 条件分支：收束上方判词，产出「verdict_observability」
  FROM base                                                                                           -- 取数来源：取自本条自建的中间结果集 base
  GROUP BY mid                                                                                        -- 分组：按会员汇总
) y
ORDER BY audit_rn;
...
-- ──────────────────────────────────────────────────────────────────────────────
--  57. P_player_month.csv   [总包 v11.1.0 行 2865~2921 · 输出 9 列]
--     原 ORDER BY：member_id, ym
--     audit_rn 之排序键与分批作业版第 57 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`ym`, y.`n_rounds`, y.`n_days`, y.`valid_bet`, y.`game_pnl`, y.`rebate`, y.`net_pnl`, y.`roi`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  58. QSB_01a_category_census.csv   [总包 v11.1.0 行 11153~11197 · 输出 22 列]
--     原 ORDER BY：n_rows DESC, category_value
--     audit_rn 之排序键与分批作业版第 58 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rows` DESC, y.`category_value`, y.`pct_of_total`, y.`n_members`, y.`n_dates`, y.`min_dt`, y.`max_dt`, y.`n_bet09`, y.`n_b14_null`, y.`n_b14_zero`, y.`n_b14_pos`, y.`n_pnl_zero`, y.`n_pnl_house_win`, y.`n_pnl_player_win`, y.`n_vb_zero`, y.`n_b41_zero`, y.`stake_sum`, y.`payout_sum`, y.`validbet_sum`, y.`n_rebet_flag`, y.`n_commission`, y.`shape_hint`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  59. QSB_01c_cat1_contamination.csv   [总包 v11.1.0 行 11250~11283 · 输出 10 列]
--     原 ORDER BY：n_b14_null DESC, n_pnl_zero_stake_pos DESC, n_rows DESC
--     audit_rn 之排序键与分批作业版第 59 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_b14_null` DESC, y.`n_pnl_zero_stake_pos` DESC, y.`n_rows` DESC, y.`bet09_value`, y.`n_b13_null`, y.`n_b13_zero`, y.`n_exact_refund`, y.`pct_pnl_zero`, y.`stake_sum`, y.`verdict_contamination`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  60. QSB_02a_lowcard_fields.csv   [总包 v11.1.0 行 11295~11316 · 输出 18 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 60 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope`, y.`n_rows`, y.`nd_bet04`, y.`nd_bet06`, y.`nd_bet07`, y.`nd_bet10`, y.`nd_bet23`, y.`nd_bet24`, y.`nd_bet25`, y.`nd_bet26`, y.`nd_bet28`, y.`nd_bet29`, y.`nd_bet30`, y.`nd_bet31`, y.`nd_bet40`, y.`nd_bet38`, y.`nd_category`, y.`nd_commission`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  61. QSB_02b2_lowcard_five_gates.csv   [总包 v11.1.0 行 11498~11747 · 输出 19 列]
--     原 ORDER BY：field_name, n_rows DESC, field_value
--     audit_rn 之排序键与分批作业版第 61 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`field_name`, y.`n_rows` DESC, y.`field_value`, y.`pct_of_field`, y.`n_null`, y.`n_bet09`, y.`n_dates`, y.`min_dt`, y.`max_dt`, y.`n_pnl_zero`, y.`n_pnl_pos`, y.`n_pnl_neg`, y.`pct_pnl_zero`, y.`pct_pnl_pos`, y.`pct_pnl_neg`, y.`n_exact_refund`, y.`n_payout_zero`, y.`payout_over_stake`, y.`verdict_state`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  62. QSB_02b_state_cross.csv   [总包 v11.1.0 行 11331~11424 · 输出 9 列]
--     原 ORDER BY：field_name, n_rows DESC, field_value
--     audit_rn 之排序键与分批作业版第 62 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`field_name`, y.`n_rows` DESC, y.`field_value`, y.`n_pnl_zero`, y.`n_pnl_pos`, y.`n_pnl_neg`, y.`n_payout_zero`, y.`n_bet09`, y.`verdict_align`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
--  63. QSB_02c_push_by_family.csv   [总包 v11.1.0 行 11437~11489 · 输出 10 列]
--     原 ORDER BY：n_rows DESC, bet09_family
--     audit_rn 之排序键与分批作业版第 63 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rows` DESC, y.`bet09_family`, y.`n_bet09_in_family`, y.`n_pnl_zero`, y.`pct_pnl_zero`, y.`n_exact_refund`, y.`n_payout_zero_stake_pos`, y.`pct_player_win`, y.`mu0_pct`, y.`verdict_family`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
    CASE                                                                                              -- 条件分支：起始分支——注型分族
...
-- ──────────────────────────────────────────────────────────────────────────────
--  64. R01_late_shoe.csv   [总包 v11.1.0 行 3463~3539 · 输出 15 列]
--     原 ORDER BY：late_share DESC, n_orders_late DESC
--     audit_rn 之排序键与分批作业版第 64 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`late_share` DESC, y.`n_orders_late` DESC, y.`uid`, y.`n_orders_all`, y.`late_def`, y.`shoe_pos_p50`, y.`stake_late_avg`, y.`stake_early_avg`, y.`stake_ramp`, y.`roi_late`, y.`roi_early`, y.`stake_late`, y.`game_pnl_late`, y.`stake_all`, y.`game_pnl_all`) AS audit_rn
FROM (
  ...
  ranked AS (                                            -- 需求 §3.2：同单号保留最新版本
    SELECT b.bet01, b.updatetime, b.sync_time, b.dt, b.bet02,                                         -- 取列：起始取列子句，本行先列 b.bet01, b.updatetime, b.sync_time, b.dt, b.bet02，涉 bet02（游戏类别）、dt（营业日）
           ...
           b.bet22, b.bet38, b.bet39, b.category,                                                     -- 续行：接续上一取列子句，续列 b.bet22, b.bet38, b.bet39, b.category，涉 bet22（五级代理线）、bet38（重對）、bet39（桌号）
           ROW_NUMBER() OVER (                                                                        -- 行号窗口表达式：以行号窗口取每组头部或去重，免出重复行
             PARTITION BY b.bet01                                                                     -- 窗口分区：按 b.bet01 分组开窗，组内各自编号或排名
             ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC) AS rn                           -- 排序：按 b.updatetime（降序）, b.sync_time（降序）, b.dt（降序）) AS rn 排列；导出必带排序，否则分页无稳定序（曾致 36.49% 重复行）
    FROM ods_mariadb_2b.ods_a168_bet02 b                 -- ★ 表名开关
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base，其后各行为其定义体（§R01）
    ...
    WHERE r.rn = 1                                                                                    -- 过滤条件：限定 r.rn等于 1
      ...
      AND UPPER(TRIM(r.bet38)) = 'N'                                                                  -- 并列条件：限定 UPPER(TRIM(r.bet38))等于 'N'，涉 bet38（重對）
      AND CAST(NULLIF(TRIM(r.bet05),'') AS BIGINT) > 0                                                -- 并列条件：限定 CAST(NULLIF(TRIM(r.bet05),'') AS BIGINT)大于 0，涉 bet05（会员号）
      AND CAST(NULLIF(TRIM(r.bet11),'') AS DECIMAL(20,8)) > 0                                         -- 并列条件：限定 CAST(NULLIF(TRIM(r.bet11),'') AS DECIMAL(20,8))大于 0，涉 bet11（汇率）
  ...
  SELECT                                                                                              -- 续行：接续上一取列子句，续列 SELECT
    ...
    SUM(CASE WHEN is_late=0 THEN stake END) / NULLIF(COUNT(*)-SUM(is_late),0)     AS stake_early_avg,  -- 汇总表达式：早段注额合计除以早段注单数 ＝ 早段均注，与 GGR 无涉，产出「stake_early_avg」
    (SUM(CASE WHEN is_late=1 THEN stake END) / NULLIF(SUM(is_late),0))                                -- 续行：汇总，取用 stake（下注额（经汇率归一化））
      / NULLIF(SUM(CASE WHEN is_late=0 THEN stake END)                                                -- 除法或乘法计算：汇总，取用 stake（下注额（经汇率归一化））
               / NULLIF(COUNT(*)-SUM(is_late),0), 0)                              AS stake_ramp,      -- 除法计算：晚段均注除以早段均注 ＝ 注额斜率，与 GGR 无涉，产出「stake_ramp」
...
-- ──────────────────────────────────────────────────────────────────────────────
--  65. R01_late_shoe_w2.csv   [总包 v11.1.0 行 10048~10124 · 输出 15 列]
--     原 ORDER BY：late_share DESC, n_orders_late DESC
--     audit_rn 之排序键与分批作业版第 65 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`late_share` DESC, y.`n_orders_late` DESC, y.`uid`, y.`n_orders_all`, y.`late_def`, y.`shoe_pos_p50`, y.`stake_late_avg`, y.`stake_early_avg`, y.`stake_ramp`, y.`roi_late`, y.`roi_early`, y.`stake_late`, y.`game_pnl_late`, y.`stake_all`, y.`game_pnl_all`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  66. R01anm_shoe_anomaly.csv   [总包 v11.1.0 行 3948~4009 · 输出 11 列]
--     原 ORDER BY：s.max_round DESC, s.dt, s.table_id, s.shoe_id
--     audit_rn 之排序键与分批作业版第 66 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`max_round` DESC, y.`dt`, y.`table_id`, y.`shoe_id`, y.`min_round`, y.`rounds_with_bet`, y.`gap`, y.`n_bettors`, y.`n_bets`, y.`anomaly_type`, y.`hi_cut_used`) AS audit_rn
FROM (
  WITH test_agents AS (                                                                               -- 公共表表达式：开启中间结果集 test_agents，其后各行为其定义体（§R01-anm）
    SELECT age001 AS agent_id                                                                         -- 取列：产出「agent_id」
    FROM ods_mariadb_2b.ods_a168_agent                                                                -- 取数来源：取自代理主档（五级代理线归属来源）
    WHERE age022 = '1'                                  -- 铁律④b：公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-anm）
  ranked AS (                                            -- 需求 §3.2：同单号保留最新版本
    SELECT b.bet01, b.dt, b.bet03, b.bet04, b.bet05, b.bet11,                                         -- 取列：起始取列子句，本行先列 b.bet01, b.dt, b.bet03, b.bet04, b.bet05, b.bet11，涉 dt（营业日）、bet03（靴号）、bet04（局内序号）
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：接续上一取列子句，续列 b.bet18~b.bet22，涉五级代理线（测试线剔除之连接键）
           b.bet38, b.bet39, b.category,                                                              -- 续行：接续上一取列子句，续列 b.bet38, b.bet39, b.category，涉 bet38（重對）、bet39（桌号）、category（注单类别）
           ROW_NUMBER() OVER (                                                                        -- 行号窗口表达式：以行号窗口取每组头部或去重，免出重复行
             PARTITION BY b.bet01                                                                     -- 窗口分区：按 b.bet01 分组开窗，组内各自编号或排名
             ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC) AS rn                           -- 排序：按 b.updatetime（降序）, b.sync_time（降序）, b.dt（降序）) AS rn 排列；导出必带排序，否则分页无稳定序（曾致 36.49% 重复行）
    FROM ods_mariadb_2b.ods_a168_bet02 b                 -- ★ 表名开关
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base，其后各行为其定义体（§R01-anm）
    SELECT r.dt                                          AS dt,                                       -- 取列：起始取列子句，本行先取「dt」，涉 dt（营业日）——本族较 §R01 之唯一增列
           CAST(NULLIF(TRIM(r.bet05),'') AS BIGINT)      AS member_id,                                -- 取值表达式：先去空白、空串归 NULL，再显式转型——本库字段多为 varchar，不转型即比较失真，产出「member_id」
           COALESCE(NULLIF(TRIM(r.bet03),''), '(空)')    AS shoe_id,                                  -- 取值表达式：空串归哨兵「(空)」以免与真靴号混算，涉 bet03（靴号），产出「shoe_id」
           COALESCE(NULLIF(TRIM(r.bet39),''), '(空)')    AS table_id,                                 -- 取值表达式：空串归哨兵「(空)」以免与真桌号混算，涉 bet39（桌号），产出「table_id」
           CAST(NULLIF(TRIM(r.bet04),'') AS INT)         AS round_no                                  -- 取值表达式：先去空白、空串归 NULL，再显式转型——本库字段多为 varchar，不转型即比较失真，产出「round_no」
    FROM ranked r                                                                                     -- 取数来源：取自本条自建的中间结果集 ranked
    LEFT JOIN test_agents t1 ON t1.agent_id = r.bet18                                                 -- 左连接：取自本条自建的中间结果集 test_agents，连接键为 bet18（一级代理线）
    LEFT JOIN test_agents t2 ON t2.agent_id = r.bet19                                                 -- 左连接：取自本条自建的中间结果集 test_agents，连接键为 bet19（二级代理线）
  ...
  shoe_new AS (          /* ★ 新粒度（(dt, table_id, shoe_id)）：所算 max_round = MAX(round_no) 为最大局号，非局数；不沿用 shoe_len 之名，见卷首命名警示横幅。★ */
    SELECT dt, table_id, shoe_id,                                                                     -- 取列：起始取列子句，本行先列 dt, table_id, shoe_id，涉 dt（营业日）、bet39（桌号）、bet03（靴号）
           MAX(round_no)                                        AS max_round,                         -- 取最大局号（≠COUNT 局数），产出「max_round」
           MIN(round_no)                                        AS min_round,                         -- 聚合表达式：最小局号——不为 1 者示首局无人下注，产出「min_round」
           COUNT(DISTINCT round_no)                             AS rounds_with_bet,                   -- 计数表达式：局内序号的去重个数——有注局数，产出「rounds_with_bet」
           MAX(round_no) - COUNT(DISTINCT round_no)             AS gap,                               -- 取值表达式：最大局号减有注局数——内部缺口，非尾部截断，产出「gap」
           COUNT(DISTINCT member_id)                            AS n_bettors,                         -- 计数表达式：会员号的去重个数，产出「n_bettors」
           COUNT(*)                                             AS n_bets                             -- 计数表达式：注单数，产出「n_bets」
    FROM base                                                                                         -- 取数来源：取自本条自建的中间结果集 base
    GROUP BY dt, table_id, shoe_id                                                                    -- 分组：按 dt, table_id, shoe_id 汇总——新粒度
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-anm）
  cut AS (               /* 阈值由数据反解，禁硬写——本包不设普适门槛（见卷首常数集中登记之纪律）。 */
    SELECT PERCENTILE_APPROX(max_round, 0.99) * 1.5 AS hi_cut                                         -- 取近似分位数表达式：全域九九分位之 1.5 倍——超长靴之切点，产出「hi_cut」
    FROM shoe_new                                                                                     -- 取数来源：取自本条自建的中间结果集 shoe_new
  )                                                                                                   -- 续行：收束上方的子查询或函数括号（§R01-anm）
  SELECT s.dt, s.table_id, s.shoe_id,                                                                 -- 取列：起始取列子句，本行先列 s.dt, s.table_id, s.shoe_id，涉 dt（营业日）、bet39（桌号）、bet03（靴号）
         s.max_round, s.min_round, s.rounds_with_bet, s.gap,                                          -- 续行：接续上一取列子句，续列最大局号、最小局号、有注局数、内部缺口
         s.n_bettors, s.n_bets,                                                                       -- 续行：接续上一取列子句，续列下注会员数、注单数
         CASE WHEN s.max_round > c.hi_cut THEN '超长：碰撞或跨日合并可疑'                              -- 条件分支：起始分支——超长者列为碰撞候选
              WHEN s.max_round <= 5       THEN '残靴：截断／换荷官／开收台可疑'                        -- 分支判定：残靴者列为截断候选
              ELSE '—' END                             AS anomaly_type,                               -- 条件分支：收束上方判词，产出「anomaly_type」
         ROUND(c.hi_cut, 2)                            AS hi_cut_used                                 -- 取值表达式：本次实际所用之上界切点（随数据现算），产出「hi_cut_used」
  FROM shoe_new s                                                                                     -- 取数来源：取自本条自建的中间结果集 shoe_new
  CROSS JOIN cut c                                                                                    -- 连接：取自本条自建的中间结果集 cut，单行广播（阈值现算）
  WHERE s.max_round > c.hi_cut OR s.max_round <= 5                                                    -- 过滤条件：限定超长或残靴二者之一
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  67. R01chk_shoe_len_by_table.csv   [总包 v11.1.0 行 3725~3855 · 输出 28 列]
--     原 ORDER BY：z.avg_max_round DESC, z.table_id
--     audit_rn 之排序键与分批作业版第 67 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`avg_max_round` DESC, y.`table_id`, y.`table_flag`, y.`n_shoes`, y.`n_shoes_old`, y.`n_shoes_delta`, y.`avg_max_round_old`, y.`avg_maxround_inflation`, y.`sd_max_round`, y.`min_max_round`, y.`ceiling_observed`, y.`ceiling_old`, y.`p50_max_round`, y.`p90_max_round`, y.`p99_max_round`, y.`collision_flag`, y.`n_stub_shoes`, y.`avg_gap`, y.`avg_n_bettors`, y.`avg_bets_per_round`, y.`round_at_pos080`, y.`corr_len_density`, y.`corr_density_lo95`, y.`corr_density_hi95`, y.`corr_density_verdict`, y.`corr_len_bettors`, y.`corr_bettors_lo95`, y.`corr_bettors_hi95`) AS audit_rn
FROM (
  WITH test_agents AS (                                                                               -- 公共表表达式：开启中间结果集 test_agents，其后各行为其定义体（§R01-chk）
    SELECT age001 AS agent_id                                                                         -- 取列：产出「agent_id」
    FROM ods_mariadb_2b.ods_a168_agent                                                                -- 取数来源：取自代理主档（五级代理线归属来源）
    WHERE age022 = '1'                                  -- 铁律④b：公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-chk）
  ranked AS (                                            -- 需求 §3.2：同单号保留最新版本
    SELECT b.bet01, b.dt, b.bet03, b.bet04, b.bet05, b.bet11,                                         -- 取列：起始取列子句，本行先列 b.bet01, b.dt, b.bet03, b.bet04, b.bet05, b.bet11，涉 dt（营业日）、bet03（靴号）、bet04（局内序号）
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：接续上一取列子句，续列 b.bet18~b.bet22，涉五级代理线（测试线剔除之连接键）
           b.bet38, b.bet39, b.category,                                                              -- 续行：接续上一取列子句，续列 b.bet38, b.bet39, b.category，涉 bet38（重對）、bet39（桌号）、category（注单类别）
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base，其后各行为其定义体（§R01-chk）
    SELECT r.dt                                          AS dt,                                       -- 取列：起始取列子句，本行先取「dt」，涉 dt（营业日）——本族较 §R01 之唯一增列
           CAST(NULLIF(TRIM(r.bet05),'') AS BIGINT)      AS member_id,                                -- 取值表达式：先去空白、空串归 NULL，再显式转型——本库字段多为 varchar，不转型即比较失真，产出「member_id」
           COALESCE(NULLIF(TRIM(r.bet03),''), '(空)')    AS shoe_id,                                  -- 取值表达式：空串归哨兵「(空)」以免与真靴号混算，涉 bet03（靴号），产出「shoe_id」
           COALESCE(NULLIF(TRIM(r.bet39),''), '(空)')    AS table_id,                                 -- 取值表达式：空串归哨兵「(空)」以免与真桌号混算，涉 bet39（桌号），产出「table_id」
  ...
  shoe_new AS (          /* ★ 新粒度（(dt, table_id, shoe_id)）：靴号纵跨日复用亦不致合并。所算 max_round = MAX(round_no) 为『该靴该桌该日观测到的最大局号』，非 COUNT(round_no) 之局数；此处刻意不沿用 shoe_len 之名，见卷首命名警示横幅。★ */
    SELECT dt, table_id, shoe_id,                                                                     -- 取列：起始取列子句，本行先列 dt, table_id, shoe_id，涉 dt（营业日）、bet39（桌号）、bet03（靴号）
           MAX(round_no)                                        AS max_round,                         -- 取最大局号（≠COUNT 局数），产出「max_round」
           COUNT(DISTINCT round_no)                             AS rounds_with_bet,                   -- 计数表达式：局内序号的去重个数——有注局数，产出「rounds_with_bet」
           MAX(round_no) - COUNT(DISTINCT round_no)             AS gap,                               -- 取值表达式：最大局号减有注局数——内部缺口，非尾部截断，产出「gap」
           COUNT(DISTINCT member_id)                            AS n_bettors,                         -- 计数表达式：会员号的去重个数——桌面稀疏度之一，产出「n_bettors」
           COUNT(*)                                             AS n_bets,                            -- 计数表达式：注单数，产出「n_bets」
           COUNT(*) * 1.0 / NULLIF(COUNT(DISTINCT round_no), 0) AS bets_per_round                     -- 除法计算：每局注单数——桌面稀疏度之二，分母以 NULLIF 守门防零除，产出「bets_per_round」
    FROM base                                                                                         -- 取数来源：取自本条自建的中间结果集 base
    GROUP BY dt, table_id, shoe_id                                                                    -- 分组：按 dt, table_id, shoe_id 汇总——新粒度
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-chk）
  shoe_old AS (          /* ★ 旧粒度（(table_id, shoe_id)）：§R01 现行口径，留作对照，其与新粒度之差即粒度污染之量。★ */
    SELECT table_id, shoe_id,                                                                         -- 取列：起始取列子句，本行先列 table_id, shoe_id，涉 bet39（桌号）、bet03（靴号）
           MAX(round_no) AS max_round_old                                                             -- 取最大局号（≠COUNT 局数），产出「max_round_old」
    FROM base                                                                                         -- 取数来源：取自本条自建的中间结果集 base
    GROUP BY table_id, shoe_id                                                                        -- 分组：按 table_id, shoe_id 汇总——旧粒度
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-chk）
  agg AS (                                                                                            -- 公共表表达式：开启中间结果集 agg，其后各行为其定义体（§R01-chk）
    SELECT table_id,                                                                                  -- 取列：起始取列子句，本行先取「table_id」，涉 bet39（桌号）
           COUNT(*)                                     AS n_shoes,                                   -- 计数表达式：本桌之靴数（新粒度），产出「n_shoes」
           AVG(max_round)                               AS avg_max_round,                             -- 聚合表达式：最大局号均值，产出「avg_max_round」
           STDDEV_SAMP(max_round)                       AS sd_max_round,                              -- 聚合表达式：最大局号样本标准差，产出「sd_max_round」
           MIN(max_round)                               AS min_max_round,                             -- 聚合表达式：最大局号下界——残靴之证据，产出「min_max_round」
           MAX(max_round)                               AS ceiling_observed,                          -- 聚合表达式：最大局号上界——物理上限（切牌收靴）之证据，产出「ceiling_observed」
           PERCENTILE_APPROX(max_round, 0.50)           AS p50_max_round,                             -- 取近似分位数表达式：中位，产出「p50_max_round」
           PERCENTILE_APPROX(max_round, 0.90)           AS p90_max_round,                             -- 取近似分位数表达式：九十分位，产出「p90_max_round」
           PERCENTILE_APPROX(max_round, 0.99)           AS p99_max_round,                             -- 取近似分位数表达式：九九分位，产出「p99_max_round」
           AVG(gap)                                     AS avg_gap,                                   -- 聚合表达式：内部缺口均值，产出「avg_gap」
           AVG(n_bettors)                               AS avg_n_bettors,                             -- 聚合表达式：每靴下注会员数均值，产出「avg_n_bettors」
           AVG(bets_per_round)                          AS avg_bets_per_round,                        -- 聚合表达式：每局注单数均值，产出「avg_bets_per_round」
           CORR(max_round, bets_per_round)              AS r_density,                                 -- 聚合表达式：靴长与桌面密度之相关——低估假说之判别力核心，产出「r_density」
           CORR(max_round, n_bettors)                   AS r_bettors,                                 -- 聚合表达式：靴长与下注人数之相关，产出「r_bettors」
           SUM(CASE WHEN max_round <= 5 THEN 1 ELSE 0 END) AS n_stub_shoes                            -- 条件分支：最大局号不超过 5 之残靴数，产出「n_stub_shoes」
    FROM shoe_new                                                                                     -- 取数来源：取自本条自建的中间结果集 shoe_new
    GROUP BY table_id                                                                                 -- 分组：按桌号汇总
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-chk）
  agg_old AS (                                                                                        -- 公共表表达式：开启中间结果集 agg_old，其后各行为其定义体（§R01-chk）
    SELECT table_id,                                                                                  -- 取列：起始取列子句，本行先取「table_id」，涉 bet39（桌号）
           COUNT(*)           AS n_shoes_old,                                                         -- 计数表达式：本桌之靴数（旧粒度），产出「n_shoes_old」
           AVG(max_round_old) AS avg_max_round_old,                                                   -- 聚合表达式：旧粒度最大局号均值，产出「avg_max_round_old」
           MAX(max_round_old) AS ceiling_old                                                          -- 聚合表达式：旧粒度最大局号上界，产出「ceiling_old」
    FROM shoe_old                                                                                     -- 取数来源：取自本条自建的中间结果集 shoe_old
    GROUP BY table_id                                                                                 -- 分组：按桌号汇总
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-chk）
  zc AS (                /* Fisher-z 变换：z = 0.5·ln((1+r)/(1−r))，se = 1/√(n−3)；反变换 tanh(x) = (e^{2x}−1)/(e^{2x}+1)。样本量护栏不设普适门槛，由统计量本身反解：n ≤ 3 者 z 无定义，|r| → 1 者对数发散，二者一律归 NULL 照登，禁以点估计代 CI 下结论。 */
    SELECT a.*,                                                                                       -- 取列：承接上一子句，取 agg 全列
           CASE WHEN a.n_shoes > 3 AND ABS(a.r_density) < 0.999999                                    -- 分支判定：样本量大于 3 且相关系数未贴边者方可变换
                THEN 0.5 * LN((1 + a.r_density) / (1 - a.r_density)) END AS z_d,                      -- 条件分支：密度相关之 Fisher-z，产出「z_d」
           CASE WHEN a.n_shoes > 3 AND ABS(a.r_bettors) < 0.999999                                    -- 分支判定：样本量大于 3 且相关系数未贴边者方可变换
                THEN 0.5 * LN((1 + a.r_bettors) / (1 - a.r_bettors)) END AS z_b,                      -- 条件分支：人数相关之 Fisher-z，产出「z_b」
           CASE WHEN a.n_shoes > 3                                                                    -- 分支判定：样本量大于 3 者方有标准误
                THEN 1.959964 / SQRT(a.n_shoes - 3) END                  AS hw                        -- 条件分支：95% 区间半宽（z=1.959964 双侧），产出「hw」
    FROM agg a                                                                                        -- 取数来源：取自本条自建的中间结果集 agg
  )                                                                                                   -- 续行：收束上方的子查询或函数括号（§R01-chk）
  SELECT                                                                                              -- 取列：起始取列子句
    z.table_id,                                                                                       -- 取列：涉 bet39（桌号）
    CASE WHEN z.table_id = '(空)' THEN '哨兵（禁并入正常桌）' ELSE '正常' END      AS table_flag,      -- 条件分支：哨兵标记——空串桌号单列，禁与正常桌混算，产出「table_flag」
    z.n_shoes,                                                                                        -- 取列：本桌靴数（新粒度）
    o.n_shoes_old,                                                                                    -- 取列：本桌靴数（旧粒度）
    z.n_shoes - o.n_shoes_old                                    AS n_shoes_delta,                    -- 取值表达式：新旧粒度靴数之差——大于 0 即证靴号跨日复用，产出「n_shoes_delta」
    ROUND(z.avg_max_round, 4)                                    AS avg_max_round,                    -- 聚合表达式：最大局号均值（新粒度），产出「avg_max_round」
    ROUND(o.avg_max_round_old, 4)                                AS avg_max_round_old,                -- 聚合表达式：最大局号均值（旧粒度），产出「avg_max_round_old」
    ROUND(o.avg_max_round_old - z.avg_max_round, 4)              AS avg_maxround_inflation,           -- 取值表达式：旧粒度较新粒度夸大之量，产出「avg_maxround_inflation」
    ROUND(z.sd_max_round, 4)                                     AS sd_max_round,                     -- 聚合表达式：最大局号样本标准差，产出「sd_max_round」
    z.min_max_round,                                                                                  -- 取列：最大局号下界
    z.ceiling_observed,                                                                               -- 取列：最大局号上界（新粒度）
    o.ceiling_old,                                                                                    -- 取列：最大局号上界（旧粒度）
    ROUND(z.p50_max_round, 2)                                    AS p50_max_round,                    -- 取近似分位数表达式：中位，产出「p50_max_round」
    ROUND(z.p90_max_round, 2)                                    AS p90_max_round,                    -- 取近似分位数表达式：九十分位，产出「p90_max_round」
    ROUND(z.p99_max_round, 2)                                    AS p99_max_round,                    -- 取近似分位数表达式：九九分位，产出「p99_max_round」
    CASE WHEN z.ceiling_observed > z.p99_max_round * 1.5                                              -- 分支判定：上界逾九九分位之 1.5 倍者
         THEN '碰撞可疑' ELSE '—' END                            AS collision_flag,                   -- 条件分支：靴号碰撞或跨日合并之指纹，产出「collision_flag」
    z.n_stub_shoes,                                                                                   -- 取列：残靴数（最大局号不超过 5）
    ROUND(z.avg_gap, 4)                                          AS avg_gap,                          -- 聚合表达式：内部缺口均值，产出「avg_gap」
    ROUND(z.avg_n_bettors, 4)                                    AS avg_n_bettors,                    -- 聚合表达式：每靴下注会员数均值，产出「avg_n_bettors」
    ROUND(z.avg_bets_per_round, 4)                               AS avg_bets_per_round,               -- 聚合表达式：每局注单数均值，产出「avg_bets_per_round」
    ROUND(0.80 * z.avg_max_round, 2)                             AS round_at_pos080,                  -- 取值表达式：本桌 shoe_pos>=0.80（LATE_DEF）对应之起始局号——跨桌可比性之直接刻度，产出「round_at_pos080」
    ROUND(z.r_density, 6)                                        AS corr_len_density,                 -- 聚合表达式：靴长与桌面密度之相关点估计，产出「corr_len_density」
    ROUND((EXP(2*(z.z_d - z.hw)) - 1) / (EXP(2*(z.z_d - z.hw)) + 1), 6) AS corr_density_lo95,         -- 取值表达式：密度相关之 95% 下界（Fisher-z 反变换），产出「corr_density_lo95」
    ROUND((EXP(2*(z.z_d + z.hw)) - 1) / (EXP(2*(z.z_d + z.hw)) + 1), 6) AS corr_density_hi95,         -- 取值表达式：密度相关之 95% 上界（Fisher-z 反变换），产出「corr_density_hi95」
    CASE                                                                                              -- 条件分支：起始分支——密度相关之判词，四态互斥
      WHEN z.z_d IS NULL THEN '○ n 不足（须 > 3）'                                                    -- 分支判定：样本量不足者照登，禁读方向
      WHEN ((EXP(2*(z.z_d - z.hw)) - 1) / (EXP(2*(z.z_d - z.hw)) + 1)) <= 0                           -- 分支判定：下界不大于零
       AND ((EXP(2*(z.z_d + z.hw)) - 1) / (EXP(2*(z.z_d + z.hw)) + 1)) >= 0                           -- 分支判定：且上界不小于零——即区间跨零
        THEN 'CI 跨 0：方向不可读'                                                                    -- 分支判定：区间跨零者方向不可读
      WHEN ((EXP(2*(z.z_d - z.hw)) - 1) / (EXP(2*(z.z_d - z.hw)) + 1)) > 0                            -- 分支判定：下界大于零
        THEN 'CI > 0：低估假说得证'                                                                   -- 分支判定：密桌靴长——分母低估坐实
      ELSE 'CI < 0：与低估假说反向'                                                                   -- 分支判定：余者密桌反而靴短——指向切牌规程异质
    END                                                          AS corr_density_verdict,             -- 条件分支：收束上方判词，产出「corr_density_verdict」
    ROUND(z.r_bettors, 6)                                        AS corr_len_bettors,                 -- 聚合表达式：靴长与下注人数之相关点估计，产出「corr_len_bettors」
    ROUND((EXP(2*(z.z_b - z.hw)) - 1) / (EXP(2*(z.z_b - z.hw)) + 1), 6) AS corr_bettors_lo95,         -- 取值表达式：人数相关之 95% 下界（Fisher-z 反变换），产出「corr_bettors_lo95」
    ROUND((EXP(2*(z.z_b + z.hw)) - 1) / (EXP(2*(z.z_b + z.hw)) + 1), 6) AS corr_bettors_hi95          -- 取值表达式：人数相关之 95% 上界（Fisher-z 反变换），产出「corr_bettors_hi95」
  FROM zc z                                                                                           -- 取数来源：取自本条自建的中间结果集 zc
  LEFT JOIN agg_old o ON o.table_id = z.table_id                                                      -- 左连接：取自本条自建的中间结果集 agg_old，连接键为 table_id（桌号）
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  68. R01inv_shoe_len_census.csv   [总包 v11.1.0 行 3865~3940 · 输出 19 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 68 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_tables`, y.`n_shoes`, y.`mean_len`, y.`sd_len`, y.`p0`, y.`p01`, y.`p25`, y.`p50`, y.`p75`, y.`p90`, y.`p99`, y.`p100`, y.`n_len_ge_70`, y.`n_len_le_5`, y.`pct_with_gap`, y.`avg_gap`, y.`corr_cross_table`, y.`round_at_080_shortest`, y.`round_at_080_longest`) AS audit_rn
FROM (
  WITH test_agents AS (                                                                               -- 公共表表达式：开启中间结果集 test_agents，其后各行为其定义体（§R01-inv）
    SELECT age001 AS agent_id                                                                         -- 取列：产出「agent_id」
    FROM ods_mariadb_2b.ods_a168_agent                                                                -- 取数来源：取自代理主档（五级代理线归属来源）
    WHERE age022 = '1'                                  -- 铁律④b：公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-inv）
  ranked AS (                                            -- 需求 §3.2：同单号保留最新版本
    SELECT b.bet01, b.dt, b.bet03, b.bet04, b.bet05, b.bet11,                                         -- 取列：起始取列子句，本行先列 b.bet01, b.dt, b.bet03, b.bet04, b.bet05, b.bet11，涉 dt（营业日）、bet03（靴号）、bet04（局内序号）
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：接续上一取列子句，续列 b.bet18~b.bet22，涉五级代理线（测试线剔除之连接键）
           b.bet38, b.bet39, b.category,                                                              -- 续行：接续上一取列子句，续列 b.bet38, b.bet39, b.category，涉 bet38（重對）、bet39（桌号）、category（注单类别）
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base，其后各行为其定义体（§R01-inv）
    SELECT r.dt                                          AS dt,                                       -- 取列：起始取列子句，本行先取「dt」，涉 dt（营业日）——本族较 §R01 之唯一增列
           CAST(NULLIF(TRIM(r.bet05),'') AS BIGINT)      AS member_id,                                -- 取值表达式：先去空白、空串归 NULL，再显式转型——本库字段多为 varchar，不转型即比较失真，产出「member_id」
           COALESCE(NULLIF(TRIM(r.bet03),''), '(空)')    AS shoe_id,                                  -- 取值表达式：空串归哨兵「(空)」以免与真靴号混算，涉 bet03（靴号），产出「shoe_id」
           COALESCE(NULLIF(TRIM(r.bet39),''), '(空)')    AS table_id,                                 -- 取值表达式：空串归哨兵「(空)」以免与真桌号混算，涉 bet39（桌号），产出「table_id」
  ...
  shoe_new AS (          /* ★ 新粒度（(dt, table_id, shoe_id)）：所算 max_round = MAX(round_no) 为最大局号，非局数；不沿用 shoe_len 之名，见卷首命名警示横幅。★ */
    SELECT dt, table_id, shoe_id,                                                                     -- 取列：起始取列子句，本行先列 dt, table_id, shoe_id，涉 dt（营业日）、bet39（桌号）、bet03（靴号）
           ...
           MAX(round_no) - COUNT(DISTINCT round_no)             AS gap,                               -- 取值表达式：最大局号减有注局数——内部缺口，非尾部截断，产出「gap」
           COUNT(DISTINCT member_id)                            AS n_bettors,                         -- 计数表达式：会员号的去重个数，产出「n_bettors」
           COUNT(*) * 1.0 / NULLIF(COUNT(DISTINCT round_no), 0) AS bets_per_round                     -- 除法计算：每局注单数，分母以 NULLIF 守门防零除，产出「bets_per_round」
    FROM base                                                                                         -- 取数来源：取自本条自建的中间结果集 base
    GROUP BY dt, table_id, shoe_id                                                                    -- 分组：按 dt, table_id, shoe_id 汇总——新粒度
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号（§R01-inv）
  by_table AS (               /* 跨桌相关之单位＝桌（非靴）：与 §R01-chk 之桌内相关分属两层，禁互相援引。 */
    SELECT table_id,                                                                                  -- 取列：起始取列子句，本行先取「table_id」，涉 bet39（桌号）
           COUNT(*)            AS n_shoes,                                                            -- 计数表达式：本桌靴数，产出「n_shoes」
           AVG(max_round)      AS avg_len,                                                            -- 聚合表达式：本桌最大局号均值，产出「avg_len」
           AVG(bets_per_round) AS avg_density                                                         -- 聚合表达式：本桌每局注单数均值，产出「avg_density」
    FROM shoe_new                                                                                     -- 取数来源：取自本条自建的中间结果集 shoe_new
    GROUP BY table_id                                                                                 -- 分组：按桌号汇总
  )                                                                                                   -- 续行：收束上方的子查询或函数括号（§R01-inv）
  SELECT                                                                                              -- 取列：起始取列子句
    (SELECT COUNT(*) FROM by_table)                                   AS n_tables,                         -- 计数表达式：全域桌数——供核对 §R01-chk 结果行数是否被显示上限截断，产出「n_tables」
    COUNT(*)                                                     AS n_shoes,                          -- 计数表达式：全域靴数（新粒度），产出「n_shoes」
    ROUND(AVG(max_round), 4)                                     AS mean_len,                         -- 聚合表达式：最大局号均值，产出「mean_len」
    ROUND(STDDEV_SAMP(max_round), 4)                             AS sd_len,                           -- 聚合表达式：最大局号样本标准差，产出「sd_len」
    MIN(max_round)                                               AS p0,                               -- 聚合表达式：最小值，产出「p0」
    ROUND(PERCENTILE_APPROX(max_round, 0.01), 2)                 AS p01,                              -- 取近似分位数表达式：一分位，产出「p01」
    ROUND(PERCENTILE_APPROX(max_round, 0.25), 2)                 AS p25,                              -- 取近似分位数表达式：二五分位，产出「p25」
    ROUND(PERCENTILE_APPROX(max_round, 0.50), 2)                 AS p50,                              -- 取近似分位数表达式：中位，产出「p50」
    ROUND(PERCENTILE_APPROX(max_round, 0.75), 2)                 AS p75,                              -- 取近似分位数表达式：七五分位，产出「p75」
    ROUND(PERCENTILE_APPROX(max_round, 0.90), 2)                 AS p90,                              -- 取近似分位数表达式：九十分位，产出「p90」
    ROUND(PERCENTILE_APPROX(max_round, 0.99), 2)                 AS p99,                              -- 取近似分位数表达式：九九分位，产出「p99」
    MAX(max_round)                                               AS p100,                             -- 聚合表达式：最大值，产出「p100」
    SUM(CASE WHEN max_round >= 70 THEN 1 ELSE 0 END)             AS n_len_ge_70,                      -- 条件分支：最大局号不少于 70 之靴数——深切规程之证据，产出「n_len_ge_70」
    SUM(CASE WHEN max_round <= 5  THEN 1 ELSE 0 END)             AS n_len_le_5,                       -- 条件分支：最大局号不超过 5 之残靴数，产出「n_len_le_5」
    ROUND(SUM(CASE WHEN gap > 0 THEN 1 ELSE 0 END) * 100.0                                            -- 条件分支：有内部缺口之靴数
          / NULLIF(COUNT(*), 0), 4)                              AS pct_with_gap,                     -- 除法计算：分母以 NULLIF 守门防零除，产出「pct_with_gap」
    ROUND(AVG(gap), 4)                                           AS avg_gap,                          -- 聚合表达式：内部缺口均值，产出「avg_gap」
    ROUND((SELECT CORR(avg_density, avg_len) FROM by_table), 6)       AS corr_cross_table,                 -- 聚合表达式：跨桌相关（单位＝桌）——为负即密桌反而靴短，产出「corr_cross_table」
    ROUND(0.80 * (SELECT MIN(avg_len) FROM by_table), 2)              AS round_at_080_shortest,            -- 取值表达式：最短桌之 shoe_pos>=0.80 起始局号，产出「round_at_080_shortest」
    ROUND(0.80 * (SELECT MAX(avg_len) FROM by_table), 2)              AS round_at_080_longest              -- 取值表达式：最长桌之 shoe_pos>=0.80 起始局号——与上一列之差即跨桌规程异质之刻度，产出「round_at_080_longest」
  FROM shoe_new                                                                                      -- 取数来源：取自本条自建的中间结果集 shoe_new
) y
ORDER BY audit_rn;

-- ──────────────────────────────────────────────────────────────────────────────
--  69. R02_same_table.csv   [总包 v11.1.0 行 4293~4365 · 输出 12 列]
--     原 ORDER BY：lift DESC, same_rate DESC
--     audit_rn 之排序键与分批作业版第 69 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`lift` DESC, y.`same_rate` DESC, y.`uid1`, y.`uid2`, y.`n_same_orders`, y.`rounds_1`, y.`rounds_2`, y.`jaccard`, y.`exp_same`, y.`n_tables`, y.`first_day`, y.`last_day`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  70. R03_player_dealer.csv   [总包 v11.1.0 行 4502~4633 · 输出 33 列]
--     原 ORDER BY：pdw.member_id, pdw.dealer_id
--     audit_rn 之排序键与分批作业版第 70 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
...
FROM (
  WITH ranked AS (                                                                                    -- 公共表表达式：开启中间结果集 ranked，其后各行为其定义体（§R03）
    SELECT b.bet01,                                                                                   -- 取列：起始取列子句——★ 2026-08-11 提速：只投影下游真正用到的列
           ...
           b.bet13, b.bet14, b.bet17, b.bet38, b.bet39,                                               -- 续行：涉 bet13（下注金额）、bet14（派彩金额）、bet17（会员净输赢）、bet38（重對）、bet39（桌号）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  71. R03b_player_dealer_daily.csv   [总包 v11.1.0 行 6143~6242 · 输出 24 列]
--     原 ORDER BY：p.bet_date, p.member_id, p.dealer_id
--     audit_rn 之排序键与分批作业版第 71 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  72. R03chk_settlement_form.csv   [总包 v11.1.0 行 4684~4723 · 输出 6 列]
--     原 ORDER BY：n_rounds DESC
--     audit_rn 之排序键与分批作业版第 72 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rounds` DESC, y.`bet_side`, y.`n_push`, y.`push_rate`, y.`p_base_dnb`, y.`p_base_incl`) AS audit_rn
FROM (
  WITH ranked AS (                                                                                    -- 公共表表达式：开启中间结果集 ranked——口径与 §R03 逐字一致，便于对照（§R03-chk）
    SELECT b.bet01, b.bet03, b.bet04, b.bet39, b.bet05, b.bet09,                                      -- 取列：起始取列子句，涉 bet03（靴号）、bet04（局内序号）、bet39（桌号）、bet05（会员号）、bet09（玩法）
           b.bet11, b.bet13, b.bet14, b.bet38, b.eid,                                                 -- 续行：接续上一取列子句，涉 bet11（汇率）、bet13（下注金额）、bet14（派彩金额）、bet38（重對）
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base——有效注单，与 §R03 同口径（§R03-chk）
    ...
    WHERE r.rn = 1                                                                                    -- 过滤条件：限定 r.rn等于 1，只取每注最终态
      AND UPPER(TRIM(r.bet38)) = 'N'                                                                  -- 并列条件：剔除测试注单，涉 bet38（重對）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  73. R03inv_pair_census.csv   [总包 v11.1.0 行 4745~4796 · 输出 6 列]
--     原 ORDER BY：1, 2
--     audit_rn 之排序键与分批作业版第 73 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`eligibility_status`, y.`n_bucket`, y.`n_pairs`, y.`n_members`, y.`n_dealers`, y.`n_rounds_total`) AS audit_rn
FROM (
  WITH ranked AS (                                                                                    -- 公共表表达式：开启中间结果集 ranked——口径与 §R03 一字不差（§R03-inv）
    SELECT b.bet01, b.bet03, b.bet04, b.bet39, b.bet05, b.bet09, b.bet11,                             -- 取列：起始取列子句，涉 bet03（靴号）、bet04（局内序号）、bet39（桌号）、bet05（会员号）
           b.bet13, b.bet14, b.bet38, b.eid,                                                          -- 续行：接续上一取列子句，涉 bet13（下注金额）、bet14（派彩金额）、bet38（重對）
  ...
  base AS (                                                                                           -- 公共表表达式：开启中间结果集 base——有效注单，哨兵**标注不删**（§R03-inv）
    ...
    FROM ranked r                                                                                     -- 取数来源：取自本条自建的中间结果集 ranked
    WHERE r.rn = 1 AND UPPER(TRIM(r.bet38)) = 'N'                                                     -- 过滤条件：只取每注最终态且非测试单，涉 bet38（重對）
      AND CAST(NULLIF(TRIM(r.bet05),'') AS BIGINT) > 0                                                -- 并列条件：剔除无效会员号，涉 bet05（会员号）
      AND CAST(NULLIF(TRIM(r.bet11),'') AS DECIMAL(20,8)) > 0                                         -- 并列条件：汇率须为正，涉 bet11（汇率）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  74. R_rebate_dist.csv   [总包 v11.1.0 行 1489~1492 · 输出 3 列]
--     原 ORDER BY：n_member DESC
--     audit_rn 之排序键与分批作业版第 74 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_member` DESC, y.`rebate_rate`, y.`pct`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  75. S01_player_score.csv   [总包 v11.1.0 行 2198~2320 · 输出 11 列]
--     原 ORDER BY：流水贡献 DESC
--     audit_rn 之排序键与分批作业版第 75 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`流水贡献` DESC, y.`member_id`, y.`游戏输赢贡献`, y.`活跃稳定`, y.`退水支出`, y.`n_ip`, y.`n_chain`, y.`人工标记史`, y.`投注产品结构熵`, y.`索提诺稳定性`, y.`生命周期阶段`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  76. S02_dealer_score.csv   [总包 v11.1.0 行 2339~2458 · 输出 9 列]
--     原 ORDER BY：在桌洗码量 DESC
--     audit_rn 之排序键与分批作业版第 76 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`在桌洗码量` DESC, y.`dealer_id`, y.`客群广度`, y.`n_rounds`, y.`n_tables`, y.`桌面输赢`, y.`当值时长`, y.`节奏稳定度`, y.`特定玩家复现率`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
             ...
             WHEN SUM(d.n_rows * g.n_g) >= 2 THEN                                                     -- 分支判定：汇总
               1.0 / (1.0 + COALESCE(SQRT(GREATEST(                                                   -- 取值表达式：承接上一子句，构成完整语句，属 交付件 S02_dealer_score.csv 之取数
                 ( SUM(d.n_rows * g.s2)                                                               -- 续行：汇总
                   - SUM(d.n_rows * g.s1) * SUM(d.n_rows * g.s1)                                      -- 加减计算：二阶矩减一阶矩平方 ＝ 方差，与 GGR 无涉
...
-- ──────────────────────────────────────────────────────────────────────────────
--  77. S02b_dealer_pace.csv   [总包 v11.1.0 行 6390~6447 · 输出 3 列]
--     原 ORDER BY：局速 DESC
--     audit_rn 之排序键与分批作业版第 77 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`局速` DESC, y.`dealer_id`, y.`平均每局秒数`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  78. S03_agent_score.csv   [总包 v11.1.0 行 2467~2566 · 输出 9 列]
--     原 ORDER BY：真实流水 DESC
--     audit_rn 之排序键与分批作业版第 78 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`真实流水` DESC, y.`lv3`, y.`线下规模`, y.`退水支出`, y.`链下游戏输赢`, y.`退水消耗比`, y.`下线留存`, y.`新增会员质量`, y.`新增会员数`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  79. S04_analyst_score.csv   [总包 v11.1.0 行 6277~6302 · 输出 7 列]
--     原 ORDER BY：标注产量 DESC
--     audit_rn 之排序键与分批作业版第 79 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`标注产量` DESC, y.`entity_id`, y.`覆盖IP广度`, y.`判定详尽度`, y.`理由空白数`, y.`首次登记`, y.`最近登记`) AS audit_rn
FROM (
  SELECT                                                                                              -- 续行：接续上一取列子句，续列 SELECT
...
-- ──────────────────────────────────────────────────────────────────────────────
--  80. S05_member_month_panel.csv   [总包 v11.1.0 行 2614~2678 · 输出 10 列]
--     原 ORDER BY：b.member_id, bet_date
--     audit_rn 之排序键与分批作业版第 80 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`bet_date`, y.`流水贡献`, y.`游戏输赢贡献`, y.`活跃稳定`, y.`退水支出`, y.`n_ip`, y.`n_chain`, y.`round_key`, y.`risk_label`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  81. S_player_tail.csv   [总包 v11.1.0 行 2077~2189 · 输出 18 列]
--     原 ORDER BY：member_id
--     audit_rn 之排序键与分批作业版第 81 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`n_rounds_all`, y.`n_rounds_tail`, y.`tail_share`, y.`tail_win`, y.`tail_lose`, y.`tail_winrate`, y.`norm_winrate`, y.`winrate_diff`, y.`tail_stake`, y.`tail_valid_bet`, y.`tail_game_pnl`, y.`tail_rebate`, y.`tail_net_pnl`, y.`tail_roi`, y.`norm_game_pnl`, y.`norm_roi`, y.`tail_profit_share`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  82. S_second_dist.csv   [总包 v11.1.0 行 1914~1982 · 输出 8 列]
--     原 ORDER BY：n_orders DESC
--     audit_rn 之排序键与分批作业版第 82 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_orders` DESC, y.`table_id`, y.`p50`, y.`p75`, y.`p90`, y.`p95`, y.`p99`, y.`max_progress`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
--  83. T02_daily_roi.csv   [总包 v11.1.0 行 5146~5190 · 输出 5 列]
--     原 ORDER BY：o.member_id, o.bet_date
--     audit_rn 之排序键与分批作业版第 83 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`bet_date`, y.`stake`, y.`game_pnl`, y.`n_rounds`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  84. T03_arbitrage.csv   [总包 v11.1.0 行 5204~5275 · 输出 10 列]
--     原 ORDER BY：hedge_rate DESC, validbet DESC
--     audit_rn 之排序键与分批作业版第 84 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`hedge_rate` DESC, y.`validbet` DESC, y.`member_id`, y.`stake`, y.`game_pnl`, y.`net_pnl`, y.`rebate`, y.`stake_cv`, y.`n_rounds`, y.`n_days`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  85. TL00_table_inventory.csv   [总包 v11.1.0 行 8168~8215 · 输出 5 列]
--     原 ORDER BY：处置类列数 DESC, 定日类列数 DESC, c.TABLE_NAME
--     audit_rn 之排序键与分批作业版第 85 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`处置类列数` DESC, y.`定日类列数` DESC, y.`TABLE_NAME`, y.`列数`, y.`时间列数`) AS audit_rn
FROM (
  SELECT  c.TABLE_NAME,
          COUNT(*)                                                   AS 列数,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  86. TL02_config_change.csv   [总包 v11.1.0 行 8238~8269 · 输出 4 列]
--     原 ORDER BY：发生变更的会员数 DESC
--     audit_rn 之排序键与分批作业版第 86 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`发生变更的会员数` DESC, y.`变更列`, y.`在册会员数`, y.`变更率`) AS audit_rn
FROM (
  SELECT  变更列,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  87. TL03_pseudo_treatment.csv   [总包 v11.1.0 行 8276~8324 · 输出 6 列]
--     原 ORDER BY：member_id, change_date
--     audit_rn 之排序键与分批作业版第 87 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`change_date`, y.`action_type`, y.`value_before`, y.`value_after`, y.`direction`) AS audit_rn
FROM (
  WITH snap AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  88. TL03b_pseudo_treatment.csv   [总包 v11.1.0 行 8417~8466 · 输出 8 列]
--     原 ORDER BY：member_id, cate, change_date
--     audit_rn 之排序键与分批作业版第 88 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`cate`, y.`change_date`, y.`prev_date`, y.`action_type`, y.`value_before`, y.`value_after`, y.`direction`) AS audit_rn
FROM (
  WITH snap AS (
      SELECT  CAST(mem001 AS STRING)                        AS member_id,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  89. TL04_log_schema.csv   [总包 v11.1.0 行 8383~8389 · 输出 5 列]
--     原 ORDER BY：TABLE_NAME, ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 89 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`TABLE_NAME`, y.`ORDINAL_POSITION`, y.`COLUMN_NAME`, y.`DATA_TYPE`, y.`COLUMN_COMMENT`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  90. TL06_log_mem_change.csv   [总包 v11.1.0 行 8473~8479 · 输出 2 列]
--     原 ORDER BY：dt
--     audit_rn 之排序键与分批作业版第 90 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`dt`, y.`行数`) AS audit_rn
FROM (
  SELECT  dt,
          COUNT(*)                                          AS 行数
  FROM    ods_mariadb_2b.ods_a168_log_mem_change
...
-- ──────────────────────────────────────────────────────────────────────────────
--  91. TL07_mem_change_shape.csv   [总包 v11.1.0 行 8519~8533 · 输出 10 列]
--     原 ORDER BY：条数 DESC
--     audit_rn 之排序键与分批作业版第 91 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`条数` DESC, y.`类别`, y.`涉及会员数`, y.`操作者数`, y.`最早异动`, y.`最晚异动`, y.`内容平均长度`, y.`内容样例`, y.`SQL平均长度`, y.`SQL样例`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  92. TL08_treatment_events.csv   [总包 v11.1.0 行 8541~8572 · 输出 9 列]
--     原 ORDER BY：ev.action_time, ev.member_id
--     audit_rn 之排序键与分批作业版第 92 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`action_time`, y.`member_id`, y.`action_date`, y.`action_class`, y.`operator_id`, y.`operator_lv`, y.`content`, y.`raw_sql`, y.`is_baccarat_member`) AS audit_rn
FROM (
  WITH ev AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  93. TL09_member_schema.csv   [总包 v11.1.0 行 8603~8607 · 输出 4 列]
--     原 ORDER BY：ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 93 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`ORDINAL_POSITION`, y.`COLUMN_NAME`, y.`DATA_TYPE`, y.`COLUMN_COMMENT`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  94. TL10_treatment_ledger.csv   [总包 v11.1.0 行 8615~8675 · 输出 10 列]
--     原 ORDER BY：p.action_time, p.member_id, p.field_name
--     audit_rn 之排序键与分批作业版第 94 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`action_time`, y.`member_id`, y.`field_name`, y.`action_date`, y.`action_class`, y.`value_before`, y.`value_after`, y.`operator_id`, y.`operator_lv`, y.`is_baccarat_member`) AS audit_rn
FROM (
  WITH ev AS (
      SELECT  CAST(lmc02 AS STRING)                         AS member_id,
              CAST(lmc04 AS STRING)                         AS action_class,
...
-- ──────────────────────────────────────────────────────────────────────────────
--  96. TL11c_population_reconcile.csv   [总包 v11.1.0 行 8969~9055 · 输出 7 列]  [🚩第 [1, 2, 3, 4, 5, 6, 7] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
...
-- ──────────────────────────────────────────────────────────────────────────────
--  97. TL14_limit_treatment.csv   [总包 v11.1.0 行 5058~5136 · 输出 15 列]
--     原 ORDER BY：l.member_id, l.action_time
--     audit_rn 之排序键与分批作业版第 97 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`action_time`, y.`action_date`, y.`operator_id`, y.`operator_lv`, y.`grp_before`, y.`grp_after`, y.`max_hi_before`, y.`max_hi_after`, y.`n_grp_before`, y.`n_grp_after`, y.`direction`, y.`has_zero_group`, y.`is_baccarat_member`, y.`filter_rule_version`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  98. T_table_span.csv   [总包 v11.1.0 行 1447~1455 · 输出 6 列]
--     原 ORDER BY：n_rounds DESC
--     audit_rn 之排序键与分批作业版第 98 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_rounds` DESC, y.`table_id`, y.`mean_sec`, y.`p50`, y.`p90`, y.`p99`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
--  99. VB_00_validbet_contract.csv   [总包 v11.1.0 行 10675~10697 · 输出 13 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 99 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope`, y.`n_rows`, y.`n_vb_null`, y.`n_vb_blank`, y.`n_vb_zero`, y.`n_vb_pos`, y.`n_vb_neg`, y.`null_rate_pct`, y.`n_fallback_also_null`, y.`vb_sum`, y.`b13_sum`, y.`b41_sum`, y.`verdict_contract`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 100. VB_00b_zero_five_rates.csv   [总包 v11.1.0 行 10801~10869 · 输出 18 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 100 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`scope`, y.`n_base_rows`, y.`n_active_member`, y.`n_zero_rows`, y.`n_zero_member`, y.`zero_record_rate_pct`, y.`zero_amount_rate_pct`, y.`zero_member_rate_pct`, y.`zero_profit_rate_pct`, y.`zero_active_rate_pct`, y.`zero_stake_sum`, y.`zero_ggr_sum`, y.`all_stake_sum`, y.`all_ggr_sum`, y.`zero_mu0_pct`, y.`base_mu0_pct`, y.`n_zero_both`, y.`verdict_zero`) AS audit_rn
FROM (
  WITH ta AS (                                                                                        -- 公共表表达式：开启中间结果集 ta——公司测试线代理
    SELECT DISTINCT age001 AS aid FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'               -- 取列与过滤：铁律④b——公司测试线
  ),                                                                                                  -- 续行：收束上方的子查询或函数括号
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 101. VB_41_identity_global.csv   [总包 v11.1.0 行 10716~10750 · 输出 17 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 101 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`test`, y.`n_rows`, y.`n_both_null`, y.`n_null_asym`, y.`n_exact_equal`, y.`n_value_diff`, y.`pct_exact_equal`, y.`max_abs_diff`, y.`sum_abs_diff`, y.`d_min`, y.`d_p01`, y.`d_p05`, y.`d_p50`, y.`d_p95`, y.`d_p99`, y.`d_max`, y.`verdict_identity`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 102. VB_41b_identity_strata.csv   [总包 v11.1.0 行 10761~10792 · 输出 10 列]
--     原 ORDER BY：n_null_asym DESC, n_value_diff DESC, n_rows DESC
--     audit_rn 之排序键与分批作业版第 102 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_null_asym` DESC, y.`n_value_diff` DESC, y.`n_rows` DESC, y.`bet09_value`, y.`commission_value`, y.`n_both_null`, y.`max_abs_diff`, y.`b41_sum`, y.`vb_sum`, y.`verdict_stratum`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 103. VB_41b_identity_strata_v18b.csv   [总包 v11.1.0 行 10876~10930 · 输出 18 列]
--     原 ORDER BY：n_null_asym DESC, n_value_diff DESC, n_rows DESC, bet09_value, commission_value
--     audit_rn 之排序键与分批作业版第 103 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 104. VB_41c1_964_detail.csv   [总包 v11.1.0 行 10939~10970 · 输出 22 列]
--     原 ORDER BY：grp, dt, bet_time, member_id, bet01
--     audit_rn 之排序键与分批作业版第 104 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`grp`, y.`dt`, y.`bet_time`, y.`member_id`, y.`bet01`, y.`table_id`, y.`shoe_id`, y.`bet09`, y.`commission`, y.`category`, y.`rebet_flag`, y.`agent_lv1`, y.`stake_raw`, y.`fx_rate`, y.`payout_raw`, y.`rebate_raw`, y.`net_raw`, y.`bet41`, y.`validbet`, y.`diff_vb_minus_b41`, y.`updatetime`, y.`sync_time`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 107. V_ipmatch.csv   [总包 v11.1.0 行 3017~3029 · 输出 5 列]
--     原 ORDER BY：b.n_member_log DESC
--     audit_rn 之排序键与分批作业版第 107 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_member_log` DESC, y.`ip`, y.`country`, y.`city`, y.`in_bet_ip`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 108. V_recon.csv   [总包 v11.1.0 行 1506~1576 · 输出 12 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 108 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_matched_round`, y.`mae_turnover`, y.`mae_rebate`, y.`mae_net`, y.`scale_turnover`, y.`scale_rebate`, y.`scale_net`, y.`mase_turnover`, y.`mase_rebate`, y.`mase_net`, y.`pct_exact_turnover`, y.`pct_exact_net`) AS audit_rn
FROM (
  ...
  rk AS (                                                                                             -- 公共表表达式：开启中间结果集 rk，其后各行为其定义体（交付件 V_recon.csv）
    SELECT b.bet01, b.bet03, b.bet04, b.bet05, b.bet11, b.bet13, b.bet16, b.bet17,                    -- 取列：起始取列子句，本行先列 b.bet01, b.bet03, b.bet04, b.bet05, b.bet11, b.bet13, b.bet16, b.bet17，涉 bet03（靴号）、bet04（局内序号）、bet05（会员号）
           b.bet38, b.bet39, b.category,                                                              -- 续行：接续上一取列子句，续列 b.bet38, b.bet39, b.category，涉 bet38（重對）、bet39（桌号）
           b.bet18, b.bet19, b.bet20, b.bet21, b.bet22,                                               -- 续行：接续上一取列子句，续列 b.bet18, b.bet19, b.bet20, b.bet21, b.bet22，涉 bet18（一级代理线）、bet19（二级代理线）、bet20（三级代理线）
           ROW_NUMBER() OVER (                                                                        -- 行号窗口表达式：以行号窗口取每组头部或去重，免出重复行
             PARTITION BY b.bet01                                                                     -- 窗口分区：按 b.bet01 分组开窗，组内各自编号或排名
             ORDER BY b.updatetime DESC, b.sync_time DESC, b.dt DESC) AS rn                           -- 排序：按 b.updatetime（降序）, b.sync_time（降序）, b.dt（降序）) AS rn 排列；导出必带排序，否则分页无稳定序（曾致 36.49% 重复行）
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 109. V_table_counts.csv   [总包 v11.1.0 行 6092~6101 · 输出 2 列]  [🚩第 [1, 2] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 110. X_combo.csv   [总包 v11.1.0 行 2776~2857 · 输出 6 列]
--     原 ORDER BY：1 DESC,2 DESC
--     audit_rn 之排序键与分批作业版第 110 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`flag_ip`, y.`flag_tail_p90`, y.`n_member`, y.`valid_bet`, y.`game_pnl`, y.`roi`) AS audit_rn
FROM (
  WITH ta AS (            -- 公司测试线代理（214 条，跨五级）
    SELECT DISTINCT age001 AS aid                                                                     -- 取列：产出「aid」
    FROM ods_mariadb_2b.ods_a168_agent WHERE age022 = '1'                                             -- 取数来源：取自代理主档（五级代理线归属来源）
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 111. Z01_table_inventory.csv   [总包 v11.1.0 行 561~579 · 输出 5 列]
--     原 ORDER BY：疑似性质, TABLE_NAME
--     audit_rn 之排序键与分批作业版第 111 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`疑似性质`, y.`表名`, y.`表类型`, y.`估计行数`, y.`表注释`) AS audit_rn
FROM (
  SELECT
      TABLE_NAME                                                   AS 表名,
      TABLE_TYPE                                                   AS 表类型,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 112. Z02_excluded_tables.csv   [总包 v11.1.0 行 589~608 · 输出 4 列]
--     原 ORDER BY：处置, TABLE_NAME
--     audit_rn 之排序键与分批作业版第 112 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`处置`, y.`表名`, y.`估计行数`, y.`表注释`) AS audit_rn
FROM (
  SELECT
      TABLE_NAME                          AS 表名,
      TABLE_ROWS                          AS 估计行数,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 113. Z03_column_dictionary.csv   [总包 v11.1.0 行 619~630 · 输出 8 列]
--     原 ORDER BY：TABLE_NAME, ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 113 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`表名`, y.`列序`, y.`列名`, y.`存储类型`, y.`可空`, y.`原始类型`, y.`业务含义`, y.`原始注释`) AS audit_rn
FROM (
  SELECT
      TABLE_NAME                                                        AS 表名,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 114. Z04_name_collision.csv   [总包 v11.1.0 行 644~667 · 输出 6 列]
--     原 ORDER BY：类型数 DESC, 释义数 DESC, 出现表数 DESC, COLUMN_NAME
--     audit_rn 之排序键与分批作业版第 114 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`类型数` DESC, y.`释义数` DESC, y.`出现表数` DESC, y.`列名`, y.`风险`, y.`明细`) AS audit_rn
FROM (
  WITH d AS (
    SELECT
        COLUMN_NAME,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 115. Z05_missing_typehint.csv   [总包 v11.1.0 行 676~695 · 输出 6 列]
--     原 ORDER BY：状态, TABLE_NAME, ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 115 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`状态`, y.`表名`, y.`列序`, y.`列名`, y.`存储类型`, y.`原始注释`) AS audit_rn
FROM (
  SELECT
      TABLE_NAME                        AS 表名,
      ORDINAL_POSITION                  AS 列序,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 116. Z06_generated_probe_sql.csv   [总包 v11.1.0 行 709~730 · 输出 5 列]
--     原 ORDER BY：t.TABLE_ROWS DESC, c.TABLE_NAME, c.ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 116 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`表名`, y.`列序`, y.`列名`, y.`估计行数`, y.`待执行SQL`) AS audit_rn
FROM (
  SELECT
      c.TABLE_NAME                                          AS 表名,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 117. Z07_probe_bet02.csv   [总包 v11.1.0 行 743~782 · 输出 5 列]  [🚩第 [1, 2, 3, 4, 5] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 118. Z08_roundkey_integrity.csv   [总包 v11.1.0 行 796~833 · 输出 9 列]  [🚩第 [9] 列无别名 → 不套 audit_rn]
-- ──────────────────────────────────────────────────────────────────────────────
-- 🚩 本件投影有列无可用别名（多为 UNION 分支之字面量列），排序键无法列全，
--    套壳后既不成全序，派生表列名亦可能重名而报错。故**原样保留，一次导全**。
WITH v AS (
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 119. Z09_time_axis_recon.csv   [总包 v11.1.0 行 854~885 · 输出 17 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 119 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`行数`, y.`dt等于账务日`, y.`dt等于下注日`, y.`账务日等于下注日`, y.`账务日不可用`, y.`开局时间不可用`, y.`下注时间不可用`, y.`下注时间纪元哨兵`, y.`开局时间纪元哨兵`, y.`账务日最小`, y.`账务日最大`, y.`下注早于开局_时序异常`, y.`下注距开局秒_最小`, y.`下注距开局秒_P50`, y.`下注距开局秒_P99`, y.`下注距开局秒_P999`, y.`下注距开局秒_最大`) AS audit_rn
FROM (
  WITH v AS (
    SELECT
        dt,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 120. Z11_sentinel_impact.csv   [总包 v11.1.0 行 919~957 · 输出 13 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 120 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`未处置_注单行数`, y.`已处置_注单行数`, y.`未处置_局键数`, y.`已处置_局键数`, y.`未处置_会员数`, y.`已处置_会员数`, y.`未处置_IP数`, y.`已处置_IP数`, y.`未处置_单局最大会员数`, y.`已处置_单局最大会员数`, y.`单局最大会员数_差额`, y.`剔除行数`, y.`剔除占比`) AS audit_rn
FROM (
  WITH v AS (
    SELECT
        CAST(bet05 AS VARCHAR) AS member_id,
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 122. _P12b-pre.csv   [总包 v11.1.0 行 9791~9799 · 输出 4 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 122 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`member_id`, y.`action`, y.`content`, y.`action_time`) AS audit_rn
FROM (
  SELECT  CAST(lmc02 AS STRING)                AS member_id,                                          -- 取值表达式：取用 lmc02（被改会员号），产出「member_id」
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 123. _PH0_1b_table_inventory.csv   [总包 v11.1.0 行 12393~12445 · 输出 10 列]
--     原 ORDER BY：h_src, table_nature, t.TABLE_NAME
--     audit_rn 之排序键与分批作业版第 123 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`h_src`, y.`table_nature`, y.`table_name`, y.`has_uid`, y.`has_time`, y.`has_amount`, y.`n_cols`, y.`scope_db`, y.`scope_pattern`, y.`scope_scanned_at`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 124. _PH0_2b_column_inventory.csv   [总包 v11.1.0 行 12457~12505 · 输出 11 列]
--     原 ORDER BY：c.TABLE_NAME, c.ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 124 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`table_name`, y.`col_pos`, y.`column_name`, y.`data_type`, y.`column_comment`, y.`name_style`, y.`col_family`, y.`fp_flag`, y.`scope_db`, y.`scope_pattern`, y.`scope_scanned_at`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 125. _PH0_3_lmc_category.csv   [总包 v11.1.0 行 12598~12615 · 输出 12 列]
--     原 ORDER BY：n_events DESC, lmc04_category
--     audit_rn 之排序键与分批作业版第 125 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_events` DESC, y.`lmc04_category`, y.`n_target_id`, y.`n_operator`, y.`dt_min`, y.`dt_max`, y.`n_in_win`, y.`avg_len_content`, y.`avg_len_sql`, y.`n_content_hit_mem01617`, y.`n_sql_hit_mem01617`, y.`n_hit_rg_semantics`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 126. _PH0_4L1_member_columns.csv   [总包 v11.1.0 行 12811~12820 · 输出 5 列]
--     原 ORDER BY：c.TABLE_NAME, c.ORDINAL_POSITION
--     audit_rn 之排序键与分批作业版第 126 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`table_name`, y.`col_pos`, y.`column_name`, y.`data_type`, y.`column_comment`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
    c.TABLE_NAME                                                           AS table_name,             -- 取列：表名
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 127. _PH0_4b_rg_keyword.csv   [总包 v11.1.0 行 12664~12694 · 输出 11 列]
--     原 ORDER BY：n_events DESC, lmc04_category
--     audit_rn 之排序键与分批作业版第 127 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`n_events` DESC, y.`lmc04_category`, y.`rg_self_exclusion`, y.`rg_cooling_off`, y.`rg_responsible`, y.`biz_limit`, y.`biz_freeze`, y.`biz_disable`, y.`biz_block`, y.`amb_exclude_any`, y.`n_in_win`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 128. _PH0_4w2_balance_points.csv   [总包 v11.1.0 行 12777~12799 · 输出 12 列]
--     原 ORDER BY：（无）
--     audit_rn 之排序键与分批作业版第 128 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`dt_slice`, y.`n_user_active`, y.`n_pts_total`, y.`pts_avg`, y.`pts_min`, y.`pts_p50`, y.`pts_p90`, y.`pts_p99`, y.`pts_max`, y.`pts_ok_avg`, y.`n_user_single_pt`, y.`pct_single_pt`) AS audit_rn
FROM (
...
-- ──────────────────────────────────────────────────────────────────────────────
-- 129. _PH0_6b_member_daily.csv   [总包 v11.1.0 行 13015~13025 · 输出 6 列]
--     原 ORDER BY：dt
--     audit_rn 之排序键与分批作业版第 129 件**逐字同一**（锁一）
-- ──────────────────────────────────────────────────────────────────────────────
SELECT y.*,
       ROW_NUMBER() OVER (ORDER BY y.`dt_day`, y.`n_rows`, y.`n_mem001`, y.`rows_per_key`, y.`n_enable_N`, y.`n_canbet_N`) AS audit_rn
FROM (
  SELECT                                                                                              -- 取列：起始取列子句
    dt                                                                     AS dt_day,                 -- 取列：分区日
