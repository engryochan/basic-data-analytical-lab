/* ═══════════════════════════════════════════════════════════════════════
   ...
   ▸ 判读契约（先写死，禁止事后择利解释）：
       · n_vb_null > 0 且占比高            → 支持①，F-02 之 §S-05 归属可判为直取口径
       · n_vb_null = 0 且 n_hedge_eq 高     → 支持②，另得 T-05 高纯度标注样本
...
SELECT m.member_id                                                         AS member_id,
       ...
       CASE WHEN COALESCE(m.sum_valid_bet,0) = 0 AND ABS(COALESCE(m.sum_game_pnl,0)) < 0.005
                 THEN '洗码与输赢皆为零'
...
WHERE m.n_orders >= 30
  AND (COALESCE(m.sum_valid_bet,0) = 0
       OR m.sum_valid_bet IS NULL
       OR ABS(COALESCE(m.sum_game_pnl,0)) < 0.005
