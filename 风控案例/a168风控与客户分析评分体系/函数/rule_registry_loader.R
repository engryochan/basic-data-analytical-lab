...
# ---------------------------------------------------------------------
# 版本 : 0.1.4        日期 : 2026-09-03        适配 : typology 1.5.004 / rules 0.1.3
# 变更 : N-7 —— 版本串 1.5.003 → 1.5.004；RULES_PATH 改指 rule_registry_v0.1.3.yaml。闸门 G1~G6 逻辑未动。
...
RULES_PATH <- file.path("规范", "rule_registry_v0.1.3.yaml")
RISKEYE_PATH <- file.path("规范", "risk_eye_schema_v0.1.0.yaml")

rules_load <- function(REG, path = RULES_PATH) {
  y <- yaml::read_yaml(path)
  if (!identical(y$registry$parent, "registry_risk_typology_v1.5.004"))
    stop("规则册 parent 非 v1.5.004", call. = FALSE)
  if (!identical(as.character(REG$meta$registry$version), "1.5.004"))
    stop("载入之登记册非 v1.5.004", call. = FALSE)
