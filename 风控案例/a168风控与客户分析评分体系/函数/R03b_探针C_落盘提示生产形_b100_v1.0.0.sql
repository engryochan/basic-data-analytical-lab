-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- R03b ＃071 · 验收探针 C · 落盘提示之生产形 · 第 100 批   v1.0.0   2026-09-05
-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- 【本探针之本体】系【自现行总包 ＃071（分批作业版1万）逐字复制】，其排序键已是裁剪后之 5 键。
--   相对生产查询，本探针只改【两处】：
--     ① 最外层 SELECT 加落盘提示 /*+ SET_VAR(...) */ —— 提示只改执行方式，不改任何列值与行序。
--     ② 批次窗改为第 100 批（audit_rn 990,001~1,000,000），以便与现档逐字节比对。
-- 【提示之三值系实测所定，非臆断】（探针 M2／M3）
--   enable_spill               会话默认 false ⇒ 落盘根本不发生，此即撑爆之直接成因，故须显式开启。
--   spill_mode = 'auto'        会话默认已是 auto（用到水位才落盘），此处显式写出以自证。
--   spill_mem_limit_threshold  会话默认已是 0.8（80% 水位），此处显式写出以自证。
-- 【⛔ 何以必用提示而非 SET —— 已实测判死】
--   探针 M3 之 P-6／P-7：SET enable_spill = true 单独执行后，另一次执行读回 @@enable_spill = 0。
--   ⇒ Superset SQL Lab【每次执行不复用会话】，SET 出去的变量下一句即失。
--   ⇒ 故落盘设定【必须逐句携带】，别无他法。
-- 【只读】纯 SELECT。
-- 【验收法】导出后与现档 R03b_player_dealer_daily_b100.csv 逐字节比对
--   （10,000 行 · 151 栏 · 15,614,711 B · MD5 e1503f6e29f189f9676fd0f7a7ee580f）。
--   ★ 完全一致方可入生产；有一字之差即弃此案。
-- ══════════════════════════════════════════════════════════════════════════════════════════════

SELECT /*+ SET_VAR(enable_spill=true,spill_mode='auto',spill_mem_limit_threshold=0.8) */ z.*,
