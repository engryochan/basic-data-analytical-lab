-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- R03b ＃071 排序键裁剪 · 验收探针 0 · 全量唯一性与 T_true   v1.0.0   2026-09-05
-- ══════════════════════════════════════════════════════════════════════════════════════════════
...
-- 【本探针所答之问】
--   问一：(bet_date, uid, dealer_id) 于【全量】上是否唯一？
--         —— 若唯一，则 audit_rn 之第 6~26 个排序键【永无机会断平手】，裁至 5 键必不改 audit_rn。
--         —— 在下本地只能以【已落盘之 365 万行】证之（严格递增、零违例）；其后之行未验，故须本探针定谳。
--   问二：T_true 究竟几何？—— 顺带得知尚余几批。
-- 【判读】
--   n_dup_groups = 0  且  t_true = n_distinct_key3   ⇒ ★ 全域唯一，裁剪安全，可放行
--   n_dup_groups > 0                                  ⇒ ⛔ 有平手，裁剪【禁止】，此案作废
-- 【何以比生产查询轻】本探针无 ROW_NUMBER、无全局排序、无 151 栏物化；
--   GROUP BY 由哈希分片分散于各 BE，不需单节点汇总。
-- ══════════════════════════════════════════════════════════════════════════════════════════════

SELECT SUM(g.c)                                       AS t_true,
       COUNT(*)                                       AS n_distinct_key3,
       SUM(CASE WHEN g.c > 1 THEN 1 ELSE 0 END)       AS n_dup_groups,
       SUM(CASE WHEN g.c > 1 THEN g.c ELSE 0 END)     AS n_rows_in_dups,
       MAX(g.c)                                       AS max_group_size
FROM (
  SELECT COUNT(*) AS c
  FROM (
    WITH
  ...
  GROUP BY y.`bet_date`, y.`uid`, y.`dealer_id`
) g;
