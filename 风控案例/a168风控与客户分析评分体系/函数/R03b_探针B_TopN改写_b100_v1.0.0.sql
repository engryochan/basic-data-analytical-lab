-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- R03b ＃071 排序键裁剪 · 验收探针 B · 5 键 ＋ 去恒等 CAST ＋ TopN 改写 · 第 100 批   v1.0.0   2026-09-05
-- ══════════════════════════════════════════════════════════════════════════════════════════════
...
-- 【本探针所改之三处】
--   ① audit_rn 之 ORDER BY 由 26 键裁为 5 键（同探针 A）。
--   ② LEFT JOIN 两侧之 CAST(… AS STRING) 去除 —— e.x_key 与 q.uid 皆源自同一列 b.bet05，
--      同型；CAST(varchar AS STRING) 系恒等转换，故 CAST(a)=CAST(b) 与 a=b 逐位等价。
--   ③ 「先全量 ROW_NUMBER 再 WHERE」改为「ORDER BY … LIMIT/OFFSET 再补 audit_rn」。
-- 【③ 何以输出必然相同】
--   前 5 键已证为【全序】（3,650,000 行严格递增、零违例；全域待探针 0 定谳），
--   故 LIMIT/OFFSET 之取位无歧义；其后 ROW_NUMBER 于这 10,000 行上给出 1..10000，
--   加 OFFSET 990000 即原 audit_rn。
-- 【③ 何以省内存】StarRocks 之 TopN 只需维持 offset+limit 之有界堆，
--   而非把全量（T_true × 151 栏）物化后作单节点全局排序。
-- 【⛔ 本项在下【未经在库实测】，不敢担保 —— 故须本探针取证。】
-- 【验收法】同探针 A：与现档 R03b_player_dealer_daily_b100.csv 逐字节比对。
--   ★ 若探针 B 一致，则 B 涵盖 A，径用 B；若 B 不一致而 A 一致，则退用 A。
-- ══════════════════════════════════════════════════════════════════════════════════════════════

SELECT v.*,
       CAST(FLOOR((v.audit_rn - 1) / 10000) + 1 AS INT)          AS batch_id,
       'A168_HF9F_20260827_0900' AS run_id,
       '2026-08-27 09:00:00' AS snapshot_sync_time
FROM (
  SELECT w.*,
         990000 + ROW_NUMBER() OVER (ORDER BY w.`residual_b` ASC, w.`roi` DESC, w.`bet_date`, w.`uid`, w.`dealer_id`) AS audit_rn
  FROM (
    SELECT y.*
    FROM (
    WITH
    ...
    SELECT q.*,
      ...
      ROUND((-e.net-e.rebate-e.agent_cost_chain_nested)/NULLIF(e.stake,0),8) AS net_margin_chain_nested_rate_canonical,
       ROUND((-e.net-e.rebate-e.agent_cost_a_legacy)/NULLIF(e.stake,0),8) AS residual_a_rate,
           ...
           --  └────────────────────────────────────────────────────────────────┘
    FROM q
    LEFT JOIN x_agg e ON e.x_key = q.`uid`
    ) y
    ORDER BY y.`residual_b` ASC, y.`roi` DESC, y.`bet_date`, y.`uid`, y.`dealer_id`
    LIMIT 10000 OFFSET 990000                          -- ★ 第 100 批：OFFSET = (k-1)*10000
  ) w
) v
ORDER BY v.audit_rn;
