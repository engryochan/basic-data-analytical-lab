/* ╔═══════════════════════════════════════════════════════════════════════════╗
   ...
   ═══ 分析窗口（正名版，务必按此措辞对外）═══════════════════════════════════
     ...
     改窗时须同时全量替换两枚窗口字面量（v5 实测：'2026-03-21' 共 175 处／'2026-08-07' 共 244 处），不得只改其一；
   ...
   ═══ 血统关系登记（2026-08-13 立 · 一族四档）════════════════════════════
     本包为「取数层」，居一族四档之料源位。改本包即改全族之数字，故列明：
       ...
       a168_取数与核验_SQL总包_v5.sql               本包(料源)   一切数字之唯一出处
   ...
   【第六批 · 序列与事件（评估线 P12 族，2026-08-20 v5 并入）】
     ⚠ 编号辨义：P12x 系评估档（尾段投注基础分析的评估）待表清单之编号，
       与卷一「P-01」（连字符式，会员月度面板）无涉，勿混。
     §P12b-pre → 只看屏幕（content 段式 20 行目检，先于 §P12b）
     §P12a → 数据库/P12a_平台日度序列.csv     dt×1 行 139 行；Regime/监控带口粮
     §P12b → 数据库/P12b_会员事件表.csv       uid×1；生存分析（标记时点/流失/右删失）
     §P12c → 数据库/P12c_会员逐日盈亏.csv     uid×dt（n>=100 域）；MDD/Sortino 指纹
   ...
   ╚══════════════════════════════════════════════════════════════════════════╝
     ...
     配套三条：
       ...
       ③ 全包现状（2026-08-12 逐行实测；计数口径＝行首以 -- 起首的导出标记行）：
          产出交付件的语句 67 条（新式 45 条 + 卷一旧式块注释 22 条；v5 并入 §P12a/§P12b/§P12c 三条，§P12b-pre 只看屏幕不计，§P12d 系改窗指引无独立语句），
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12a · 平台日度序列（存为「数据库/P12a_平台日度序列.csv」）
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12b · 会员事件表（存为「数据库/P12b_会员事件表.csv」）
...
/* ─────────────────────────────────────────────────────────────────────────
   §P12c · 会员逐日盈亏（存为「数据库/P12c_会员逐日盈亏.csv」）
