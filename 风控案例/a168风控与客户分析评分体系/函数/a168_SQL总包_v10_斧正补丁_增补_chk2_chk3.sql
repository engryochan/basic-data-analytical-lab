...
SELECT                                                                                              -- 主查询：960 笔逐笔明细（全量，不抽样、不分页）
    ...
    CASE WHEN m.category = '1'                                                                      -- 条件分支：是否落在 §P11c 全量洁净域内——
          ...
          AND NOT EXISTS (SELECT 1 FROM test_agents t                                               -- 续行：五级代理线任一命中测试线即出域
                          WHERE t.agent_id IN (m.bet18, m.bet19, m.bet20, m.bet21, m.bet22))        -- 续行：单列 IN 多值，非多列 IN，StarRocks 支持
...
flagged AS (                                                                                        -- 逐行打七面闸旗（1 = 通过该闸）
    SELECT r.bet01,                                                                                 -- 取列：注单号
           ...
           CASE WHEN NOT EXISTS (SELECT 1 FROM test_agents t                                        -- 条件分支：闸三·五级代理线无测试线——
                                 WHERE t.agent_id IN (r.bet18, r.bet19, r.bet20,                    -- 续行：单列 IN 多值
                                                      r.bet21, r.bet22))                            -- 续行：五级全查
