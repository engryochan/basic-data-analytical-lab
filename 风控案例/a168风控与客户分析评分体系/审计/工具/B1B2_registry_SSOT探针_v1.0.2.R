#!/usr/bin/env Rscript
# ══════════════════════════════════════════════════════════════════════════
# B-1 ／ B-2 · Registry SSOT 投影一致性 ＋ loader 六元组重锚  探针 v1.0.2
...
# 用法：Rscript 审计/工具/B1B2_registry_SSOT探针_v1.0.2.R
...
cat(strrep("=", 92), "\n")
cat("B-1 / B-2 · Registry SSOT 探针 v1.0.2（★ 结构先探 · 来源层计数 · 防崩）\n")
cat("工作目录：", getwd(), "\n")
cat(strrep("=", 92), "\n\n")
...
if (file.exists(P_LOADER)) {
  ...
  if (reads_csv && reads_yaml && !has_assert) {
    ...
    note("B1", "L4", "loader SSOT 断言", "有断言，待逐条核实", FALSE)
  }
} else {
