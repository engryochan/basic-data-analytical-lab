/* ╔═══════════════════════════════════════════════════════════════════════════╗
   ...
   ⚠ 2026-08-18 注释斧正（代码零改动）：shoe_len 之定义，其注释与 CTE 名历来称其为
         ...
         ★★ 2026-08-23 全档现算斧正（旧文「三处」与「皆 GROUP BY shoe_id, table_id」
            ...
            二者非等价：不含桌号者会把**异桌同号靴合并**，其 max_round 被系统性夸大，
            致该二处之 shoe_pos 被系统性**压低**（分母偏大）。此为口径差异，非精度差异。
            ⚠ 本轮只斧正注释、不动代码：该二处已就地加粒度警示横幅；
              是否统一粒度须另立变更单，并同步下游三档之 must= 清单。
            ⚠ 此差异与新立之 §R01-chk 族所验者同源——该族之 n_shoes_delta 与
              avg_maxround_inflation 两列，正为量化此类粒度污染而设。
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ★ 粒度辨义（本族之核心）：
     · 旧粒度 = (shoe_id, table_id)  —— §R01 现行，隐含「靴号全局唯一」之假设
     · 新粒度 = (dt, shoe_id, table_id) —— 本族并报，若 bet03 系日内循环序号
       则旧粒度会把异日同号靴合并，MAX(round_no) 被系统性**夸大**
     二者差额即粒度污染之量，由 §R01-chk 之 n_shoes_delta 与
     avg_maxround_inflation 两列直接给出。
   ★★ 裁定纪律（永久生效，不因本族结果而松动）★★
...
--   ★ 判读：pct_cross_day > 0 即证 bet03 系日内／场次循环序号，则旧粒度
--     (shoe_id, table_id) 会把异日同号靴合并，MAX(round_no) 被系统性夸大，
--     §R01-chk 起一律以新粒度为准；pct_cross_day = 0 则两粒度等价，旧粒度可留用。
...
--   ★ 判读三处：
--     ① corr_density_verdict —— CI 跨 0 者方向不可读；CI > 0 即分母低估假说得证；
--        CI < 0 则与低估假说反向（密桌反而靴短），指向切牌规程异质。
...
shoe_len AS (          /* ★★ 命名与粒度双警示（2026-08-23 现算立）★★ 一、本 CTE 名为 shoe_len（靴长）系历史沿用，所算 max_round = MAX(round_no) 为『最大局号』非『局数』，下游切勿以 COUNT 重实现。二、本处 GROUP BY 只含 bet03、**不含桌号**，与 §R01／§B01／§B01-D／§P11b 之 (shoe_id, table_id) 粒度不同：异桌同号靴会被合并，max_round 被系统性夸大，本条之 shoe_pos 因而被系统性压低。此为口径差异非精度差异；改动须另立变更单。 */
...
shoe_len AS (          /* ★★ 命名与粒度双警示（2026-08-23 现算立）★★ 一、本 CTE 名为 shoe_len（靴长）系历史沿用，所算 max_round = MAX(round_no) 为『最大局号』非『局数』。二、本处 GROUP BY 只含 bet03、**不含桌号**，与 §R01／§B01／§B01-D／§P11b 之 (shoe_id, table_id) 粒度不同：异桌同号靴会被合并，max_round 被系统性夸大，本条之 shoe_pos 因而被系统性压低。此为口径差异非精度差异；改动须另立变更单。 */
