# SC-15 · 经济 KPI 规格 v1.1.0

**日期**：2026-08-23　**置放**：`规范/`　**性质**：规格冻结（**不改任何既有档**）
**版级**：`v1.0.0 → v1.1.0`（MINOR）。理由见第九节之「文档类级差判据」——
本轮所增系**边界条件与新规则**（非负截断、噪声地板、文档类级差判据本身），
既有定义之方向一字未反，属**收窄**非**反转**，故判 MINOR 而非 MAJOR。
...
$$\text{risk\_weighted\_exposure} \;=\; P(\text{Risk})\times\text{stake}
\qquad\text{（风险加权流水暴露，}\textbf{非损失}\text{）}$$

$$\boxed{\;\mathbb{E}[L] \;=\; P(\text{Risk}) \times \text{stake} \times \max\bigl(0,\ \hat{\mu}_i - \mu_0\bigr)\;}$$

其中 $\hat{\mu}_i$ 为该主体**收缩后**之前瞻 ROI，$\mu_0 = -\text{hold\%}$ 为零假设。
**只有超出 $\mu_0$ 的那一部分才是损失**；其余是赌场本来就该赚到而没赚到的抽水，属经营常态。

**算例**：域内 hold ＝ 2%（故 $\mu_0=-2\%$），某会员实测 ROI ＝ +0.5%，流水 100 万。
超额 $=0.5\%-(-2\%)=2.5\%$ → 风险增量 $=1{,}000{,}000\times2.5\%=25{,}000$。
**不是把 100 万流水整个当损失。**

> 此误与「GGR ＝ 利润」同类：**把营收当利润，把流水当损失。** 二者皆须在本册第一节即封死。

### R4-6a · 非负截断之噪声地板（本轮新增，与截断同批锁死）

$\max(0,\cdot)$ 于**逐员**层面正确，然**一加总即出事**：

$$\Sigma_i \max\bigl(0,\ \hat\mu_i-\mu_0\bigr)\times\text{stake}_i \;>\; 0
\qquad\textbf{恒成立，即便全体皆纯噪声}$$

零假设下 $\hat\mu_i-\mu_0$ 对称分布于 0 两侧，约半数为正；截负留正再求和，
**必得一个虚假的正数**。此与本项目已现算证伪之「`ROI > 0` 命中主体系方差」
系**同一失效模式**，只是这次伪装成了「可避免损失」之金额。

**三条同批锁死**：

1. $\hat\mu_i$ **必用收缩后估计**（向 $\mu_0$ 收缩，权 ∝ 暴露量），**禁用实测 ROI**；
2. **并报噪声地板** $\;\mathcal{F}_0=\mathbb{E}_{H_0}\bigl[\Sigma\max(0,\varepsilon_i)\cdot\text{stake}_i\bigr]$，
   以自助法现算（预注册种子 `20260819`，`B = 1000`）；
3. **可避免损失恒报净额**：

$$\text{avoidable\_loss}_{\text{net}}=\Sigma_i\max\bigl(0,\hat\mu_i-\mu_0\bigr)\text{stake}_i-\mathcal{F}_0$$

**净额不显著大于 0 者判 `NO_ACTIONABLE_LOSS`，不得上报任何金额。**

> 否则经营层会拿到一个「T-05 可挽回 250 万」之数字，而其中大半是运气。
...
## 〇之二、`offset_rate` 正式废名

含糊之 `offset_rate` **废弃，不再作为任何正式字段名**；一律改用
`stake_symmetry`（金额对称度）与 `pnl_offset_rate`（盈亏抵销度）二者。
凡遗留 `offset_rate` 者，落地前须逐处正名，否则判 `FAIL`。
...
5. **截负求和必有虚正。** 凡出 `avoidable_loss` 者，**必并报噪声地板与净额**；只报毛额者判 `FAIL`。

---

## 九、文档类级差判据（本轮新立）

现行 `版本编号纪律 v1.0.0` 之级差判据（纯代码 MD5／`schema_diff`）系为 **SQL 与 schema** 而立，
文档类无从套用。故另立，并入下一次 `版本编号纪律` 之修订：

| 级 | 判据 | 例 |
|:--|:--|:--|
| **PATCH** | 措辞、补注、勘误、排版；**已锁定义一字不动** | 错别字、行号更新 |
| **MINOR** | **新增**定义／规则／边界条件；既有定义之方向不反 | 本轮：非负截断、噪声地板、本表 |
| **MAJOR** | **改既有定义之语义或反转其方向**；下游据旧定义所出之结论须重算 | 若将来把 $\mu_0$ 由 $-\text{hold\%}$ 改为 0 |

**本轮自评**：`max(0,\cdot)` 系于既有式上**加边界条件**，未反转任何既有结论
（原式之正值区间取值不变，仅负值区间由「记负」改为「记零」），属**收窄**非**反转**
→ 判 **MINOR**，`v1.0.0 → v1.1.0`。

⚠ 本判据**自身**须经裁示方入 `版本编号纪律`；在其正式入册前，本节只作本册之内规。
...
系**子集读取**（读己所需、忽略未知键）抑或**全等校验**（键集须逐字相同）？

| 判定 | 推论 |
|:--|:--|
| 子集读取 | `economic_metrics:` 属 additive → `v1.5.1` → `v1.6.0` 即足，**无须 `v2.0.0`** |
| 全等校验 | 新增区块即 breaking → **先升 loader、后升登记册**，次序不可颠倒 |

探针已备：`审计/工具/A_P0_registry_loader_schema_probe.R`（静态模式扫描 ＋
tempdir 内注入未知键之动态实证，**不改任何在册档**）。

**在本探针出结论之前：禁宣布 `v1.6.0`，禁开 P1 之 SQL 施工。**

### P1 施工序（承裁示第九节，次序不可颠倒）

```
P1-0  核验 bet16 / validbet 在源表之真实语义（禁因「存在于原始表」即默认可用）
P1-1  核验二者能否按各自 grain 正确聚合
P1-2  定字段名、单位、分母
P1-3  定恒等式与 QA gate
P1-4  定各 T 类 economic contract
P1-5  查 registry_loader schema 行为（← 本节探针）
P1-6  定 v1.5.1 / v1.6.0 / v2.0.0
P1-7  方允许 SQL 施工
```
